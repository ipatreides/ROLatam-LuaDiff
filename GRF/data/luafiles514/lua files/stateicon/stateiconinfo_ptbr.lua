StateIconList_string = {
	[EFST_IDs.EFST_PROVOKE] = {
		descript = {
			{"Provocar"},
			{"%s"},
			{
				"DEF reduzida"
			},
			{
				"ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_ENDURE] = {
		descript = {
			{"Vigor"},
			{"%s"},
			{
				"DEFM aumentada"
			},
			{
				"Evita reagir a ataques"
			}
		}
	},
	[EFST_IDs.EFST_TWOHANDQUICKEN] = {
		descript = {
			{
				"Rapidez com Duas Mãos"
			},
			{"%s"},
			{
				"Bônus de Vel.Atq"
			}
		}
	},
	[EFST_IDs.EFST_CONCENTRATION] = {
		descript = {
			{"Concentrar"},
			{"%s"},
			{
				"DES aumentada"
			},
			{
				"AGI aumentada"
			}
		}
	},
	[EFST_IDs.EFST_HIDING] = {
		descript = {
			{
				"Esconderijo"
			},
			{"%s"},
			{
				"Personagem invisível"
			},
			{
				"Pode ser revelado"
			}
		}
	},
	[EFST_IDs.EFST_CLOAKING] = {
		descript = {
			{
				"Furtividade"
			},
			{"%s"},
			{
				"Personagem invisível"
			}
		}
	},
	[EFST_IDs.EFST_ENCHANTPOISON] = {
		descript = {
			{
				"Envenenar Arma"
			},
			{"%s"},
			{
				"Arma de prop. Veneno"
			}
		}
	},
	[EFST_IDs.EFST_POISONREACT] = {
		descript = {
			{
				"Refletir Veneno"
			},
			{"%s"},
			{
				"Chance de autoconjurar"
			},
			{
				"Envenenar no alvo"
			},
			{
				"ao receber danos físicos"
			}
		}
	},
	[EFST_IDs.EFST_QUAGMIRE] = {
		descript = {
			{
				"Pântano dos Mortos"
			},
			{"%s"},
			{
				"AGI e DES reduzidas"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_ANGELUS] = {
		descript = {
			{"Angelus"},
			{"%s"},
			{
				"Bônus de DEF e HP"
			}
		}
	},
	[EFST_IDs.EFST_BLESSING] = {
		descript = {
			{"Bênção"},
			{"%s"},
			{
				"FOR, INT, DES e"
			},
			{
				"Precisão melhorados"
			}
		}
	},
	[EFST_IDs.EFST_CRUCIS] = {
		descript = {
			{
				"Signum Crucis"
			},
			{"%s"},
			{
				"DEF reduzida"
			}
		}
	},
	[EFST_IDs.EFST_INC_AGI] = {
		descript = {
			{
				"Aumentar Agilidade"
			},
			{"%s"},
			{
				"Bônus de AGI e Vel.Atq"
			},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_DEC_AGI] = {
		descript = {
			{
				"Diminuir Agilidade"
			},
			{"%s"},
			{
				"AGI reduzida"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_SLOWPOISON] = {
		descript = {
			{
				"Retardar Veneno"
			},
			{"%s"},
			{
				"Impede a perda de HP"
			},
			{
				"ao sofrer Envenenamento"
			}
		}
	},
	[EFST_IDs.EFST_IMPOSITIO] = {
		descript = {
			{
				"Impositio Manus"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM"
			}
		}
	},
	[EFST_IDs.EFST_SUFFRAGIUM] = {
		descript = {
			{"Suffragium"},
			{"%s"},
			{
				"Reduz a Conj. Variável"
			}
		}
	},
	[EFST_IDs.EFST_ASPERSIO] = {
		descript = {
			{"Aspersio"},
			{"%s"},
			{
				"Arma de prop. Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_BENEDICTIO] = {
		descript = {
			{
				"Armadura Abençoada"
			},
			{"%s"},
			{
				"Armadura de prop. Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_KYRIE] = {
		descript = {
			{
				"Kyrie Eleison"
			},
			{"%s"},
			{
				"Proteção contra ataques"
			}
		}
	},
	[EFST_IDs.EFST_MAGNIFICAT] = {
		descript = {
			{"Magnificat"},
			{"%s"},
			{
				"Acelera a regen."
			},
			{
				"natural de SP"
			}
		}
	},
	[EFST_IDs.EFST_GLORIA] = {
		descript = {
			{"Glória"},
			{"%s"},
			{
				"SOR aumentada"
			}
		}
	},
	[EFST_IDs.EFST_LEXAETERNA] = {
		descript = {
			{
				"Lex Aeterna"
			},
			{"%s"},
			{
				"Recebe o dobro de dano"
			}
		}
	},
	[EFST_IDs.EFST_ADRENALINE] = {
		descript = {
			{
				"Adrenalina Pura"
			},
			{"%s"},
			{
				"Bônus de Vel.Atq"
			},
			{
				"e de Precisão"
			}
		}
	},
	[EFST_IDs.EFST_WEAPONPERFECT] = {
		descript = {
			{
				"Manejo Perfeito"
			},
			{"%s"},
			{
				"Anula a penalidade"
			},
			{
				"de tamanho da arma"
			}
		}
	},
	[EFST_IDs.EFST_OVERTHRUST] = {
		descript = {
			{
				"Força Violenta"
			},
			{"%s"},
			{
				"Bônus de ATQ."
			}
		}
	},
	[EFST_IDs.EFST_MAXIMIZE] = {
		descript = {
			{
				"Amplificar Poder"
			},
			{"%s"},
			{
				"Causa o máx. de dano físico"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_RIDING] = {
		descript = {
			{"Montaria"}
		}
	},
	[EFST_IDs.EFST_FALCON] = {
		descript = {
			{
				"Ave de Rapina"
			},
			{
				"Domesticando Ave"
			}
		}
	},
	[EFST_IDs.EFST_TRICKDEAD] = {
		descript = {
			{
				"Fingir de morto"
			},
			{
				"Imune a ataques"
			}
		}
	},
	[EFST_IDs.EFST_SHOUT] = {
		descript = {
			{
				"Grito de Guerra"
			},
			{"%s"},
			{
				"Bônus de FOR e ATQ"
			}
		}
	},
	[EFST_IDs.EFST_ENERGYCOAT] = {
		descript = {
			{
				"Proteção Arcana"
			},
			{"%s"},
			{
				"Reduz o dano recebido"
			},
			{
				"em proporção ao SP atual"
			}
		}
	},
	[EFST_IDs.EFST_BROKENARMOR] = {
		descript = {
			{
				"Armadura quebrada"
			}
		}
	},
	[EFST_IDs.EFST_BROKENWEAPON] = {
		descript = {
			{
				"Arma quebrada"
			}
		}
	},
	[EFST_IDs.EFST_ILLUSION] = {
		descript = {
			{"Alucinação"},
			{"%s"},
			{
				"Visão invertida"
			},
			{
				"Recebe danos aleatórios"
			},
			{
				"que não são verdadeiros"
			}
		}
	},
	[EFST_IDs.EFST_WEIGHTOVER50] = {
		descript = {
			{
				"70% acima do peso"
			},
			{
				"Desativa a regen."
			},
			{
				"natural de HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_WEIGHTOVER90] = {
		descript = {
			{
				"90% acima do peso"
			},
			{
				"Não pode atacar"
			},
			{
				"ou usar habilidades"
			},
			{
				"Desativa a regen."
			},
			{
				"natural de HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_ATTHASTE_POTION1] = {
		descript = {
			{
				"Poção da Concentração"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_ATTHASTE_POTION2] = {
		descript = {
			{
				"Poção do Despertar"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_ATTHASTE_POTION3] = {
		descript = {
			{
				"Poção da Fúria"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_MOVHASTE_POTION] = {
		descript = {
			{"%s"},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_MOVHASTE_INFINITY] = {
		descript = {
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPWEAPON] = {
		descript = {
			{
				"Remover Arma"
			},
			{"%s"},
			{
				"Arma removida e não"
			},
			{
				"pode ser reequipada"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPSHIELD] = {
		descript = {
			{
				"Remover Escudo"
			},
			{"%s"},
			{
				"Escudo removido e não"
			},
			{
				"pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPARMOR] = {
		descript = {
			{
				"Remover Armadura"
			},
			{"%s"},
			{
				"Armadura removida e não"
			},
			{
				"pode ser reequipada"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPHELM] = {
		descript = {
			{
				"Remover Capacete"
			},
			{"%s"},
			{
				"Elmo removido e não"
			},
			{
				"pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTWEAPON] = {
		descript = {
			{
				"Revestir Arma"
			},
			{"%s"},
			{
				"Impede a arma de ser"
			},
			{
				"removida ou destruída"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTSHIELD] = {
		descript = {
			{
				"Revestir Escudo"
			},
			{"%s"},
			{
				"Impede o escudo de ser"
			},
			{
				"removido ou destruído"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTARMOR] = {
		descript = {
			{
				"Revestir Armadura"
			},
			{"%s"},
			{
				"Impede a armadura de ser"
			},
			{
				"removida ou destruída"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTHELM] = {
		descript = {
			{
				"Revestir Capacete"
			},
			{"%s"},
			{
				"Impede o elmo de ser"
			},
			{
				"removido ou destruído"
			}
		}
	},
	[EFST_IDs.EFST_AUTOGUARD] = {
		descript = {
			{"Bloqueio"},
			{"%s"},
			{
				"Chance de bloquear"
			},
			{
				"ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_REFLECTSHIELD] = {
		descript = {
			{
				"Escudo Refletor"
			},
			{"%s"},
			{
				"Reflete parte dos danos"
			},
			{
				"físicos ao atacante."
			}
		}
	},
	[EFST_IDs.EFST_PROVIDENCE] = {
		descript = {
			{
				"Divina Providência"
			},
			{"%s"},
			{
				"Resistência a raça Demônio"
			},
			{
				"e propriedade Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_DEFENDER] = {
		descript = {
			{
				"Aura Sagrada"
			},
			{"%s"},
			{
				"Resist. à distância"
			},
			{
				"Vel.Atq reduzida"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_WEAPONPROPERTY] = {
		descript = {
			{"Aspersio"},
			{"%s"},
			{
				"Arma de prop. Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_AUTOSPELL] = {
		descript = {
			{
				"Desejo Arcano"
			},
			{"%s"},
			{
				"Chance de autoconjurar a"
			},
			{
				"magia selecionada ao"
			},
			{
				"realizar ataques físicos."
			},
			{
				"Consome mais SP"
			}
		}
	},
	[EFST_IDs.EFST_SPEARQUICKEN] = {
		descript = {
			{
				"Rapidez com Lança"
			},
			{"%s"},
			{
				"CRIT aumentado"
			},
			{
				"Esquiva aumentada"
			},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_WHISTLE] = {
		descript = {
			{"Assovio"},
			{"%s"},
			{
				"Bônus de Esquiva"
			},
			{
				"e Esquiva Perfeita"
			}
		}
	},
	[EFST_IDs.EFST_ASSASSINCROSS] = {
		descript = {
			{
				"Crepúsculo Sangrento"
			},
			{"%s"},
			{
				"Bônus de Vel.Atq"
			}
		}
	},
	[EFST_IDs.EFST_POEMBRAGI] = {
		descript = {
			{
				"Poema de Bragi"
			},
			{"%s"},
			{
				"Reduz a Pos-conj."
			},
			{
				"e Conj. Variável."
			}
		}
	},
	[EFST_IDs.EFST_APPLEIDUN] = {
		descript = {
			{
				"Maçãs de Idun"
			},
			{"%s"},
			{
				"Bônus de HP máx."
			},
			{
				"e Cura Recebida"
			}
		}
	},
	[EFST_IDs.EFST_HUMMING] = {
		descript = {
			{"Sibilo"},
			{"%s"},
			{
				"Bônus de Precisão"
			}
		}
	},
	[EFST_IDs.EFST_DONTFORGETME] = {
		descript = {
			{
				"Não me Abandones"
			},
			{"%s"},
			{
				"Movimento e"
			},
			{
				"Vel.Atq reduzidos."
			}
		}
	},
	[EFST_IDs.EFST_FORTUNEKISS] = {
		descript = {
			{
				"Beijo da Sorte"
			},
			{"%s"},
			{
				"Bônus de CRIT"
			},
			{
				"e dano crítico"
			}
		}
	},
	[EFST_IDs.EFST_SERVICEFORYOU] = {
		descript = {
			{
				"Dança Cigana"
			},
			{"%s"},
			{
				"Bônus de SP máx."
			},
			{
				"Menor custo de SP"
			}
		}
	},
	[EFST_IDs.EFST_RICHMANKIM] = {
		descript = {
			{
				"Banquete de Njord"
			},
			{"%s"},
			{
				"Bônus de EXP"
			},
			{
				"ao caçar monstros"
			}
		}
	},
	[EFST_IDs.EFST_ETERNALCHAOS] = {
		descript = {
			{
				"Ritmo Caótico"
			},
			{"%s"},
			{"DEF zero."}
		}
	},
	[EFST_IDs.EFST_DRUMBATTLEFIELD] = {
		descript = {
			{
				"Rufar dos Tambores"
			},
			{"%s"},
			{
				"Bônus de ATQ e DEF"
			}
		}
	},
	[EFST_IDs.EFST_RINGNIBELUNGEN] = {
		descript = {
			{
				"Anel dos Nibelungos"
			},
			{"%s"},
			{
				"Sob efeito de"
			},
			{
				"bônus aleatório"
			}
		}
	},
	[EFST_IDs.EFST_ROKISWEIL] = {
		descript = {
			{
				"Lamento de Loki"
			},
			{"%s"},
			{
				"Não pode usar"
			},
			{
				"habilidades."
			}
		}
	},
	[EFST_IDs.EFST_INTOABYSS] = {
		descript = {
			{
				"Canção Preciosa"
			},
			{"%s"},
			{
				"Anula o custo"
			},
			{"de gemas"}
		}
	},
	[EFST_IDs.EFST_SIEGFRIED] = {
		descript = {
			{
				"Ode a Siegfried"
			},
			{"%s"},
			{
				"Tolerância e"
			},
			{
				"Resistência"
			},
			{"aumentadas"}
		}
	},
	[EFST_IDs.EFST_STEELBODY] = {
		descript = {
			{
				"Corpo Fechado"
			},
			{"%s"},
			{
				"Reduz o dano recebido"
			},
			{
				"Impede o uso de habilidades"
			},
			{
				"Vel.Atq e movimento reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_EXTREMITYFIST] = {
		descript = {
			{
				"Punho Supremo de Asura"
			},
			{"%s"},
			{
				"Impede a regen. de SP"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYFIRE] = {
		descript = {
			{"Chama"},
			{"%s"},
			{
				"Arma de prop. Fogo"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYWATER] = {
		descript = {
			{"Geada"},
			{"%s"},
			{
				"Arma de prop. Água"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYWIND] = {
		descript = {
			{"Ventania"},
			{"%s"},
			{
				"Arma de prop. Vento"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYGROUND] = {
		descript = {
			{"Terremoto"},
			{"%s"},
			{
				"Arma de prop. Terra"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYUNDEAD] = {
		descript = {
			{
				"Zumbificação"
			},
			{"%s"},
			{
				"Armadura de prop. Maldito"
			}
		}
	},
	[EFST_IDs.EFST_AURABLADE] = {
		descript = {
			{
				"Lâmina de Aura"
			},
			{"%s"},
			{
				"Bônus de ATQ"
			}
		}
	},
	[EFST_IDs.EFST_PARRYING] = {
		descript = {
			{
				"Aparar Golpe"
			},
			{"%s"},
			{
				"Chance de bloquear ataque"
			}
		}
	},
	[EFST_IDs.EFST_LKCONCENTRATION] = {
		descript = {
			{"Dedicação"},
			{"%s"},
			{
				"Bônus de ATQ"
			},
			{
				"Reduz a DEF"
			}
		}
	},
	[EFST_IDs.EFST_TENSIONRELAX] = {
		descript = {
			{"Relaxar"},
			{"%s"},
			{"Acelera a"},
			{
				"regen. de HP"
			}
		}
	},
	[EFST_IDs.EFST_ASSUMPTIO] = {
		descript = {
			{"Assumptio"},
			{"%s"},
			{
				"Cura recebida e"
			},
			{
				"DEF aumentadas"
			}
		}
	},
	[EFST_IDs.EFST_GROUNDMAGIC] = {
		descript = {
			{
				"Terreno Encantado"
			},
			{"%s"},
			{
				"Bônus especiais"
			}
		}
	},
	[EFST_IDs.EFST_MAGICPOWER] = {
		descript = {
			{
				"Amplificação Mística"
			},
			{"%s"},
			{
				"Bônus de ATQM"
			}
		}
	},
	[EFST_IDs.EFST_EDP] = {
		descript = {
			{
				"Encantar com Veneno Mortal"
			},
			{"%s"},
			{
				"Dano físico aumentado"
			},
			{
				"Chance de infligir"
			},
			{
				"Envenenamento Mortal"
			}
		}
	},
	[EFST_IDs.EFST_TRUESIGHT] = {
		descript = {
			{"Visão Real"},
			{"%s"},
			{
				"Todos os atributos aumentados"
			},
			{
				"ATQ, CRIT e Precisão melhorados"
			}
		}
	},
	[EFST_IDs.EFST_WINDWALK] = {
		descript = {
			{
				"Caminho do Vento"
			},
			{"%s"},
			{
				"Esquiva  aumentada"
			},
			{
				"Movimento aumentado"
			}
		}
	},
	[EFST_IDs.EFST_MELTDOWN] = {
		descript = {
			{
				"Golpe Estilhaçante"
			},
			{"%s"},
			{
				"Chance de destruir a arma ou"
			},
			{
				"a armadura dos personagens."
			},
			{
				"Chance de reduzir o ATQ"
			},
			{
				"ou a DEF dos monstros"
			}
		}
	},
	[EFST_IDs.EFST_CARTBOOST] = {
		descript = {
			{
				"Impulso do Carrinho"
			},
			{"%s"},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_SWORDREJECT] = {
		descript = {
			{
				"Instinto de Defesa"
			},
			{
				"Chance de refletir ataques."
			},
			{
				"Reduz pela metade os ataques"
			},
			{
				"de monstros ou realizados"
			},
			{
				"com espadas e adagas"
			}
		}
	},
	[EFST_IDs.EFST_MARIONETTE_MASTER] = {
		descript = {
			{
				"Controle de Marionete"
			},
			{"%s"},
			{
				"Transfere atributos"
			},
			{
				"para o personagem"
			}
		}
	},
	[EFST_IDs.EFST_MARIONETTE] = {
		descript = {
			{
				"Controle de Marionete"
			},
			{"%s"},
			{
				"Recebendo atributos"
			},
			{
				"do controlador"
			}
		}
	},
	[EFST_IDs.EFST_BLOODING] = {
		descript = {
			{
				"Sangramento"
			},
			{"%s"},
			{
				"Drena HP a cada 10 seg."
			},
			{
				"Impede a regen."
			},
			{
				"natural de HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_JOINTBEAT] = {
		descript = {
			{
				"Ataque Vital"
			},
			{"%s"},
			{
				"Efeito anormal provocado"
			},
			{
				"pelo acerto de um ponto vital"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTEXP] = {
		descript = {
			{
				"Mãe, Pai, amo vocês!"
			},
			{"%s"},
			{
				"Anula a penalidade"
			},
			{
				"de EXP ao morrer"
			}
		}
	},
	[EFST_IDs.EFST_AUTOBERSERK] = {
		descript = {
			{
				"Instinto de Sobrevivência"
			},
			{
				"Ativa Provocar se o"
			},
			{
				"HP for menor que 25%"
			}
		}
	},
	[EFST_IDs.EFST_STORMKICK_ON] = {
		descript = {
			{
				"Postura do Tornado"
			},
			{
				"Chance de ativar a Postura"
			},
			{
				"ao realizar ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_DOWNKICK_ON] = {
		descript = {
			{
				"Postura da Patada Voadora"
			},
			{
				"Chance de ativar a Postura"
			},
			{
				"ao realizar ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_TURNKICK_ON] = {
		descript = {
			{
				"Postura da Rasteira"
			},
			{
				"Chance de ativar a Postura"
			},
			{
				"ao realizar ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_COUNTER_ON] = {
		descript = {
			{
				"Postura do Contrachute"
			},
			{
				"Chance de ativar a Postura"
			},
			{
				"ao realizar ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_DODGE_ON] = {
		descript = {
			{"Cambalhota"},
			{
				"Chance de bloquear danos"
			},
			{
				"mágicos ou físicos a distância."
			},
			{
				"Também bloqueia ataques"
			},
			{
				"físicos de curta distância"
			},
			{
				"se Cólera estiver ativada"
			}
		}
	},
	[EFST_IDs.EFST_STRUP] = {
		descript = {
			{"Cólera"},
			{"%s"},
			{
				"FOR aumentada"
			},
			{
				"Melhora o dano das"
			},
			{
				"habilidades de chute"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYDARK] = {
		descript = {
			{"%s"},
			{
				"Ataque de propriedade Sombrio"
			}
		}
	},
	[EFST_IDs.EFST_ADRENALINE2] = {
		descript = {
			{
				"Adrenalina Concentrada"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_PROPERTYTELEKINESIS] = {
		descript = {
			{"Brisa Leve"},
			{"%s"},
			{
				"Ataque de prop."
			},
			{"Fantasma"}
		}
	},
	[EFST_IDs.EFST_SOULLINK] = {
		descript = {
			{"Espírito"},
			{"%s"},
			{
				"Sob efeito de"
			},
			{
				"um Espírito."
			}
		}
	},
	[EFST_IDs.EFST_PLUSATTACKPOWER] = {
		descript = {
			{"%s"},
			{
				"ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_PLUSMAGICPOWER] = {
		descript = {
			{"%s"},
			{
				"ATQM aumentado"
			}
		}
	},
	[EFST_IDs.EFST_KAIZEL] = {
		descript = {
			{"Kaizel"},
			{"%s"},
			{
				"Revive ao morrer"
			},
			{
				"Kyrie Eleison por"
			},
			{
				"3 segundos."
			}
		}
	},
	[EFST_IDs.EFST_KAAHI] = {
		descript = {
			{"Kaahi"},
			{"%s"},
			{
				"Drena SP para recuperar HP"
			},
			{
				"quando sofrer danos físicos"
			}
		}
	},
	[EFST_IDs.EFST_KAUPE] = {
		descript = {
			{"Kaupe"},
			{"%s"},
			{
				"Chance de bloquear um ataque"
			}
		}
	},
	[EFST_IDs.EFST_ONEHANDQUICKEN] = {
		descript = {
			{
				"Rapidez com Uma Mão"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_SUN_COMFORT] = {
		descript = {
			{
				"Proteção Solar"
			},
			{"%s"},
			{
				"DEF melhorada"
			}
		}
	},
	[EFST_IDs.EFST_MOON_COMFORT] = {
		descript = {
			{
				"Proteção Lunar"
			},
			{"%s"},
			{
				"Esquiva melhorada"
			}
		}
	},
	[EFST_IDs.EFST_STAR_COMFORT] = {
		descript = {
			{
				"Proteção Estelar"
			},
			{"%s"},
			{
				"Vel.Atq melhorada"
			}
		}
	},
	[EFST_IDs.EFST_PRESERVE] = {
		descript = {
			{"Preservar"},
			{"%s"},
			{
				"Impede cópia de Plágio"
			}
		}
	},
	[EFST_IDs.EFST_CHASEWALK2] = {
		descript = {
			{"%s"},
			{
				"FOR aumentada"
			}
		}
	},
	[EFST_IDs.EFST_DOUBLECASTING] = {
		descript = {
			{
				"Lanças Duplas"
			},
			{"%s"},
			{
				"Chance de repetir Relâmpago,"
			},
			{
				"Lanças de Fogo e Lanças de Gelo"
			}
		}
	},
	[EFST_IDs.EFST_OVERTHRUSTMAX] = {
		descript = {
			{
				"Força Violentíssima"
			},
			{"%s"},
			{
				"Bônus de ATQ"
			}
		}
	},
	[EFST_IDs.EFST_TAROTCARD] = {
		descript = {
			{
				"Destino nas Cartas"
			},
			{"%s"},
			{
				"Efeito de uma Carta"
			}
		}
	},
	[EFST_IDs.EFST_CR_SHRINK] = {
		descript = {
			{"Submissão"},
			{"%s"},
			{
				"Ao bloquear com sucesso"
			},
			{
				"chance de Atordoamento"
			}
		}
	},
	[EFST_IDs.EFST_WZ_SIGHTBLASTER] = {
		descript = {
			{
				"Explosão Protetora"
			},
			{"%s"},
			{
				"Causa dano mágico e"
			},
			{
				"empurra oponentes"
			}
		}
	},
	[EFST_IDs.EFST_RG_CCONFINE_M] = {
		descript = {
			{
				"Confinamento"
			},
			{"%s"},
			{"Imóvel"},
			{
				"Esquiva +50."
			}
		}
	},
	[EFST_IDs.EFST_RG_CCONFINE_S] = {
		descript = {
			{
				"Confinamento"
			},
			{"%s"},
			{
				"Impossibilidade"
			},
			{
				"de se locomover"
			},
			{
				"temporariamente"
			}
		}
	},
	[EFST_IDs.EFST_GS_MADNESSCANCEL] = {
		descript = {
			{
				"Resistência Final"
			},
			{"%s"},
			{"Imóvel"},
			{
				"ATQ aumentado"
			},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_GS_GATLINGFEVER] = {
		descript = {
			{
				"Ataque Gatling"
			},
			{"%s"},
			{
				"ATQ aumentado"
			},
			{
				"Esquiva e movimento reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_EARTHSCROLL] = {
		descript = {
			{
				"Pausa Feliz"
			},
			{"%s"},
			{
				"Chance de não consumir"
			},
			{
				"um Pergaminho de"
			},
			{
				"Coluna de Pedra"
			}
		}
	},
	[EFST_IDs.EFST_NJ_UTSUSEMI] = {
		descript = {
			{
				"Troca de Pele"
			},
			{"%s"},
			{
				"Bloqueia ataque físico"
			},
			{
				"se afastando do alvo"
			}
		}
	},
	[EFST_IDs.EFST_NJ_BUNSINJYUTSU] = {
		descript = {
			{
				"Imagem Falsa"
			},
			{"%s"},
			{
				"Bloqueia alguns"
			},
			{
				"ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_NJ_NEN] = {
		descript = {
			{"Aura ninja"},
			{"%s"},
			{
				"FOR aumentada"
			},
			{
				"INT aumentada"
			}
		}
	},
	[EFST_IDs.EFST_GS_ADJUSTMENT] = {
		descript = {
			{
				"Pânico do Justiceiro"
			},
			{"%s"},
			{
				"Esquiva aumentada"
			},
			{
				"Precisão reduzida"
			},
			{
				"Resist. a danos"
			},
			{
				"físico a distância"
			}
		}
	},
	[EFST_IDs.EFST_GS_ACCURACY] = {
		descript = {
			{
				"Aumentar Precisão"
			},
			{"%s"},
			{
				"AGI aumentada"
			},
			{
				"DES aumentada"
			},
			{
				"Precisão aumentada"
			}
		}
	},
	[EFST_IDs.EFST_GRIFFON] = {
		descript = {
			{
				"Montar Grifo"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_STR] = {
		descript = {
			{"%s"},
			{
				"Bônus de FOR"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_AGI] = {
		descript = {
			{"%s"},
			{
				"Bônus de AGI"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_VIT] = {
		descript = {
			{"%s"},
			{
				"Bônus de VIT"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_DEX] = {
		descript = {
			{"%s"},
			{
				"Bônus de DES"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_INT] = {
		descript = {
			{"%s"},
			{
				"Bônus de INT"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_LUK] = {
		descript = {
			{"%s"},
			{
				"Bônus de SOR"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_BASICAVOIDANCE] = {
		descript = {
			{"%s"},
			{
				"Bônus de Esquiva"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_BASICHIT] = {
		descript = {
			{"%s"},
			{
				"Bônus de Precisão"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_CRITICALSUCCESSVALUE] = {
		descript = {
			{"%s"},
			{
				"Bônus de CRIT"
			}
		}
	},
	[EFST_IDs.EFST_CASH_PLUSEXP] = {
		descript = {
			{"%s"},
			{
				"Bônus de EXP"
			}
		}
	},
	[EFST_IDs.EFST_CASH_DEATHPENALTY] = {
		descript = {
			{"%s"},
			{
				"Anula a perda de"
			},
			{
				"EXP ao morrer"
			}
		}
	},
	[EFST_IDs.EFST_CASH_RECEIVEITEM] = {
		descript = {
			{"%s"},
			{
				"Aumenta a chance de um"
			},
			{
				"monstro derrubar itens"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_STR_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta FOR"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_AGI_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta AGI"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_VIT_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta VIT"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_DEX_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta DES"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_INT_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta INT"
			}
		}
	},
	[EFST_IDs.EFST_FOOD_LUK_CASH] = {
		descript = {
			{"%s"},
			{
				"Aumenta SOR"
			}
		}
	},
	[EFST_IDs.EFST_SLOWCAST] = {
		descript = {
			{
				"Conjuração Lenta"
			},
			{"%s"},
			{
				"Conj. variável"
			},
			{"Aumentada"}
		}
	},
	[EFST_IDs.EFST_CRITICALWOUND] = {
		descript = {
			{
				"Ferimento Crítico"
			},
			{"%s"},
			{
				"Reduz a Cura Recebida"
			}
		}
	},
	[EFST_IDs.EFST_MOVHASTE_HORSE] = {
		descript = {
			{"%s"},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_PROTECT_DEF] = {
		descript = {
			{
				"Poção de DEF"
			},
			{"%s"},
			{
				"Aumenta resist."
			},
			{
				"à danos físicos"
			}
		}
	},
	[EFST_IDs.EFST_PROTECT_MDEF] = {
		descript = {
			{
				"Poção de DEFM"
			},
			{"%s"},
			{
				"Aumenta resist."
			},
			{
				"à danos mágicos"
			}
		}
	},
	[EFST_IDs.EFST_HEALPLUS] = {
		descript = {
			{
				"Poção da Regeneração"
			},
			{"%s"},
			{
				"Aumenta a Cura Recebida"
			}
		}
	},
	[EFST_IDs.EFST_S_LIFEPOTION] = {
		descript = {
			{
				"Poção Menor de Vida"
			},
			{"%s"},
			{
				"Recupera HP continuamente"
			},
			{
				"Não funciona com Frenesi"
			}
		}
	},
	[EFST_IDs.EFST_L_LIFEPOTION] = {
		descript = {
			{
				"Poção Média de Vida"
			},
			{"%s"},
			{
				"Recupera HP continuamente"
			},
			{
				"Não funciona com Frenesi"
			}
		}
	},
	[EFST_IDs.EFST_CRITICALPERCENT] = {
		descript = {
			{"Abrasivo"},
			{"%s"},
			{
				"CRIT aumentado"
			}
		}
	},
	[EFST_IDs.EFST_PLUSAVOIDVALUE] = {
		descript = {
			{
				"Cálice da Ilusão"
			},
			{"%s"},
			{
				"Esquiva Perfeita"
			},
			{"aumentada"}
		}
	},
	[EFST_IDs.EFST_ATKER_ASPD] = {
		descript = {
			{
				"Pílula de HP"
			},
			{"%s"},
			{
				"HP máx. aumentado"
			},
			{
				"regen. natural de HP"
			},
			{"aumentada"}
		}
	},
	[EFST_IDs.EFST_TARGET_ASPD] = {
		descript = {
			{
				"Poção Mental"
			},
			{"%s"},
			{
				"SP máx. aumentado"
			},
			{
				"Custo de SP reduzido"
			}
		}
	},
	[EFST_IDs.EFST_ATKER_MOVESPEED] = {
		descript = {
			{
				"Pílula de SP"
			},
			{"%s"},
			{
				"SP máx. aumentado"
			},
			{
				"regen. natural de SP"
			},
			{"aumentada"}
		}
	},
	[EFST_IDs.EFST_ATKER_BLOOD] = {
		descript = {
			{
				"Suco de Gato"
			},
			{"%s"},
			{
				"Reduz o custo de SP"
			}
		}
	},
	[EFST_IDs.EFST_TARGET_BLOOD] = {
		descript = {
			{
				"Mega Resistência"
			},
			{"%s"},
			{
				"Aumenta a tolerância à"
			},
			{
				"Atordoamento, Congelamento,"
			},
			{
				"Petrificação, Sono, Silêncio,"
			},
			{
				"Cegueira, Maldição, Caos,"
			},
			{
				"Envenenamento e Sangramento"
			}
		}
	},
	[EFST_IDs.EFST_ARMOR_PROPERTY] = {
		descript = {
			{
				"Propriedade"
			},
			{"%s"},
			{
				"Armadura Elemental"
			}
		}
	},
	[EFST_IDs.EFST_HELLPOWER] = {
		descript = {
			{
				"Poder Infernal"
			},
			{"%s"},
			{
				"Impossível reviver com"
			},
			{
				"Ressuscitar, Martírio"
			},
			{
				"e Amuleto de Siegfried"
			}
		}
	},
	[EFST_IDs.EFST_CASH_PLUSONLYJOBEXP] = {
		descript = {
			{"%s"},
			{
				"Bônus de EXP de Classe"
			}
		}
	},
	[EFST_IDs.EFST_PARTYFLEE] = {
		descript = {
			{
				"Vento Uivante"
			},
			{"%s"},
			{
				"Esquiva aumentada"
			}
		}
	},
	[EFST_IDs.EFST_ANGEL_PROTECT] = {
		descript = {
			{
				"Muito obrigado!"
			},
			{"%s"},
			{
				"Acelera a regeneração de HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_ENCHANTBLADE] = {
		descript = {
			{
				"Encantar Lâmina"
			},
			{"%s"},
			{
				"Adiciona ATQM nos"
			},
			{
				"ataques básicos"
			}
		}
	},
	[EFST_IDs.EFST_REFRESH] = {
		descript = {
			{
				"Purificação"
			},
			{"%s"},
			{
				"Imunidade a"
			},
			{
				"efeitos negativos"
			}
		}
	},
	[EFST_IDs.EFST_GIANTGROWTH] = {
		descript = {
			{
				"Força Titânica"
			},
			{"%s"},
			{
				"Bônus de FOR"
			},
			{
				"Mais dano corpo a corpo"
			},
			{
				"Ao atacar, chance de"
			},
			{
				"causar um dano maior."
			}
		}
	},
	[EFST_IDs.EFST_STONEHARDSKIN] = {
		descript = {
			{
				"Escamas Rochosas"
			},
			{"%s"},
			{
				"DEF e DEFM aumentados"
			},
			{
				"Ao receber dano, chance"
			},
			{
				"de quebrar a arma do oponente"
			},
			{
				"ou de reduzir o ATQ dos monstros"
			}
		}
	},
	[EFST_IDs.EFST_VITALITYACTIVATION] = {
		descript = {
			{
				"Vitalidade Rúnica"
			},
			{"%s"},
			{
				"Maior cura recebida."
			},
			{
				"Reduz danos refletidos."
			}
		}
	},
	[EFST_IDs.EFST_FIGHTINGSPIRIT] = {
		descript = {
			{
				"Aura de Combate"
			},
			{"%s"},
			{
				"Bônus de ATQ"
			},
			{
				"e de Vel.Atq."
			}
		}
	},
	[EFST_IDs.EFST_ABUNDANCE] = {
		descript = {
			{
				"Regeneração Espiritual"
			},
			{"%s"},
			{
				"Recupera 60 de SP"
			},
			{
				"a cada 10 segundos"
			}
		}
	},
	[EFST_IDs.EFST_LAUDAAGNUS] = {
		descript = {
			{
				"Lauda Agnus"
			},
			{"%s"},
			{
				"Bônus de HP."
			}
		}
	},
	[EFST_IDs.EFST_LAUDARAMUS] = {
		descript = {
			{
				"Lauda Ramus"
			},
			{"%s"},
			{
				"Bônus de Dano Crítico."
			}
		}
	},
	[EFST_IDs.EFST_CLOAKINGEXCEED] = {
		descript = {
			{"Ocultação"},
			{
				"Personagem invisível."
			},
			{
				"Movimento acelerado."
			},
			{
				"Indetectável por Insetos e Demônios"
			}
		}
	},
	[EFST_IDs.EFST_HALLUCINATIONWALK] = {
		descript = {
			{
				"Passos da Ilusão"
			},
			{"%s"},
			{
				"Esquiva aumentada"
			},
			{
				"Chance de bloquear magia"
			}
		}
	},
	[EFST_IDs.EFST_RENOVATIO] = {
		descript = {
			{"Renovatio"},
			{"%s"},
			{
				"Regenera o HP máx."
			},
			{
				"a cada 5 segundos."
			}
		}
	},
	[EFST_IDs.EFST_WEAPONBLOCKING] = {
		descript = {
			{
				"Reflexo de Combate"
			},
			{"%s"},
			{
				"Chance de bloquear ataque físico"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_EXPIATIO] = {
		descript = {
			{"Expiatio"},
			{"%s"},
			{
				"Ignora DEF e DEFM"
			}
		}
	},
	[EFST_IDs.EFST_POISONINGWEAPON] = {
		descript = {
			{
				"Aplicar Toxina"
			},
			{"%s"},
			{
				"Bônus da Toxina"
			},
			{
				"Mais dano corpo a corpo"
			},
			{
				"Ao atacar, chance de"
			},
			{
				"aplicar a Toxina ativa."
			}
		}
	},
	[EFST_IDs.EFST_TOXIN] = {
		descript = {
			{"Cicuta"},
			{"%s"},
			{
				"Drena SP continuamente."
			},
			{
				"Cancela suas conjurações"
			},
			{
				"ignorando itens como a Carta Fen"
			}
		}
	},
	[EFST_IDs.EFST_PARALYSE] = {
		descript = {
			{"Curare"},
			{"%s"},
			{
				"Vel.Atq reduzida"
			},
			{
				"Esquiva reduzida"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_VENOMBLEED] = {
		descript = {
			{"Beladona"},
			{"%s"},
			{
				"Reduz o HP máx."
			}
		}
	},
	[EFST_IDs.EFST_MAGICMUSHROOM] = {
		descript = {
			{"Waraitake"},
			{"%s"},
			{
				"Drena HP continuamente"
			},
			{
				"Força a conjuração de"
			},
			{
				"habilidades aleatórias"
			}
		}
	},
	[EFST_IDs.EFST_DEATHHURT] = {
		descript = {
			{"Squamata"},
			{"%s"},
			{
				"Reduz a efetividade de cura"
			}
		}
	},
	[EFST_IDs.EFST_PYREXIA] = {
		descript = {
			{"Pirexia"},
			{"%s"},
			{"Cegueira."},
			{
				"Dano contínuo."
			}
		}
	},
	[EFST_IDs.EFST_OBLIVIONCURSE] = {
		descript = {
			{"Oblivium"},
			{"%s"},
			{
				"Impede o uso de habilidades"
			}
		}
	},
	[EFST_IDs.EFST_LEECHESEND] = {
		descript = {
			{"Vípera"},
			{"%s"},
			{
				"Drena HP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_FROSTMISTY] = {
		descript = {
			{"Hipotermia"},
			{"%s"},
			{
				"Conj. fixa aumentada"
			},
			{
				"DEF, Vel.Atq e de"
			},
			{
				"movimento reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_FEARBREEZE] = {
		descript = {
			{
				"Disparo Selvagem"
			},
			{"%s"},
			{
				"Chance de ativar"
			},
			{
				"disparos adicionais"
			}
		}
	},
	[EFST_IDs.EFST_MARSHOFABYSS] = {
		descript = {
			{
				"Pântano de Niflheim"
			},
			{"%s"},
			{
				"AGI, DES, DEF e DEFM reduzidas"
			},
			{
				"Movimento e esquiva reduzidas"
			}
		}
	},
	[EFST_IDs.EFST_RECOGNIZEDSPELL] = {
		descript = {
			{
				"Maestria Arcana"
			},
			{"%s"},
			{
				"Causa o máx. de dano mágico"
			},
			{
				"Custo de SP aumentado"
			}
		}
	},
	[EFST_IDs.EFST_STASIS] = {
		descript = {
			{
				"Distorção Arcana"
			},
			{"%s"},
			{
				"Não pode usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_WUGRIDER] = {
		descript = {
			{
				"Montaria em Worg"
			},
			{"%s"},
			{
				"Movimento acelerado"
			},
			{
				"Ataques apenas com"
			},
			{
				"habilidades de Worg"
			}
		}
	},
	[EFST_IDs.EFST_ACCELERATION] = {
		descript = {
			{"Aceleração"},
			{"%s"},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_HOVERING] = {
		descript = {
			{"Planar"},
			{"%s"},
			{
				"Evita armadilhas e"
			},
			{
				"magias de solo"
			}
		}
	},
	[EFST_IDs.EFST_MVPCARD_TAOGUNKA] = {
		descript = {
			{"Tao Gunka"},
			{"%s"},
			{
				"HP máx. aumentado"
			},
			{
				"DEF e DEFM reduzida"
			}
		}
	},
	[EFST_IDs.EFST_MVPCARD_MISTRESS] = {
		descript = {
			{
				"Abelha-Rainha"
			},
			{"%s"},
			{
				"Anula o consumo de gemas"
			},
			{
				"Custo de SP aumentado"
			}
		}
	},
	[EFST_IDs.EFST_MVPCARD_ORCHERO] = {
		descript = {
			{"Orc Herói"},
			{"%s"},
			{
				"Imune a Atordoamento"
			}
		}
	},
	[EFST_IDs.EFST_MVPCARD_ORCLORD] = {
		descript = {
			{
				"Senhor dos Orcs"
			},
			{"%s"},
			{
				"Reflete danos"
			},
			{
				"físicos recebidos"
			}
		}
	},
	[EFST_IDs.EFST_OVERHEAT] = {
		descript = {
			{
				"Superaquecimento"
			},
			{
				"Drena HP continuamente"
			},
			{
				"Resfrie seu M.E.C.H.A"
			}
		}
	},
	[EFST_IDs.EFST_SHAPESHIFT] = {
		descript = {
			{
				"Reconfigurar Elemento"
			},
			{"%s"},
			{
				"Armadura com Elemento"
			}
		}
	},
	[EFST_IDs.EFST_INFRAREDSCAN] = {
		descript = {
			{
				"Sensor Infravermelho"
			},
			{"%s"},
			{"Detectado."},
			{
				"Esquiva reduzida."
			}
		}
	},
	[EFST_IDs.EFST_MAGNETICFIELD] = {
		descript = {
			{
				"Campo Magnético"
			},
			{"%s"},
			{"Imóvel"},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_NEUTRALBARRIER] = {
		descript = {
			{
				"Campo Protetor"
			},
			{"%s"},
			{
				"DEF e DEFM aumentada"
			},
			{
				"Bloqueia danos físicos a distância"
			}
		}
	},
	[EFST_IDs.EFST_STEALTHFIELD] = {
		descript = {
			{
				"Campo de Invisibilidade"
			},
			{"%s"},
			{"Invisível"},
			{
				"Movimento reduzido"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_MANU_ATK] = {
		descript = {
			{
				"Oportunidade de Manuk"
			},
			{"%s"},
			{
				"Dano físico aumentado contra"
			},
			{
				"monstros do campo em Manuka"
			}
		}
	},
	[EFST_IDs.EFST_MANU_DEF] = {
		descript = {
			{
				"Coragem de Manuk"
			},
			{"%s"},
			{
				"Resistência aos monstros"
			},
			{
				"dos campos em Manuka"
			}
		}
	},
	[EFST_IDs.EFST_SPL_ATK] = {
		descript = {
			{
				"Geleia de Frutas de Pinguicula"
			},
			{"%s"},
			{
				"Dano físico contra monstros"
			},
			{
				"nos campos de Esplendor"
			}
		}
	},
	[EFST_IDs.EFST_SPL_DEF] = {
		descript = {
			{
				"Geleia de Mel de Vagalume"
			},
			{"%s"},
			{
				"Resistência aos monstros"
			},
			{
				"nos campos de Esplendor"
			}
		}
	},
	[EFST_IDs.EFST_REPRODUCE] = {
		descript = {
			{"Mimetismo"},
			{
				"Copia 1 habilidade"
			},
			{
				"da qual seja alvo"
			}
		}
	},
	[EFST_IDs.EFST_MANU_MATK] = {
		descript = {
			{
				"Fé de Manuk"
			},
			{"%s"},
			{
				"Dano mágico aumentado"
			},
			{
				"contra os monstros em Manuka"
			}
		}
	},
	[EFST_IDs.EFST_SPL_MATK] = {
		descript = {
			{
				"Lágrima de Cornus"
			},
			{"%s"},
			{
				"Dano físico aumentado"
			},
			{
				"contra monstros em Esplendor"
			}
		}
	},
	[EFST_IDs.EFST_STR_SCROLL] = {
		descript = {
			{"%s"},
			{
				"Aumenta FOR"
			}
		}
	},
	[EFST_IDs.EFST_INT_SCROLL] = {
		descript = {
			{"%s"},
			{
				"Aumenta INT"
			}
		}
	},
	[EFST_IDs.EFST_LG_REFLECTDAMAGE] = {
		descript = {
			{
				"Escudo Supressor"
			},
			{"%s"},
			{
				"Resist. a danos refletidos"
			}
		}
	},
	[EFST_IDs.EFST_FORCEOFVANGUARD] = {
		descript = {
			{
				"Proteção da Vanguarda"
			},
			{"%s"},
			{
				"DEF e HP máx. aumentados"
			},
			{
				"Drena SP continuamente"
			},
			{
				"Chance de receber Pontos de Fúria"
			}
		}
	},
	[EFST_IDs.EFST_AUTOSHADOWSPELL] = {
		descript = {
			{
				"Desejo das Sombras"
			},
			{"%s"},
			{
				"Aumenta ATQM"
			},
			{
				"Pode usar magias aprendidas"
			},
			{
				"usando Plágio e Mimetismo"
			}
		}
	},
	[EFST_IDs.EFST_SHADOWFORM] = {
		descript = {
			{
				"Vínculo Sombrio"
			},
			{"%s"},
			{
				"Direciona ataques para o alvo"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_SHIELDSPELL_DEF] = {
		descript = {
			{
				"Aegis Domini"
			},
			{"%s"},
			{
				"Magia ativada pela DEF do escudo."
			}
		}
	},
	[EFST_IDs.EFST_SHIELDSPELL_MDEF] = {
		descript = {
			{
				"Aegis Domini"
			},
			{"%s"},
			{
				"Magia ativada pela DEFM do escudo"
			}
		}
	},
	[EFST_IDs.EFST_SHIELDSPELL_REF] = {
		descript = {
			{
				"Aegis Domini"
			},
			{"%s"},
			{
				"Magia ativada pelo Refino do escudo"
			}
		}
	},
	[EFST_IDs.EFST_BODYPAINT] = {
		descript = {
			{
				"Borrifar Tinta"
			},
			{"%s"},
			{
				"Chance de Cegueira"
			},
			{
				"e de reduzir Vel.Atq"
			}
		}
	},
	[EFST_IDs.EFST_EXEEDBREAK] = {
		descript = {
			{
				"Exceder Limite"
			},
			{"%s"},
			{
				"Causa maior dano no"
			},
			{
				"próximo ataque físico"
			}
		}
	},
	[EFST_IDs.EFST_ADORAMUS] = {
		descript = {
			{"Adoração"},
			{"%s"},
			{
				"AGI e movimento reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_PRESTIGE] = {
		descript = {
			{
				"Prestígio Divino"
			},
			{"%s"},
			{
				"DEF aumentada"
			},
			{
				"Chance de bloquear ataques"
			},
			{
				"mágicos de alvo único"
			}
		}
	},
	[EFST_IDs.EFST_INVISIBILITY] = {
		descript = {
			{
				"Forma Etérea"
			},
			{"%s"},
			{"Invisível"},
			{
				"Drena SP continuamente"
			},
			{
				"Ataques de prop. Fantasma"
			},
			{
				"Não pode usar itens e habilidades"
			}
		}
	},
	[EFST_IDs.EFST_DEADLYINFECT] = {
		descript = {
			{
				"Pestilência"
			},
			{"%s"},
			{
				"Chance de infligir o"
			},
			{
				"efeito negativo atual"
			},
			{
				"ao atacar ou receber dano"
			}
		}
	},
	[EFST_IDs.EFST_BANDING] = {
		descript = {
			{
				"Formação Real"
			},
			{"%s"},
			{
				"Aumenta DEF"
			}
		}
	},
	[EFST_IDs.EFST_INSPIRATION] = {
		descript = {
			{
				"Consagração"
			},
			{"%s"},
			{
				"Aumenta Precisão, Atributos"
			},
			{
				"Aumenta ATQ, ATQM, HP"
			},
			{
				"Imune a efeitos negativos"
			},
			{
				"Consome HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_ENERVATION] = {
		descript = {
			{
				"Máscara da Fraqueza"
			},
			{"%s"},
			{
				"ATQ reduzido"
			},
			{
				"Esferas removidas"
			}
		}
	},
	[EFST_IDs.EFST_GROOMY] = {
		descript = {
			{
				"Máscara da Melancolia"
			},
			{"%s"},
			{
				"Precisão e Vel.Atq reduzidas"
			},
			{
				"Montaria, Ave, Homunculus"
			},
			{
				"e Mascotes retirados"
			}
		}
	},
	[EFST_IDs.EFST_RAISINGDRAGON] = {
		descript = {
			{
				"Dragão Ascendente"
			},
			{"%s"},
			{
				"Aumenta as Esferas"
			},
			{
				"Aumenta HP, SP, Vel.Atq"
			},
			{
				"Ativa Fúria Interior"
			}
		}
	},
	[EFST_IDs.EFST_IGNORANCE] = {
		descript = {
			{
				"Máscara da Tolice"
			},
			{"%s"},
			{
				"Dreno de SP"
			},
			{
				"Impede o uso de habilidades"
			}
		}
	},
	[EFST_IDs.EFST_LAZINESS] = {
		descript = {
			{
				"Máscara da Ociosidade"
			},
			{"%s"},
			{
				"Esquiva reduzida"
			},
			{
				"Custo de SP aumentado"
			},
			{
				"Conjuração variável aumentada"
			}
		}
	},
	[EFST_IDs.EFST_LIGHTNINGWALK] = {
		descript = {
			{
				"Salto Relâmpago"
			},
			{"%s"},
			{
				"Chance de anular danos"
			},
			{
				"físicos a distância"
			},
			{
				"se aproximando do alvo"
			}
		}
	},
	[EFST_IDs.EFST_ACARAJE] = {
		descript = {
			{"Acarajé"},
			{"%s"},
			{
				"Precisão aumentada"
			},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_UNLUCKY] = {
		descript = {
			{
				"Máscara do Infortúnio"
			},
			{"%s"},
			{
				"CRIT reduzido"
			},
			{
				"Esquiva Perfeita reduzida"
			},
			{
				"Recebe um efeito negativo"
			},
			{
				"Gasta zeny ao usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_CURSEDCIRCLE_TARGET] = {
		descript = {
			{
				"Campo Amaldiçoado"
			},
			{"%s"},
			{
				"Não pode andar,"
			},
			{
				"atacar e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_WEAKNESS] = {
		descript = {
			{
				"Máscara da Vulnerabilidade"
			},
			{"%s"},
			{
				"HP máx. reduzido."
			},
			{
				"Arma e Escudo removidos"
			},
			{
				"e não podem ser reequipados"
			}
		}
	},
	[EFST_IDs.EFST_CRESCENTELBOW] = {
		descript = {
			{
				"Cotovelada Ascendente"
			},
			{"%s"},
			{
				"Chance de refletir dano físico"
			},
			{
				"Empurra o atacante para trás"
			}
		}
	},
	[EFST_IDs.EFST_STRIPACCESSARY] = {
		descript = {
			{
				"Remover Acessório"
			},
			{"%s"},
			{
				"Acessório removido e"
			},
			{
				"não pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_POPECOOKIE] = {
		descript = {
			{
				"Biscoito da Papisa"
			},
			{"%s"},
			{
				"Dano físico e mágico aumentado"
			},
			{
				"Resist. a todas as propriedades"
			}
		}
	},
	[EFST_IDs.EFST_GENTLETOUCH_ENERGYGAIN] = {
		descript = {
			{
				"Chakra da Energia"
			},
			{"%s"},
			{
				"Ao atacar, chance de"
			},
			{
				"ganhar Esfera Espiritual."
			},
			{
				"Dano de Último Dragão,"
			},
			{
				"Combo Esmagador"
			},
			{
				"e Punho do Tigre +50%."
			}
		}
	},
	[EFST_IDs.EFST_GENTLETOUCH_CHANGE] = {
		descript = {
			{
				"Chakra da Fúria"
			},
			{"%s"},
			{
				"Bônus de ATQ e Vel.Atq"
			},
			{
				"Dano de Pancada Corporal"
			},
			{
				"e Explosão Espiritual"
			}
		}
	},
	[EFST_IDs.EFST_GENTLETOUCH_REVITALIZE] = {
		descript = {
			{
				"Chakra do Vigor"
			},
			{"%s"},
			{
				"Bônus de HP e DEF"
			},
			{
				"Dano de Garra de Tigre"
			},
			{
				"e Portões do Inferno"
			}
		}
	},
	[EFST_IDs.EFST_SWING] = {
		descript = {
			{
				"Ritmo Contagiante"
			},
			{"%s"},
			{
				"Aumenta Vel.Atq e"
			},
			{
				"movimento. Reduz"
			},
			{
				"a conjuração fixa"
			}
		}
	},
	[EFST_IDs.EFST_SYMPHONY_LOVE] = {
		descript = {
			{
				"Balada Sinfônica"
			},
			{"%s"},
			{
				"DEFM aumentada"
			}
		}
	},
	[EFST_IDs.EFST_DEEP_SLEEP] = {
		descript = {
			{
				"Sono Profundo"
			},
			{"%s"},
			{
				"Recebe 1,5x mais dano"
			},
			{
				"Impossível conversar"
			}
		}
	},
	[EFST_IDs.EFST_SIRCLEOFNATURE] = {
		descript = {
			{
				"Sibilo de Eir"
			},
			{"%s"},
			{
				"Acelera a regen."
			},
			{
				"natural de HP"
			}
		}
	},
	[EFST_IDs.EFST_COLD] = {
		descript = {
			{
				"Cristalização"
			},
			{"%s"},
			{
				"Drena HP e SP continuamente"
			},
			{
				"Não pode andar, atacar e usar itens"
			}
		}
	},
	[EFST_IDs.EFST_GLOOMYDAY] = {
		descript = {
			{"Ode a Hela"},
			{"%s"},
			{
				"Aumenta custo de SP"
			},
			{
				"e a Conjuração Fixa."
			},
			{
				"Reduz Esquiva e a"
			},
			{
				"Velocidade de ataque."
			}
		}
	},
	[EFST_IDs.EFST_SONG_OF_MANA] = {
		descript = {
			{
				"Canção de Alfheim"
			},
			{"%s"},
			{
				"Acelera a regen."
			},
			{
				"natural de SP"
			}
		}
	},
	[EFST_IDs.EFST_DANCE_WITH_WUG] = {
		descript = {
			{
				"Dança com Lobos"
			},
			{"%s"},
			{
				"Reduz Conj. fixa e"
			},
			{
				"aumenta o dano"
			},
			{
				"físico a distância"
			}
		}
	},
	[EFST_IDs.EFST_RUSH_WINDMILL] = {
		descript = {
			{
				"Sinfonia dos Ventos"
			},
			{"%s"},
			{
				"Aumenta o ATQ"
			},
			{
				"e o movimento"
			}
		}
	},
	[EFST_IDs.EFST_ECHOSONG] = {
		descript = {
			{
				"Canção de Balder"
			},
			{"%s"},
			{
				"DEF aumentada"
			}
		}
	},
	[EFST_IDs.EFST_HARMONIZE] = {
		descript = {
			{"Harmonizar"},
			{"%s"},
			{
				"Atributos ajustados"
			}
		}
	},
	[EFST_IDs.EFST_STRIKING] = {
		descript = {
			{
				"Encanto de Órion"
			},
			{"%s"},
			{
				"ATQ aumentado"
			},
			{
				"Bônus de Precisão perfeita"
			}
		}
	},
	[EFST_IDs.EFST_WARMER] = {
		descript = {
			{
				"Aquecer Terreno"
			},
			{"%s"},
			{
				"Impede congelamento"
			},
			{
				"Cancela efeitos de frio"
			},
			{
				"Recupera HP a cada 3 seg."
			}
		}
	},
	[EFST_IDs.EFST_MOONLIT_SERENADE] = {
		descript = {
			{
				"Serenata ao Luar"
			},
			{"%s"},
			{
				"ATQM aumentado"
			}
		}
	},
	[EFST_IDs.EFST_SATURDAY_NIGHT_FEVER] = {
		descript = {
			{"Loucura"},
			{"%s"},
			{
				"Não pode usar itens"
			},
			{
				"Reduz Esquiva e Precisão"
			},
			{
				"Drena 1% de HP e SP por seg."
			}
		}
	},
	[EFST_IDs.EFST_ANALYZE] = {
		descript = {
			{"Analisar"},
			{"%s"},
			{
				"DEF e DEFM reduzida"
			}
		}
	},
	[EFST_IDs.EFST_LERADS_DEW] = {
		descript = {
			{
				"Orvalho de Idun"
			},
			{"%s"},
			{
				"HP máx. aumentado"
			}
		}
	},
	[EFST_IDs.EFST_MELODYOFSINK] = {
		descript = {
			{
				"Cântico da Iluminação"
			},
			{"%s"},
			{
				"Reduz INT e drena"
			},
			{
				"SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_BEYOND_OF_WARCRY] = {
		descript = {
			{
				"Clamor de Batalha"
			},
			{"%s"},
			{
				"HP e FOR reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_UNLIMITED_HUMMING_VOICE] = {
		descript = {
			{
				"Murmúrio Perene"
			},
			{"%s"},
			{
				"Aumenta o dano mágico"
			},
			{
				"de todas as propriedades"
			}
		}
	},
	[EFST_IDs.EFST_GN_CARTBOOST] = {
		descript = {
			{
				"Propulsão do Carrinho"
			},
			{"%s"},
			{
				"Movimento acelerado"
			}
		}
	},
	[EFST_IDs.EFST_THORNS_TRAP] = {
		descript = {
			{
				"Armadilha de Espinhos"
			},
			{"%s"},
			{
				"Imobiliza e causa"
			},
			{
				"dano contínuo"
			}
		}
	},
	[EFST_IDs.EFST_BLOOD_SUCKER] = {
		descript = {
			{
				"Planta Sanguessuga"
			},
			{"%s"},
			{
				"Chance de converter"
			},
			{
				"HP por ataque físico"
			}
		}
	},
	[EFST_IDs.EFST_MANDRAGORA] = {
		descript = {
			{
				"Grito da Mandrágora"
			},
			{"%s"},
			{
				"INT reduzida"
			},
			{
				"Conj. fixa aumentada"
			}
		}
	},
	[EFST_IDs.EFST_AB_SECRAMENT] = {
		descript = {
			{
				"Sacramentum"
			},
			{"%s"},
			{
				"Conj. fixa reduzida"
			}
		}
	},
	[EFST_IDs.EFST_ASSUMPTIO2] = {
		descript = {
			{"Assumptio"},
			{"%s"},
			{
				"Cura recebida e"
			},
			{
				"DEF aumentadas"
			}
		}
	},
	[EFST_IDs.EFST_STOMACHACHE] = {
		descript = {
			{
				"Dor de Barriga"
			},
			{"%s"},
			{
				"Todos os atributos reduzidos"
			},
			{
				"Movimento reduzido"
			},
			{
				"Força a sentar a cada 10 segundos"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_PROMOTE_HEALTH_RESERCH] = {
		descript = {
			{
				"Poção de HP"
			},
			{"%s"},
			{
				"HP máx. aumentado"
			}
		}
	},
	[EFST_IDs.EFST_ENERGY_DRINK_RESERCH] = {
		descript = {
			{
				"Poção de SP"
			},
			{"%s"},
			{
				"SP máx. aumentado"
			}
		}
	},
	[EFST_IDs.EFST_EXTRACT_WHITE_POTION_Z] = {
		descript = {
			{
				"Poção Branca Especial"
			},
			{"%s"},
			{
				"Acelera a regen."
			},
			{
				"natural de HP"
			}
		}
	},
	[EFST_IDs.EFST_VITATA_500] = {
		descript = {
			{
				"Poção Vitata 500"
			},
			{"%s"},
			{
				"SP máx. aumentado"
			},
			{
				"Acelera a regen."
			},
			{
				"natural de SP"
			}
		}
	},
	[EFST_IDs.EFST_EXTRACT_SALAMINE_JUICE] = {
		descript = {
			{
				"Suco Celular Enriquecido"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_VACUUM_EXTREME] = {
		descript = {
			{"Tornado"},
			{"%s"},
			{"Imóvel"},
			{
				"Puxa alvos para a área"
			}
		}
	},
	[EFST_IDs.EFST_SAVAGE_STEAK] = {
		descript = {
			{
				"Churrasco de Selvagem"
			},
			{"%s"},
			{
				"Aumenta FOR"
			}
		}
	},
	[EFST_IDs.EFST_COCKTAIL_WARG_BLOOD] = {
		descript = {
			{
				"Coquetel Uivante"
			},
			{"%s"},
			{
				"Aumenta INT"
			}
		}
	},
	[EFST_IDs.EFST_MINOR_BBQ] = {
		descript = {
			{
				"Carne ao Vinho"
			},
			{"%s"},
			{
				"Aumenta VIT"
			}
		}
	},
	[EFST_IDs.EFST_SIROMA_ICE_TEA] = {
		descript = {
			{
				"Chá Gelado de Siroma"
			},
			{"%s"},
			{
				"Aumenta DES"
			}
		}
	},
	[EFST_IDs.EFST_DROCERA_HERB_STEAMED] = {
		descript = {
			{
				"Cozido de Drosera"
			},
			{"%s"},
			{
				"Aumenta AGI"
			}
		}
	},
	[EFST_IDs.EFST_PUTTI_TAILS_NOODLES] = {
		descript = {
			{
				"Macarrão com Petite"
			},
			{"%s"},
			{
				"Aumenta SOR"
			}
		}
	},
	[EFST_IDs.EFST_EL_WAIT] = {
		descript = {
			{
				"Elemental: Modo de espera"
			},
			{"%s"},
			{""}
		}
	},
	[EFST_IDs.EFST_EL_PASSIVE] = {
		descript = {
			{
				"Elemental: modo passivo"
			},
			{"%s"},
			{""}
		}
	},
	[EFST_IDs.EFST_EL_DEFENSIVE] = {
		descript = {
			{
				"Elemental: modo defensivo"
			},
			{"%s"},
			{""}
		}
	},
	[EFST_IDs.EFST_EL_OFFENSIVE] = {
		descript = {
			{
				"Elemental: modo ofensivo"
			},
			{"%s"},
			{""}
		}
	},
	[EFST_IDs.EFST_MORA_BUFF] = {
		descript = {
			{
				"Fruto de Mora"
			},
			{"%s"},
			{
				"Resistência ao monstros"
			},
			{
				"dos campos de Mora"
			}
		}
	},
	[EFST_IDs.EFST_NEEDLE_OF_PARALYZE] = {
		descript = {
			{"Paralisia"},
			{"%s"},
			{"Imóvel"},
			{
				"Reduz DEF e DEFM."
			}
		}
	},
	[EFST_IDs.EFST_PAIN_KILLER] = {
		descript = {
			{"Analgésico"},
			{"%s"},
			{
				"Reduz o dano recebido"
			},
			{
				"Evita reagir a ataques"
			}
		}
	},
	[EFST_IDs.EFST_G_LIFEPOTION] = {
		descript = {
			{
				"Poção Maior de Vida"
			},
			{"%s"},
			{
				"Recupera HP continuamente"
			},
			{
				"Não funciona com Frenesi"
			}
		}
	},
	[EFST_IDs.EFST_VITALIZE_POTION] = {
		descript = {
			{
				"Poção Energizante"
			},
			{"%s"},
			{
				"Bônus de dano físico e mágico"
			},
			{
				"Efetividade de cura aumentada"
			}
		}
	},
	[EFST_IDs.EFST_LIGHT_OF_REGENE] = {
		descript = {
			{
				"Luz da Vida"
			},
			{"%s"},
			{
				"Sacrifica a lealdade"
			},
			{
				"para ressuscitar"
			}
		}
	},
	[EFST_IDs.EFST_OVERED_BOOST] = {
		descript = {
			{
				"Superestimular"
			},
			{"%s"},
			{
				"Esquiva aumentada"
			},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_ODINS_POWER] = {
		descript = {
			{
				"Poder de Odin"
			},
			{"%s"},
			{
				"ATQ e ATQM aumentados"
			},
			{
				"DEF e DEFM reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_STYLE_CHANGE] = {
		descript = {
			{
				"Modo Lutador"
			},
			{
				"Modo de luta de Eleanor"
			}
		}
	},
	[EFST_IDs.EFST_MAGMA_FLOW] = {
		descript = {
			{
				"Fluxo de Magma"
			},
			{"%s"},
			{
				"Chance de espalhar magma"
			},
			{
				"quando receber danos"
			}
		}
	},
	[EFST_IDs.EFST_GRANITIC_ARMOR] = {
		descript = {
			{
				"Armadura de Granito"
			},
			{"%s"},
			{
				"Resistência a danos"
			},
			{
				"Drena HP após o término"
			}
		}
	},
	[EFST_IDs.EFST_PYROCLASTIC] = {
		descript = {
			{
				"Tremor de Fogo"
			},
			{"%s"},
			{
				"Aumenta ATQ da Arma"
			}
		}
	},
	[EFST_IDs.EFST_VOLCANIC_ASH] = {
		descript = {
			{
				"Cinzas Vulcânicas"
			},
			{"%s"},
			{
				"Precisão reduzida"
			},
			{
				"Chance da conj. falhar"
			},
			{
				"Reduz resist. a prop. Fogo"
			}
		}
	},
	[EFST_IDs.EFST_MAGIC_CANDY] = {
		descript = {
			{
				"Doce Mágico"
			},
			{"%s"},
			{
				"ATQM aumentado"
			},
			{
				"Conj. fixa reduzida"
			},
			{
				"Conj. interruptível"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_ALL_RIDING] = {
		descript = {
			{"Montaria"}
		}
	},
	[EFST_IDs.EFST_MACRO] = {
		descript = {
			{
				"Utilizando Macro"
			},
			{"%s"},
			{
				"Utilizando Macro"
			}
		}
	},
	[EFST_IDs.EFST_MACRO_POSTDELAY] = {
		descript = {
			{
				"Tempo de espera de uso de Macro"
			},
			{"%s"},
			{
				"Impossibilitado de usar Macro"
			}
		}
	},
	[EFST_IDs.EFST_OVERLAPEXPUP] = {
		descript = {
			{
				"Comida de Gato"
			},
			{"%s"},
			{
				"Bônus de EXP"
			},
			{
				"Bônus de DROP"
			}
		}
	},
	[EFST_IDs.EFST_CRUSHSTRIKE] = {
		descript = {
			{
				"Golpe Titânico"
			},
			{"%s"},
			{
				"Causa maior dano no"
			},
			{
				"próximo ataque físico"
			}
		}
	},
	[EFST_IDs.EFST_MONSTER_TRANSFORM] = {
		descript = {
			{"Disface"},
			{"%s"},
			{
				"Forma alterada"
			},
			{
				"para um monstro"
			}
		}
	},
	[EFST_IDs.EFST_SIT] = {
		descript = {
			{"Sentando"}
		}
	},
	[EFST_IDs.EFST_SKF_CAST] = {
		descript = {
			{"%s"},
			{"Conj. var."},
			{"reduzida"}
		}
	},
	[EFST_IDs.EFST_SKF_ASPD] = {
		descript = {
			{"%s"},
			{
				"Bônus de Vel.Atq"
			}
		}
	},
	[EFST_IDs.EFST_SKF_ATK] = {
		descript = {
			{"%s"},
			{
				"Bônus de ATQ"
			}
		}
	},
	[EFST_IDs.EFST_SKF_MATK] = {
		descript = {
			{"%s"},
			{
				"Bônus de ATQM"
			}
		}
	},
	[EFST_IDs.EFST_REWARD_PLUSONLYJOBEXP] = {
		descript = {
			{"%s"},
			{
				"Bônus de EXP de Classe"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_NORECOVER] = {
		descript = {
			{
				"Irrecuperável"
			},
			{"%s"},
			{
				"Não pode recuperar HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_SET_NUM_DEF] = {
		descript = {
			{"DEF zero"},
			{"%s"},
			{
				"DEF reduzida a 1"
			}
		}
	},
	[EFST_IDs.EFST_SET_NUM_MDEF] = {
		descript = {
			{"DEFM zero"},
			{"%s"},
			{
				"DEFM reduzida a 1"
			}
		}
	},
	[EFST_IDs.EFST_SET_PER_DEF] = {
		descript = {
			{"DEF zero"},
			{"%s"},
			{
				"DEF reduzida"
			}
		}
	},
	[EFST_IDs.EFST_SET_PER_MDEF] = {
		descript = {
			{"DEFM zero"},
			{"%s"},
			{
				"DEFM reduzida"
			}
		}
	},
	[EFST_IDs.EFST_MEIKYOUSISUI] = {
		descript = {
			{
				"Purificação da Alma"
			},
			{"%s"},
			{"Imóvel"},
			{
				"Recupera HP"
			},
			{
				"Recupera SP"
			},
			{
				"Chance de não receber dano"
			}
		}
	},
	[EFST_IDs.EFST_ATTHASTE_CASH] = {
		descript = {
			{
				"Poção de Ouro"
			},
			{"%s"},
			{
				"Vel.Atq aumentada"
			}
		}
	},
	[EFST_IDs.EFST_2011RWC] = {
		descript = {
			{
				"Fogo de Artifício"
			},
			{"%s"},
			{
				"Todos atributos +3"
			},
			{
				"ATQ, ATQM + 5%"
			}
		}
	},
	[EFST_IDs.EFST_IZAYOI] = {
		descript = {
			{"Inspiração"},
			{"%s"},
			{
				"ATQM aumentado"
			},
			{
				"Conj. fixa removida"
			},
			{
				"Conj. variável reduzida"
			},
			{
				"Drena SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_KG_KAGEHUMI] = {
		descript = {
			{
				"Esmagamento Sombrio"
			},
			{"%s"},
			{"Imóvel"},
			{
				"Invisibilidade cancelada"
			},
			{
				"Não pode usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_KYOMU] = {
		descript = {
			{
				"Vazio das Sombras"
			},
			{"%s"},
			{
				"Anula efeitos de"
			},
			{
				"reflexão de dano"
			}
		}
	},
	[EFST_IDs.EFST_KAGEMUSYA] = {
		descript = {
			{
				"Contrato das Sombras"
			},
			{"%s"},
			{
				"Ativa Ataque Duplo."
			},
			{
				"Drena SP por seg."
			},
			{
				"Bônus de dano em"
			},
			{
				"certas habilidades."
			}
		}
	},
	[EFST_IDs.EFST_ZANGETSU] = {
		descript = {
			{
				"Distorção Crescente"
			},
			{"%s"},
			{
				"ATQ e ATQM alterados"
			}
		}
	},
	[EFST_IDs.EFST_PHI_DEMON] = {
		descript = {
			{
				"Amuleto do Espírito"
			},
			{"%s"},
			{
				"Dano físico e mágico"
			},
			{
				"contra a raça Demônio"
			}
		}
	},
	[EFST_IDs.EFST_GENSOU] = {
		descript = {
			{
				"Ilusão do Luar"
			},
			{"%s"},
			{
				"HP e SP alterados"
			},
			{
				"Danos mágicos serão"
			},
			{
				"refletidos em área"
			}
		}
	},
	[EFST_IDs.EFST_AKAITSUKI] = {
		descript = {
			{
				"Luar Sinistro"
			},
			{"%s"},
			{
				"Recebe dano ao curar"
			},
			{
				"HP com habilidades"
			}
		}
	},
	[EFST_IDs.EFST_GM_BATTLE] = {
		descript = {
			{
				"Pílula de Combate"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM"
			},
			{
				"HP e SP máx. reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_GM_BATTLE2] = {
		descript = {
			{
				"Super Pílula"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM"
			},
			{
				"HP e SP máx. reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_2011RWC_SCROLL] = {
		descript = {
			{
				"Elixir Rubro"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM"
			},
			{
				"Vel.Atq aumentada"
			},
			{
				"Conj. variável reduzida"
			},
			{
				"Autoconjura 'Concentrar'"
			},
			{
				"ao realizar ataques físicos"
			}
		}
	},
	[EFST_IDs.EFST_MYSTICPOWDER] = {
		descript = {
			{"Pó Místico"},
			{"%s"},
			{
				"Esquiva e SOR"
			},
			{"aumentadas"}
		}
	},
	[EFST_IDs.EFST_M_LIFEPOTION] = {
		descript = {
			{
				"Água Milagrosa"
			},
			{"%s"},
			{
				"Recupera 4% de HP"
			},
			{
				"a cada 3 segundos."
			},
			{
				"Não funciona com Frenesi."
			}
		}
	},
	[EFST_IDs.EFST_FLOWER_LEAF] = {
		descript = {
			{
				"Cheiro de Flores"
			},
			{"%s"},
			{
				"Bônus de Esquiva"
			},
			{
				"Bônus de Esquiva Perfeita"
			}
		}
	},
	[EFST_IDs.EFST_RAY_OF_PROTECTION] = {
		descript = {
			{
				"Luz Protetora"
			},
			{"%s"},
			{
				"Imunidade a efeitos"
			},
			{
				"negativos diversos"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_ATK] = {
		descript = {
			{
				"Ataque Temporal"
			},
			{"%s"},
			{
				"Aumenta o dano físico e mágico"
			},
			{
				"contra os servos de Himelmez"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_DEF] = {
		descript = {
			{
				"Defesa Temporal"
			},
			{"%s"},
			{
				"Aumenta a resistência contra"
			},
			{
				"os servos de Himelmez"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_HEAL] = {
		descript = {
			{
				"Cura Temporal"
			},
			{"%s"},
			{
				"Aumenta a efetividade de cura"
			},
			{
				"e a Cura Recebida"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_HIDDEN] = {
		descript = {
			{
				"Escudo Temporal"
			},
			{"%s"},
			{
				"Aumenta a resistência contra"
			},
			{
				"os escudos de Amdarais"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_STATE] = {
		descript = {
			{
				"Estado Temporal"
			},
			{"%s"},
			{
				"Todos os atributos +20"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_ITEMDEF] = {
		descript = {
			{
				"Poder Temporal"
			},
			{"%s"},
			{
				"Aumenta a DEF e DEFM"
			}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_HPSP] = {
		descript = {
			{
				"Energia Temporal"
			},
			{"%s"},
			{
				"Aumenta o HP e SP"
			}
		}
	},
	[EFST_IDs.EFST_ALMIGHTY] = {
		descript = {
			{
				"Bolinho Divino"
			},
			{"%s"},
			{
				"Atributos, ATQ e"
			},
			{
				"ATQM aumentados"
			}
		}
	},
	[EFST_IDs.EFST_GVG_GIANT] = {
		descript = {
			{
				"Sangue de Gigante"
			},
			{"%s"},
			{
				"Aumenta o dano físico e"
			},
			{
				"mágico contra personagens"
			}
		}
	},
	[EFST_IDs.EFST_GVG_GOLEM] = {
		descript = {
			{
				"Núcleo Golem"
			},
			{"%s"},
			{
				"Aumenta a resist."
			},
			{
				"a personagens"
			}
		}
	},
	[EFST_IDs.EFST_GVG_STUN] = {
		descript = {
			{
				"Fada da Estrela"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{
				"a Atordoamento"
			}
		}
	},
	[EFST_IDs.EFST_GVG_STONE] = {
		descript = {
			{
				"Fada da Pedra"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{
				"a Petrificação"
			}
		}
	},
	[EFST_IDs.EFST_GVG_FREEZ] = {
		descript = {
			{
				"Fada do Gelo"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{
				"a Congelamento"
			}
		}
	},
	[EFST_IDs.EFST_GVG_SLEEP] = {
		descript = {
			{
				"Fada do Sonho"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{"a Sono"}
		}
	},
	[EFST_IDs.EFST_GVG_CURSE] = {
		descript = {
			{
				"Fada da Sorte"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{"a Maldição"}
		}
	},
	[EFST_IDs.EFST_GVG_SILENCE] = {
		descript = {
			{
				"Fada da Música"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{"a Silêncio"}
		}
	},
	[EFST_IDs.EFST_GVG_BLIND] = {
		descript = {
			{
				"Fada da Luz"
			},
			{"%s"},
			{
				"Garante imunidade"
			},
			{"a Cegueira"}
		}
	},
	[EFST_IDs.EFST_GEFFEN_MAGIC1] = {
		descript = {
			{
				"Bênção da Lydia"
			},
			{"%s"},
			{
				"Aumenta o dano físico"
			},
			{
				"contra Humanoides"
			}
		}
	},
	[EFST_IDs.EFST_GEFFEN_MAGIC2] = {
		descript = {
			{
				"Bênção da Fenrir"
			},
			{"%s"},
			{
				"Aumenta o dano mágico"
			},
			{
				"contra Humanoides"
			}
		}
	},
	[EFST_IDs.EFST_GEFFEN_MAGIC3] = {
		descript = {
			{
				"Bênção de Chaos"
			},
			{"%s"},
			{
				"Aumenta a resistência"
			},
			{
				"contra Humanoides"
			}
		}
	},
	[EFST_IDs.EFST_QUEST_BUFF1] = {
		descript = {
			{
				"Bênção de Reno"
			},
			{"%s"},
			{
				"ATQ e ATQM aumentados"
			}
		}
	},
	[EFST_IDs.EFST_QUEST_BUFF2] = {
		descript = {
			{
				"Bênção de Reno"
			},
			{"%s"},
			{
				"ATQ e ATQM aumentados"
			}
		}
	},
	[EFST_IDs.EFST_QUEST_BUFF3] = {
		descript = {
			{
				"Bênção de Reno"
			},
			{"%s"},
			{
				"ATQ e ATQM aumentados"
			}
		}
	},
	[EFST_IDs.EFST_FENRIR_CARD] = {
		descript = {
			{
				"Poder da Fenrir"
			},
			{
				"ATQM aumentado"
			},
			{
				"Conjuração fixa reduzida"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_FRIGG_SONG] = {
		descript = {
			{
				"Canção de Frigga"
			},
			{
				"HP máx. aumentado"
			},
			{
				"Recupera HP continuamente"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_OFFERTORIUM] = {
		descript = {
			{
				"Offertorium"
			},
			{
				"Custo de SP aumentado"
			},
			{
				"Aumenta a efet. de cura"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_TELEKINESIS_INTENSE] = {
		descript = {
			{
				"Telecinesia"
			},
			{
				"Custo de SP reduzido"
			},
			{
				"Dano fantasma aumentado"
			},
			{
				"Conjuração variável reduzida"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_FULL_THROTTLE] = {
		descript = {
			{
				"Maximizar Poder"
			},
			{
				"Movimento acelerado"
			},
			{
				"Atributos aumentados"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_REBOUND] = {
		descript = {
			{"Fadiga"},
			{
				"Movimento reduzido"
			},
			{
				"Desabilita a regen."
			},
			{
				"natural de HP e SP"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_UNLIMIT] = {
		descript = {
			{"Ilimitar"},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"físico a distância"
			}
		}
	},
	[EFST_IDs.EFST_KINGS_GRACE] = {
		descript = {
			{"Graça Real"},
			{"Imóvel"},
			{
				"Imune a ataques"
			},
			{
				"Imune a efeitos negativos"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_DARKCROW] = {
		descript = {
			{
				"Garra Sombria"
			},
			{
				"Desativa os efeitos"
			},
			{"%s"},
			{
				"de reflexão física"
			},
			{
				"Mais dano corpo a corpo"
			}
		}
	},
	[EFST_IDs.EFST_CHILL] = {
		descript = {
			{"Frio"},
			{
				"Imune a Queimadura"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_BURNT] = {
		descript = {
			{"Queimadura"},
			{
				"Resist. a propriedade Fogo"
			},
			{
				"Resist. a alvos de Fogo"
			},
			{
				"Recebe dano físico de"
			},
			{
				"propriedade fogo"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_B_TRAP] = {
		descript = {
			{
				"Armadilha Aderente"
			},
			{"%s"},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_E_CHAIN] = {
		descript = {
			{
				"Reação Ilimitada"
			},
			{"%s"},
			{
				"Chance de ativar"
			},
			{
				"Reação em Cadeia"
			}
		}
	},
	[EFST_IDs.EFST_C_MARKER] = {
		descript = {
			{"Marcação"},
			{"%s"},
			{
				"Esquiva -10"
			}
		}
	},
	[EFST_IDs.EFST_H_MINE] = {
		descript = {
			{"Granada"},
			{"%s"},
			{
				"Artefato explosivo"
			}
		}
	},
	[EFST_IDs.EFST_P_ALTER] = {
		descript = {
			{
				"Proteção Platina"
			},
			{"%s"},
			{
				"ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_HEAT_BARREL] = {
		descript = {
			{"Furor"},
			{"%s"},
			{
				"Aumenta ATQ"
			},
			{"e Vel.Atq."},
			{
				"Precisão e Conj."
			},
			{
				"fixa reduzidas"
			}
		}
	},
	[EFST_IDs.EFST_ANTI_M_BLAST] = {
		descript = {
			{
				"Anti-matéria"
			},
			{"%s"},
			{
				"Reduz resist. a"
			},
			{
				"raça Humanoide"
			}
		}
	},
	[EFST_IDs.EFST_SWORDCLAN] = {
		descript = {
			{
				"Espada Escarlate"
			},
			{
				"FOR +1, VIT +1"
			},
			{
				"HP +30, SP +10"
			}
		}
	},
	[EFST_IDs.EFST_ARCWANDCLAN] = {
		descript = {
			{
				"Cajado Celeste"
			},
			{
				"INT +1, DES +1"
			},
			{
				"HP +30, SP +10"
			}
		}
	},
	[EFST_IDs.EFST_GOLDENMACECLAN] = {
		descript = {
			{
				"Maça Dourada"
			},
			{
				"SOR +1, INT +1"
			},
			{
				"HP +30, SP +10"
			}
		}
	},
	[EFST_IDs.EFST_CROSSBOWCLAN] = {
		descript = {
			{
				"Balestra Esmeralda"
			},
			{
				"DES +1, AGI +1"
			},
			{
				"HP +30, SP +10"
			}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE1] = {
		descript = {
			{
				"Carta para KDY"
			},
			{"ATQ +33"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE2] = {
		descript = {
			{
				"Carta para KJH"
			},
			{"ATQM +33"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE3] = {
		descript = {
			{
				"Carta para LMH"
			},
			{
				"HP máx. +3%"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE4] = {
		descript = {
			{
				"Carta para LMS"
			},
			{
				"SP máx. +3%"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE5] = {
		descript = {
			{
				"Carta para CHJ"
			},
			{
				"Esquiva +33"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE6] = {
		descript = {
			{
				"Carta para KJK"
			},
			{"Vel.Atq +3"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE7] = {
		descript = {
			{
				"Carta para MISA"
			},
			{"DEF +33"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE8] = {
		descript = {
			{
				"Carta para YAOB"
			},
			{"DEFM +33"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE9] = {
		descript = {
			{
				"Carta para LEONA"
			},
			{"CRIT +33"},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_PACKING_ENVELOPE10] = {
		descript = {
			{
				"Carta para BIBIC"
			},
			{
				"Precisão +33"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_GLASTHEIM_TRANS] = {
		descript = {
			{
				"Morto-Vivo!"
			},
			{"%s"},
			{
				"SP máx. -50%."
			}
		}
	},
	[EFST_IDs.EFST_HEAT_BARREL_AFTER] = {
		descript = {
			{
				"Efeito negativo"
			},
			{"%s"},
			{
				"Não pode usar item"
			},
			{
				"Não pode usar habilidade"
			},
			{
				"Não pode atacar"
			}
		}
	},
	[EFST_IDs.EFST_BEEF_RIB_STEW] = {
		descript = {
			{
				"Costelinha Grelhada"
			},
			{"%s"},
			{
				"Conj. var. -5%."
			},
			{
				"Custo de SP -3%."
			}
		}
	},
	[EFST_IDs.EFST_PORK_RIB_STEW] = {
		descript = {
			{
				"Costela Refogada"
			},
			{"%s"},
			{
				"Vel.Atq +5%."
			},
			{
				"Custo de SP -2%."
			}
		}
	},
	[EFST_IDs.EFST_CHUSEOK_TUESDAY] = {
		descript = {
			{
				"Força Elemental"
			},
			{"%s"},
			{
				"Dano contra alvos de Terra."
			},
			{
				"Resist. à Fogo aumentada."
			}
		}
	},
	[EFST_IDs.EFST_CHUSEOK_WEEKEND] = {
		descript = {
			{
				"Força Elemental"
			},
			{"%s"},
			{
				"Dano contra alvos de Neutro."
			},
			{
				"Resist. à Neutro aumentada."
			}
		}
	},
	[EFST_IDs.EFST_LIMIT_POWER_BOOSTER] = {
		descript = {
			{
				"Poção Ilimitada"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM,"
			},
			{
				"Precisão, Esquiva, Vel.Atq."
			},
			{
				"Custo de SP reduzido."
			},
			{
				"Conjuração fixa menor."
			}
		}
	},
	[EFST_IDs.EFST_EP16_DEF] = {
		descript = {
			{
				"Poção Sagrada"
			},
			{
				"Resistência a imortais +10%"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_STONECURSE] = {
		descript = {
			{
				"Petrificação"
			},
			{"%s"},
			{
				"Drena HP continuamente"
			},
			{
				"Não pode andar, atacar"
			},
			{
				"e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_FREEZING] = {
		descript = {
			{
				"Congelamento"
			},
			{"%s"},
			{
				"DEF e DEFM reduzidas"
			},
			{
				"Não pode andar, atacar"
			},
			{
				"e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_STUN] = {
		descript = {
			{
				"Atordoamento"
			},
			{"%s"},
			{
				"Não pode andar, atacar"
			},
			{
				"e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_SLEEP] = {
		descript = {
			{"Sono"},
			{"%s"},
			{"Imóvel"},
			{
				"Não pode andar, atacar"
			},
			{
				"e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_STONECURSE_ING] = {
		descript = {
			{
				"Petrificação"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_BURNNING] = {
		descript = {
			{"Incêndio"},
			{"%s"},
			{
				"DEFM reduzida"
			},
			{
				"Recebe dano contínuo"
			}
		}
	},
	[EFST_IDs.EFST_BODYSTATE_IMPRISON] = {
		descript = {
			{"Exílio"},
			{"%s"},
			{
				"Não pode andar, atacar"
			},
			{
				"e usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_POISON] = {
		descript = {
			{
				"Envenenamento"
			},
			{"%s"},
			{"Visão ruim"},
			{
				"DEF reduzida"
			},
			{
				"Drena HP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_CURSE] = {
		descript = {
			{"Maldição"},
			{"%s"},
			{
				"SOR reduzida"
			},
			{
				"Movimento reduzido."
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_SILENCE] = {
		descript = {
			{"Silêncio"},
			{"%s"},
			{
				"Não pode usar habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_CONFUSION] = {
		descript = {
			{"Caos"},
			{"%s"},
			{
				"Anda em direções aleatórias"
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_HEAVYPOISON] = {
		descript = {
			{
				"Envenenamento Mortal"
			},
			{"%s"},
			{
				"Drena HP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HEALTHSTATE_FEAR] = {
		descript = {
			{"Medo"},
			{"%s"},
			{"Imóvel"},
			{
				"Esquiva e Precisão reduzidas"
			}
		}
	},
	[EFST_IDs.EFST_SU_STOOP] = {
		descript = {
			{
				"Quatro Patas"
			},
			{"%s"},
			{
				"Reduz o dano recebido."
			}
		}
	},
	[EFST_IDs.EFST_CATNIPPOWDER] = {
		descript = {
			{"Pulverizar"},
			{"%s"},
			{
				"Desacelera."
			},
			{
				"Reduz ATQ e ATQM."
			},
			{
				"Regen. de HP e SP maior."
			}
		}
	},
	[EFST_IDs.EFST_SV_ROOTTWIST] = {
		descript = {
			{
				"Hera Venenosa"
			},
			{"%s"},
			{"Imóvel."},
			{
				"Recebe 100 de dano"
			},
			{
				"fixo a cada segundo."
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_NOTHING] = {
		descript = {
			{"Neutro"},
			{"%s"},
			{
				"Arma de prop. Neutro"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_WATER] = {
		descript = {
			{"Água"},
			{"%s"},
			{
				"Arma de prop. Água"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_GROUND] = {
		descript = {
			{"Terra"},
			{"%s"},
			{
				"Arma de prop. Terra"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_FIRE] = {
		descript = {
			{"Fogo"},
			{"%s"},
			{
				"Arma de prop. Fogo"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_WIND] = {
		descript = {
			{"Vento"},
			{"%s"},
			{
				"Arma de prop. Vento"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_POISON] = {
		descript = {
			{"Veneno"},
			{"%s"},
			{
				"Arma de prop. Veneno"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_SAINT] = {
		descript = {
			{"Sagrado"},
			{"%s"},
			{
				"Arma de prop. Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_DARKNESS] = {
		descript = {
			{"Sombrio"},
			{"%s"},
			{
				"Arma de prop. Sombrio"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_TELEKINESIS] = {
		descript = {
			{"Fantasma"},
			{"%s"},
			{
				"Arma de prop. Fantasma"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_PROPERTY_UNDEAD] = {
		descript = {
			{"Maldito"},
			{"%s"},
			{
				"Arma de prop. Maldito"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_NOTHING] = {
		descript = {
			{"Neutro"},
			{"%s"},
			{
				"Mais resist. a Neutro"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_WATER] = {
		descript = {
			{"Água"},
			{"%s"},
			{
				"Mais resist. a Água"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_GROUND] = {
		descript = {
			{"Terra"},
			{"%s"},
			{
				"Mais resist. a Terra"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_FIRE] = {
		descript = {
			{"Fogo"},
			{"%s"},
			{
				"Mais resist. a Fogo"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_WIND] = {
		descript = {
			{"Vento"},
			{"%s"},
			{
				"Mais resist. a Vento"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_POISON] = {
		descript = {
			{"Veneno"},
			{"%s"},
			{
				"Mais resist. a Veneno"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_SAINT] = {
		descript = {
			{"Sagrado"},
			{"%s"},
			{
				"Mais resist. a Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_DARKNESS] = {
		descript = {
			{"Sombrio"},
			{"%s"},
			{
				"Mais resist. a Sombrio"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_TELEKINESIS] = {
		descript = {
			{"Fantasma"},
			{"%s"},
			{
				"Mais resist. a Fantasma"
			}
		}
	},
	[EFST_IDs.EFST_RESIST_PROPERTY_UNDEAD] = {
		descript = {
			{"Maldito"},
			{"%s"},
			{
				"Mais resist. a Maldito"
			}
		}
	},
	[EFST_IDs.EFST_BITESCAR] = {
		descript = {
			{
				"Leptospirose"
			},
			{"%s"},
			{
				"Drena o HP a"
			},
			{
				"cada segundo."
			}
		}
	},
	[EFST_IDs.EFST_ARCLOUSEDASH] = {
		descript = {
			{
				"Impulso de Arclouse"
			},
			{"%s"},
			{"Acelera."},
			{
				"Bônus de AGI."
			}
		}
	},
	[EFST_IDs.EFST_TUNAPARTY] = {
		descript = {
			{
				"Proteção da Orla"
			},
			{"%s"},
			{
				"Bloqueia ataques."
			}
		}
	},
	[EFST_IDs.EFST_SHRIMP] = {
		descript = {
			{
				"Chuva de Mariscos"
			},
			{"%s"},
			{
				"Bônus de ATQ e ATQM."
			}
		}
	},
	[EFST_IDs.EFST_FRESHSHRIMP] = {
		descript = {
			{
				"Camarão Fresquinho"
			},
			{"%s"},
			{
				"Recupera HP"
			},
			{
				"continuamente."
			}
		}
	},
	[EFST_IDs.EFST_PERIOD_RECEIVEITEM] = {
		descript = {
			{"%s"},
			{
				"Bônus de DROP"
			}
		}
	},
	[EFST_IDs.EFST_PERIOD_PLUSJOBEXP] = {
		descript = {
			{"%s"},
			{
				"Bônus de EXP"
			},
			{"de Classe"}
		}
	},
	[EFST_IDs.EFST_HELM_VERKANA] = {
		descript = {
			{
				"Runa Berkana"
			},
			{"%s"},
			{
				"Reduz ATQ e ATQM"
			},
			{
				"Chance de autoconjurar"
			},
			{
				"'Escudos Milenares'"
			},
			{
				"ao receber dano físico"
			}
		}
	},
	[EFST_IDs.EFST_HELM_RHYDO] = {
		descript = {
			{"Runa Raido"},
			{"%s"},
			{
				"Não pode ser empurrado"
			}
		}
	},
	[EFST_IDs.EFST_HELM_TURISUS] = {
		descript = {
			{
				"Runa Turizas"
			},
			{"%s"},
			{
				"FOR aumentada"
			},
			{
				"INT reduzida"
			}
		}
	},
	[EFST_IDs.EFST_HELM_HAGALAS] = {
		descript = {
			{
				"Runa Hagalaz"
			},
			{"%s"},
			{
				"Reflete parte do"
			},
			{
				"dano físico recebido"
			}
		}
	},
	[EFST_IDs.EFST_HELM_ISIA] = {
		descript = {
			{"Runa Isa"},
			{"%s"},
			{
				"Recupera HP"
			},
			{
				"continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HELM_ASIR] = {
		descript = {
			{
				"Runa Othila"
			},
			{"%s"},
			{
				"ATQ e Vel.Atq"
			},
			{"aumentados"}
		}
	},
	[EFST_IDs.EFST_HELM_URJ] = {
		descript = {
			{"Runa Uruz"},
			{"%s"},
			{
				"Recupera SP"
			},
			{
				"continuamente"
			}
		}
	},
	[EFST_IDs.EFST_SUHIDE] = {
		descript = {
			{"Gato-Mia"},
			{"%s"},
			{
				"Ninguém te vê!"
			}
		}
	},
	[EFST_IDs.EFST_DORAM_BUF_01] = {
		descript = {
			{
				"Leite de Kaluna"
			},
			{"%s"},
			{
				"Recupera 10 de HP"
			},
			{
				"a cada 10 segundos"
			}
		}
	},
	[EFST_IDs.EFST_DORAM_BUF_02] = {
		descript = {
			{"Manjericão"},
			{"%s"},
			{
				"Recupera 5 de SP"
			},
			{
				"a cada 10 segundos"
			}
		}
	},
	[EFST_IDs.EFST_SPRITEMABLE] = {
		descript = {
			{"Invocacão"}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_RECEIVEITEM] = {
		descript = {
			{
				"Bônus de DROP"
			},
			{"%s"},
			{
				"Mais chances de"
			},
			{
				"derrubar itens"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_PLUSEXP] = {
		descript = {
			{
				"Bônus de EXP"
			},
			{"%s"},
			{
				"Mais ganho de EXP"
			},
			{
				"de Base e Classe"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_PLUSJOBEXP] = {
		descript = {
			{
				"EXP de Classe"
			},
			{"%s"},
			{
				"Mais ganho de"
			},
			{
				"EXP de Classe"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_DEADPENALTY] = {
		descript = {
			{
				"Pen. de Morte"
			},
			{"%s"},
			{
				"Reduz a perda de"
			},
			{
				"EXP quando morrer."
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_ADDSTOREITEMCOUNT] = {
		descript = {
			{"Armazém"},
			{"%s"},
			{
				"Mais espaços de"
			},
			{
				"itens no Armazém."
			}
		}
	},
	[EFST_IDs.EFST_HISS] = {
		descript = {
			{
				"Riscar Fósforo"
			},
			{"%s"},
			{
				"Esquiva perfeita +50."
			},
			{
				"Movimento acelerado."
			}
		}
	},
	[EFST_IDs.EFST_NYANGGRASS] = {
		descript = {
			{
				"Mato de Gato"
			},
			{"%s"},
			{
				"Ignora DEF e DEFM"
			}
		}
	},
	[EFST_IDs.EFST_CHATTERING] = {
		descript = {
			{"Despertar"},
			{"%s"},
			{
				"ATQ e ATQM +100."
			},
			{
				"Acelera o movimento"
			},
			{
				"por 10 segundos."
			}
		}
	},
	[EFST_IDs.EFST_GROOMING] = {
		descript = {
			{"Lambida"},
			{"%s"},
			{
				"Esquiva +100."
			}
		}
	},
	[EFST_IDs.EFST_PROTECTIONOFSHRIMP] = {
		descript = {
			{
				"Festa do Camarão"
			},
			{"%s"},
			{"Aumenta a"},
			{
				"regen. de SP."
			}
		}
	},
	[EFST_IDs.EFST_EP16_2_BUFF_SS] = {
		descript = {
			{"Poção SS"},
			{"%s"},
			{
				"Vel.Atq +10."
			}
		}
	},
	[EFST_IDs.EFST_EP16_2_BUFF_SC] = {
		descript = {
			{"Poção SC"},
			{"%s"},
			{"CRIT +30"}
		}
	},
	[EFST_IDs.EFST_EP16_2_BUFF_AC] = {
		descript = {
			{"Poção AC"},
			{"%s"},
			{
				"Conj. var. -80%"
			}
		}
	},
	[EFST_IDs.EFST_GS_MAGICAL_BULLET] = {
		descript = {
			{
				"Bala Mágica"
			},
			{"%s"},
			{
				"Adiciona ATQM"
			},
			{
				"no ataque básico"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_RECEIVEITEM_2ND] = {
		descript = {
			{
				"Bônus de DROP II"
			},
			{"%s"},
			{
				"Mais chances de"
			},
			{
				"derrubar itens"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_PLUSEXP_2ND] = {
		descript = {
			{
				"Bônus de EXP II"
			},
			{"%s"},
			{
				"Mais ganho de EXP"
			},
			{
				"de Base e Classe"
			}
		}
	},
	[EFST_IDs.EFST_AID_PERIOD_PLUSJOBEXP_2ND] = {
		descript = {
			{
				"EXP de Classe II"
			},
			{"%s"},
			{
				"Mais ganho de"
			},
			{
				"EXP de Classe"
			}
		}
	},
	[EFST_IDs.EFST_KIEL_CARD] = {
		descript = {
			{
				"Poção Fantástica"
			},
			{"%s"},
			{
				"Pós-conjuração -15%"
			}
		}
	},
	[EFST_IDs.EFST_CHEERUP] = {
		descript = {
			{"Animação!"},
			{"%s"},
			{
				"Atributos +3."
			}
		}
	},
	[EFST_IDs.EFST_S_MANAPOTION] = {
		descript = {
			{
				"Poção Menor de Mana"
			},
			{"%s"},
			{
				"Recupera SP a cada 5 seg."
			},
			{
				"Não funciona com Frenesi."
			}
		}
	},
	[EFST_IDs.EFST_M_DEFSCROLL] = {
		descript = {
			{"Escrituras"},
			{"%s"},
			{
				"Bônus de DEF"
			},
			{
				"Bônus de DEFM"
			}
		}
	},
	[EFST_IDs.EFST_LHZ_DUN_N1] = {
		descript = {
			{"RT-017"},
			{"%s"},
			{
				"Aumenta o dano contra Seyren,"
			},
			{
				"Eremes, Randel e Gertie"
			},
			{
				"Resistência a Flamel,"
			},
			{
				"Chen, Kathryne e Howard."
			}
		}
	},
	[EFST_IDs.EFST_LHZ_DUN_N2] = {
		descript = {
			{"RT-017"},
			{"%s"},
			{
				"Aumenta o dano contra Celia,"
			},
			{
				"Flamel, Kathryne e Howard."
			},
			{
				"Resistência a Margaretha,"
			},
			{
				"Chen, Cecil, Alphoccio e Trentini"
			}
		}
	},
	[EFST_IDs.EFST_LHZ_DUN_N3] = {
		descript = {
			{"RT-017"},
			{"%s"},
			{
				"Aumenta o dano contra Margaretha,"
			},
			{
				"Chen, Cecil, Alphoccio e Trentini"
			},
			{
				"Resistência a Seyren,"
			},
			{
				"Randel, Eremes e Gertie."
			}
		}
	},
	[EFST_IDs.EFST_LHZ_DUN_N4] = {
		descript = {
			{"RT-017"},
			{"%s"},
			{
				"Aumenta o dano e a resistência"
			},
			{
				"aos MVPs da Tumba da Honra."
			}
		}
	},
	[EFST_IDs.EFST_ALL_STAT_DOWN] = {
		descript = {
			{
				"Perda Total"
			},
			{"%s"},
			{
				"Todos atributos"
			},
			{
				"foram reduzidos"
			}
		}
	},
	[EFST_IDs.EFST_GRADUAL_GRAVITY] = {
		descript = {
			{
				"Gravidade Gradual"
			},
			{"%s"},
			{
				"Drena HP por segundo"
			}
		}
	},
	[EFST_IDs.EFST_SPECIALCOOKIE] = {
		descript = {
			{
				"Biscoito Especial"
			},
			{"%s"},
			{
				"Aumenta Vel.Atq"
			},
			{
				"Reduz Conj. Var."
			}
		}
	},
	[EFST_IDs.EFST_ATK_POPCORN] = {
		descript = {
			{"Saboroso!"},
			{"%s"},
			{
				"ATQ da arma +10%."
			}
		}
	},
	[EFST_IDs.EFST_MATK_POPCORN] = {
		descript = {
			{"Delicioso!"},
			{"%s"},
			{
				"Dano mágico +10%."
			}
		}
	},
	[EFST_IDs.EFST_ASPD_POPCORN] = {
		descript = {
			{"Gostoso!"},
			{"%s"},
			{
				"Vel.atq. +10%."
			}
		}
	},
	[EFST_IDs.EFST_LIGHTOFMOON] = {
		descript = {
			{"Luz Lunar"},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Chute Lunar"
			}
		}
	},
	[EFST_IDs.EFST_LIGHTOFSUN] = {
		descript = {
			{"Luz Solar"},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Explosão Solar"
			}
		}
	},
	[EFST_IDs.EFST_LIGHTOFSTAR] = {
		descript = {
			{
				"Luz Estelar"
			},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Chuva Estelar"
			}
		}
	},
	[EFST_IDs.EFST_LUNARSTANCE] = {
		descript = {
			{
				"Postura Lunar"
			},
			{"%s"},
			{
				"Bônus de HP"
			},
			{
				"Poderes Lunares"
			}
		}
	},
	[EFST_IDs.EFST_UNIVERSESTANCE] = {
		descript = {
			{
				"Postura do Universo"
			},
			{"%s"},
			{
				"Bônus de atributos"
			},
			{
				"Poderes Universais"
			}
		}
	},
	[EFST_IDs.EFST_SUNSTANCE] = {
		descript = {
			{
				"Postura Solar"
			},
			{"%s"},
			{
				"Bônus de ATQ"
			},
			{
				"Poderes Solares"
			}
		}
	},
	[EFST_IDs.EFST_FLASHKICK] = {
		descript = {
			{
				"Marca da Estrela"
			}
		}
	},
	[EFST_IDs.EFST_NEWMOON] = {
		descript = {
			{"Penumbra"},
			{"%s"},
			{"Invisível"},
			{"Pode usar"},
			{
				"Chute Lunar"
			}
		}
	},
	[EFST_IDs.EFST_STARSTANCE] = {
		descript = {
			{
				"Postura Estelar"
			},
			{"%s"},
			{"Vel.Atq"},
			{
				"Poderes Estelares"
			}
		}
	},
	[EFST_IDs.EFST_DIMENSION] = {
		descript = {
			{
				"Livro das Dimensões"
			},
			{"%s"},
			{
				"Efeitos especiais"
			},
			{
				"em Hipernova e"
			},
			{
				"Advento Imperial"
			}
		}
	},
	[EFST_IDs.EFST_CREATINGSTAR] = {
		descript = {
			{
				"Livro da Criação"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_FALLINGSTAR] = {
		descript = {
			{
				"Chuva Estelar"
			},
			{"%s"},
			{
				"Ao realizar ataques,"
			},
			{
				"chance de autoconjurar"
			},
			{
				"uma Estrela Cadente"
			}
		}
	},
	[EFST_IDs.EFST_GRAVITYCONTROL] = {
		descript = {
			{
				"Controle Gravitacional"
			},
			{
				"Impossível atacar"
			},
			{
				"ou se mover"
			}
		}
	},
	[EFST_IDs.EFST_SOULCOLLECT] = {
		descript = {
			{
				"Coletar Alma"
			},
			{"%s"},
			{
				"Atraindo Almas"
			}
		}
	},
	[EFST_IDs.EFST_SOULREAPER] = {
		descript = {
			{
				"Ceifar Alma"
			},
			{"%s"},
			{
				"Ao realizar ataques,"
			},
			{
				"chance de conseguir"
			},
			{
				"ceifar uma Alma"
			}
		}
	},
	[EFST_IDs.EFST_SOULUNITY] = {
		descript = {
			{
				"União Espiritual"
			},
			{"%s"},
			{
				"A cada 3 segundos,"
			},
			{
				"regeneração de HP"
			}
		}
	},
	[EFST_IDs.EFST_SOULSHADOW] = {
		descript = {
			{
				"Espírito das Sombras"
			},
			{"%s"},
			{
				"Bônus de CRIT"
			},
			{
				"e de Vel.Atq"
			}
		}
	},
	[EFST_IDs.EFST_SOULFAIRY] = {
		descript = {
			{
				"Espírito da Fada"
			},
			{"%s"},
			{
				"Bônus de ATQM"
			},
			{
				"Conj. var. reduzida"
			}
		}
	},
	[EFST_IDs.EFST_SOULFALCON] = {
		descript = {
			{
				"Espírito do Falcão"
			},
			{"%s"},
			{
				"Bônus de ATQ"
			},
			{
				"e de Precisão"
			}
		}
	},
	[EFST_IDs.EFST_SOULGOLEM] = {
		descript = {
			{
				"Espírito do Golem"
			},
			{"%s"},
			{"Aumenta a"},
			{"DEF e DEFM"}
		}
	},
	[EFST_IDs.EFST_SOULDIVISION] = {
		descript = {
			{
				"Divisão de Alma"
			},
			{
				"Pós-conjuração"
			},
			{
				"foi aumentada"
			}
		}
	},
	[EFST_IDs.EFST_INFINITY_DRINK] = {
		descript = {
			{
				"Poção Infinita"
			},
			{"%s"},
			{
				"Bônus de HP e SP."
			},
			{
				"Dano crítico e distância."
			},
			{
				"Dano mágico elemental."
			},
			{
				"Conjuração ininterruptível."
			}
		}
	},
	[EFST_IDs.EFST_HUNTING_EVENT] = {
		descript = {
			{
				"Caçada Real"
			},
			{"%s"},
			{
				"Aumenta a regeneração"
			},
			{
				"da Carne e Batata Doce"
			}
		}
	},
	[EFST_IDs.EFST_PERIOD_RECEIVEITEM_2ND] = {
		descript = {
			{
				"Manual de Mascar"
			},
			{"%s"},
			{
				"DROP aumentado"
			}
		}
	},
	[EFST_IDs.EFST_PERIOD_PLUSEXP_2ND] = {
		descript = {
			{
				"Manual de Mascar"
			},
			{"%s"},
			{
				"EXP aumentada"
			}
		}
	},
	[EFST_IDs.EFST_ENSEMBLEFATIGUE] = {
		descript = {
			{"Cansaço"},
			{"%s"},
			{
				"Não pode usar"
			},
			{
				"habilidades"
			},
			{
				"Vel.Atq -30%."
			}
		}
	},
	[EFST_IDs.EFST_ADAPTATION] = {
		descript = {
			{
				"Aquecimento"
			},
			{"%s"},
			{
				"Custo de SP"
			},
			{
				"reduzido em 20%"
			}
		}
	},
	[EFST_IDs.EFST_ANCILLA] = {
		descript = {
			{
				"Criar Ancilla"
			},
			{"%s"},
			{
				"Efetv. de cura +15%."
			},
			{
				"Regen. de SP +30%."
			},
			{
				"Adoramus Neutro"
			}
		}
	},
	[EFST_IDs.EFST_WEAPONBLOCK_ON] = {
		descript = {
			{"Reflexo"},
			{"%s"},
			{
				"Permite usar"
			},
			{"Retaliação"}
		}
	},
	[EFST_IDs.EFST_ASSUMPTIO_BUFF] = {
		descript = {
			{"Assumptio"},
			{"%s"},
			{
				"Cura recebida e"
			},
			{
				"DEF aumentadas"
			}
		}
	},
	[EFST_IDs.EFST_BASILICA_BUFF] = {
		descript = {
			{"Basílica"},
			{"%s"},
			{
				"Dano mágico Sagrado"
			},
			{
				"Dano físico contra"
			},
			{
				"Sombrios e Malditos"
			}
		}
	},
	[EFST_IDs.EFST_OVERLAPEXPUP2] = {
		descript = {
			{
				"Comida de Gato II"
			},
			{"%s"},
			{
				"Bônus de EXP"
			},
			{
				"Bônus de DROP"
			}
		}
	},
	[EFST_IDs.EFST_SOULCURSE] = {
		descript = {
			{
				"Assombração"
			},
			{"%s"},
			{
				"Recebe mais dano de"
			},
			{
				"propriedade Sombrio"
			}
		}
	},
	[EFST_IDs.EFST_SOUND_OF_DESTRUCTION] = {
		descript = {
			{
				"Prelúdio do Ragnarök"
			},
			{"%s"},
			{
				"Recebe 2x mais dano"
			}
		}
	},
	[EFST_IDs.EFST_HELPANGEL] = {
		descript = {
			{
				"Sob Proteção"
			},
			{"%s"},
			{
				"Cura 1.000 de HP e"
			},
			{
				"350 de SP por seg."
			}
		}
	},
	[EFST_IDs.EFST_SWEETSFAIR_ATK] = {
		descript = {
			{
				"Festividade"
			},
			{"%s"},
			{
				"ATQ da arma +7%"
			}
		}
	},
	[EFST_IDs.EFST_SWEETSFAIR_MATK] = {
		descript = {
			{"Solenidade"},
			{"%s"},
			{
				"Dano mágico +7%"
			}
		}
	},
	[EFST_IDs.EFST_FLOWER_LEAF2] = {
		descript = {
			{
				"Comemoração"
			},
			{"%s"},
			{
				"Resist. a Fogo"
			},
			{
				"Aumenta a Vel.Atq"
			},
			{
				"Conj. Var. reduzida"
			}
		}
	},
	[EFST_IDs.EFST_FLOWER_LEAF3] = {
		descript = {
			{
				"Poder Físico"
			},
			{"%s"},
			{
				"Dano físico +5%."
			}
		}
	},
	[EFST_IDs.EFST_FLOWER_LEAF4] = {
		descript = {
			{
				"Poder Mágico"
			},
			{"%s"},
			{
				"Dano mágico +5%."
			}
		}
	},
	[EFST_IDs.EFST_MISTY_FROST] = {
		descript = {
			{"Calafrio"},
			{"%s"},
			{
				"Recebe mais dano"
			},
			{
				"de Esquife de Gelo"
			}
		}
	},
	[EFST_IDs.EFST_MAGIC_POISON] = {
		descript = {
			{
				"Vício Mágico"
			},
			{"%s"},
			{
				"Recebe 50% mais dano"
			},
			{
				"de todas as propriedades"
			}
		}
	},
	[EFST_IDs.EFST_JPNONLY_TACTICS] = {
		descript = {
			{
				"Influência da Fauna"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Chilique de Picy, Ferida de Tarou"
			},
			{
				"Cometas Lunáticos e Ataque Selvagem"
			}
		}
	},
	[EFST_IDs.EFST_DEADLY_DEFEASANCE] = {
		descript = {
			{
				"Negação Mágica"
			},
			{"%s"},
			{
				"Cancela o efeito de"
			},
			{
				"Imunidade Mágica"
			}
		}
	},
	[EFST_IDs.EFST_CLIMAX_DES_HU] = {
		descript = {
			{"Zéfiro"},
			{"%s"},
			{"ATQM + 100"},
			{
				"Dano mágico de"
			},
			{
				"Vento aumentado"
			}
		}
	},
	[EFST_IDs.EFST_CLIMAX] = {
		descript = {
			{
				"Intensificação"
			},
			{"%s"},
			{
				"Intensifica Florescer"
			},
			{
				"Tufão Destrutivo"
			},
			{
				"Pilares de Pedra e"
			},
			{
				"Cacos de Gelo"
			}
		}
	},
	[EFST_IDs.EFST_LUXANIMA] = {
		descript = {
			{
				"Luz da Alma"
			},
			{"%s"},
			{
				"Bônus de dano crítico,"
			},
			{
				"tamanho, distância e"
			},
			{
				"corpo a corpo. Ao atacar,"
			},
			{
				"chance de autoconjurar"
			},
			{
				"Explosão Rúnica nv. 1."
			}
		}
	},
	[EFST_IDs.EFST_BATH_FOAM_A] = {
		descript = {
			{
				"Sais de Banho A"
			},
			{"%s"},
			{
				"Dano contra monstros"
			},
			{
				"da Sauna Meditatio"
			},
			{
				"aumentado em 5%."
			}
		}
	},
	[EFST_IDs.EFST_BATH_FOAM_B] = {
		descript = {
			{
				"Sais de Banho B"
			},
			{"%s"},
			{
				"Dano contra monstros"
			},
			{
				"da Sauna Meditatio"
			},
			{
				"aumentado em 10%."
			}
		}
	},
	[EFST_IDs.EFST_BATH_FOAM_C] = {
		descript = {
			{
				"Sais de Banho C"
			},
			{"%s"},
			{
				"Dano contra monstros"
			},
			{
				"da Sauna Meditatio"
			},
			{
				"aumentado em 15%."
			}
		}
	},
	[EFST_IDs.EFST_AROMA_OIL] = {
		descript = {
			{
				"Aromatizante"
			},
			{"%s"},
			{
				"Resist. aos monstros"
			},
			{
				"da Sauna Meditatio"
			},
			{
				"aumentada em 10%."
			}
		}
	},
	[EFST_IDs.EFST_POWERFUL_FAITH] = {
		descript = {
			{
				"Mantra da Força"
			},
			{"%s"},
			{
				"ATQ aumentado"
			},
			{
				"P.ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_SINCERE_FAITH] = {
		descript = {
			{
				"Mantra da Energia"
			},
			{"%s"},
			{
				"Aumenta a Vel.Atq"
			},
			{
				"e Precisão perfeita"
			}
		}
	},
	[EFST_IDs.EFST_FIRM_FAITH] = {
		descript = {
			{
				"Mantra da Saúde"
			},
			{"%s"},
			{
				"HP aumentado"
			},
			{
				"TEN aumentado"
			}
		}
	},
	[EFST_IDs.EFST_HELLS_PLANT_ARMOR] = {
		descript = {
			{
				"Planta Infernal"
			},
			{"%s"},
			{
				"Causa dano corpo a"
			},
			{
				"corpo ao seu redor"
			}
		}
	},
	[EFST_IDs.EFST_LOCKON_LASER] = {
		descript = {
			{
				"Laser Rastreador"
			},
			{
				"Alvo de bombas"
			}
		}
	},
	[EFST_IDs.EFST_REF_T_POTION] = {
		descript = {
			{"Poção X"},
			{"%s"},
			{
				"Tolerância a"
			},
			{
				"reflexão +100%."
			}
		}
	},
	[EFST_IDs.EFST_ADD_ATK_DAMAGE] = {
		descript = {
			{
				"Elixir Vermelho"
			},
			{"%s"},
			{
				"Dano físico a"
			},
			{
				"distância e"
			},
			{
				"corpo a corpo +15%."
			}
		}
	},
	[EFST_IDs.EFST_ADD_MATK_DAMAGE] = {
		descript = {
			{
				"Elixir Azul"
			},
			{"%s"},
			{
				"Dano mágico"
			},
			{
				"de todas as"
			},
			{
				"propriedades +15%."
			}
		}
	},
	[EFST_IDs.EFST_SERVANTWEAPON] = {
		descript = {
			{
				"Espada Alada"
			},
			{"%s"},
			{
				"Ao realizar ataques"
			},
			{
				"básicos, chance de"
			},
			{
				"1 espada cair no alvo"
			}
		}
	},
	[EFST_IDs.EFST_SERVANT_SIGN] = {
		descript = {
			{
				"Marca Alada"
			},
			{"%s"},
			{
				"Marca causada"
			},
			{
				"por Espada Alada"
			}
		}
	},
	[EFST_IDs.EFST_CHARGINGPIERCE] = {
		descript = {
			{
				"Acumular Poder"
			},
			{"%s"},
			{
				"Acumula 1 ponto"
			},
			{
				"até 10 pontos por 5s."
			}
		}
	},
	[EFST_IDs.EFST_DRAGONIC_AURA] = {
		descript = {
			{
				"Aura Draconiana"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Lança das Mil Pontas,"
			},
			{
				"Sopro e Bafo do Dragão"
			}
		}
	},
	[EFST_IDs.EFST_VIGOR] = {
		descript = {
			{
				"Determinação"
			},
			{"%s"},
			{
				"Dano físico aumentado"
			},
			{
				"A cada ataque, HP reduzido"
			}
		}
	},
	[EFST_IDs.EFST_CLIMAX_EARTH] = {
		descript = {
			{
				"Empalamento"
			},
			{"%s"},
			{
				"Resist. à prop."
			},
			{
				"Terra -100%."
			}
		}
	},
	[EFST_IDs.EFST_CLIMAX_BLOOM] = {
		descript = {
			{"Pólen"},
			{"%s"},
			{
				"Resist. à prop."
			},
			{
				"Fogo -100%."
			}
		}
	},
	[EFST_IDs.EFST_CLIMAX_CRYIMP] = {
		descript = {
			{"Geleira"},
			{"%s"},
			{
				"DEF +300, DEFM +100"
			},
			{
				"Resist. prop. Água +30%."
			},
			{
				"Dano mágico Água +30%."
			}
		}
	},
	[EFST_IDs.EFST_HOLY_OIL] = {
		descript = {
			{
				"Óleo Ungido"
			},
			{"%s"},
			{
				"Recebe mais dano de"
			},
			{
				"ataques à distância."
			}
		}
	},
	[EFST_IDs.EFST_SHADOW_EXCEED] = {
		descript = {
			{
				"Carrasco Sombrio"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Talho Eterno e"
			},
			{
				"Impacto Brutal"
			}
		}
	},
	[EFST_IDs.EFST_DANCING_KNIFE] = {
		descript = {
			{
				"Adagas Voadoras"
			},
			{"%s"},
			{
				"Causa dano físico"
			},
			{
				"ao seu redor."
			}
		}
	},
	[EFST_IDs.EFST_POTENT_VENOM] = {
		descript = {
			{
				"Adulterar Veneno"
			},
			{"%s"},
			{"Ignora TEN"}
		}
	},
	[EFST_IDs.EFST_SHADOW_SCAR] = {
		descript = {
			{"Profanação"},
			{"%s"},
			{
				"Recebe mais dano"
			},
			{
				"físico corpo a corpo"
			}
		}
	},
	[EFST_IDs.EFST_MEDIALE] = {
		descript = {
			{
				"Mediale Votum"
			},
			{"%s"},
			{
				"Regenera HP a"
			},
			{
				"cada 2 segundos"
			}
		}
	},
	[EFST_IDs.EFST_A_VITA] = {
		descript = {
			{
				"Argutus Vita"
			},
			{"%s"},
			{
				"Ignora TENM"
			}
		}
	},
	[EFST_IDs.EFST_A_TELUM] = {
		descript = {
			{
				"Argutus Telum"
			},
			{"%s"},
			{"Ignora TEN"}
		}
	},
	[EFST_IDs.EFST_PRE_ACIES] = {
		descript = {
			{
				"Presens Acies"
			},
			{"%s"},
			{
				"Bônus de T.CRIT"
			}
		}
	},
	[EFST_IDs.EFST_COMPETENTIA] = {
		descript = {
			{
				"Competentia"
			},
			{"%s"},
			{
				"Bônus de P.ATQ"
			},
			{
				"Bônus de S.ATQM"
			}
		}
	},
	[EFST_IDs.EFST_GUARD_STANCE] = {
		descript = {
			{
				"Posição de Defesa"
			},
			{"%s"},
			{
				"DEF aumentada"
			},
			{
				"ATQ reduzido"
			}
		}
	},
	[EFST_IDs.EFST_ATTACK_STANCE] = {
		descript = {
			{
				"Posição de Ataque"
			},
			{"%s"},
			{
				"DEF reduzida"
			},
			{
				"P.ATQ aumentado"
			},
			{
				"S.ATQM aumentado"
			}
		}
	},
	[EFST_IDs.EFST_GUARDIAN_S] = {
		descript = {
			{
				"Escudo Guardião"
			},
			{"%s"},
			{
				"Absorve danos físicos"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_DEEPBLIND] = {
		descript = {
			{"Escuridão"},
			{"%s"},
			{
				"Visão reduzida"
			},
			{
				"Esquiva reduzida"
			},
			{
				"Esquiva perfeita reduzida"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_DEEPSILENCE] = {
		descript = {
			{"Mudez"},
			{"%s"},
			{
				"Vel.Atq reduzida"
			},
			{
				"Não pode usar"
			},
			{
				"as habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_LASSITUDE] = {
		descript = {
			{"Letargia"},
			{"%s"},
			{
				"CRIT reduzido"
			},
			{
				"Movimento reduzido"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_FROSTBITE] = {
		descript = {
			{
				"Esfriamento"
			},
			{"%s"},
			{
				"Armadura de prop. Água"
			},
			{
				"Não é possível mover, usar"
			},
			{
				"itens e usar habilidades"
			},
			{
				"DEF, DEFM reduzidas"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_SWOONING] = {
		descript = {
			{"Tontura"},
			{"%s"},
			{
				"Recebe mais dano."
			},
			{
				"Não é possível mover, usar"
			},
			{
				"itens nem habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_LIGHTNINGSTRIKE] = {
		descript = {
			{"Atormento"},
			{"%s"},
			{
				"Armadura de prop. Vento"
			},
			{
				"Reduz resist. a Terra"
			},
			{
				"Não é possível mover, usar"
			},
			{
				"itens ou habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_CRYSTALLIZATION] = {
		descript = {
			{
				"Concretação"
			},
			{"%s"},
			{
				"Armadura de prop. Terra"
			},
			{
				"DEFM reduzida"
			},
			{
				"Não é possível mover, usar"
			},
			{
				"itens ou habilidades"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_CONFLAGRATION] = {
		descript = {
			{"Ardência"},
			{"%s"},
			{
				"Armadura de prop. Fogo"
			},
			{
				"Reduz HP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_MISFORTUNE] = {
		descript = {
			{"Azar"},
			{"%s"},
			{
				"Precisão reduzida"
			},
			{
				"Ao usar habilidades,"
			},
			{
				"chance de falhar"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_DEADLYPOISON] = {
		descript = {
			{
				"Intoxicação"
			},
			{"%s"},
			{
				"DEF reduzida"
			},
			{
				"Reduz resist. à Veneno"
			},
			{
				"Reduz HP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_DEPRESSION] = {
		descript = {
			{"Tristeza"},
			{"%s"},
			{
				"Custo de SP maior"
			},
			{
				"Reduz SP continuamente"
			}
		}
	},
	[EFST_IDs.EFST_HANDICAPSTATE_HOLYFLAME] = {
		descript = {
			{
				"Fogo Sagrado"
			},
			{"%s"},
			{
				"Ao receber danos"
			},
			{
				"mágicos, cura HP."
			},
			{
				"Recebe mais dano"
			},
			{
				"de ataques físicos."
			}
		}
	},
	[EFST_IDs.EFST_REBOUND_S] = {
		descript = {
			{"Remissão"},
			{"%s"},
			{
				"Reduz o dano"
			},
			{
				"recebido pela"
			},
			{"Redenção"}
		}
	},
	[EFST_IDs.EFST_HOLY_S] = {
		descript = {
			{
				"Escudo Divino"
			},
			{"%s"},
			{
				"Resist. Sombrio e Maldito"
			},
			{
				"Dano mágico Sagrado"
			},
			{
				"Dano de Crux Tempestas"
			}
		}
	},
	[EFST_IDs.EFST_ULTIMATE_S] = {
		descript = {
			{"Ultimato"},
			{"%s"},
			{
				"Revive imediatamente"
			},
			{
				"logo após morrer"
			}
		}
	},
	[EFST_IDs.EFST_SPEAR_SCAR] = {
		descript = {
			{
				"Cicatriz da Justiça"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Toque do Oblívio e"
			},
			{
				"Disparo Perfurante"
			}
		}
	},
	[EFST_IDs.EFST_SHIELD_POWER] = {
		descript = {
			{
				"Escudo Reforçado"
			},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Aegis Inferi"
			},
			{
				"Choque Rápido e"
			},
			{
				"Escudo Compressor"
			}
		}
	},
	[EFST_IDs.EFST_SHADOW_WEAPON] = {
		descript = {
			{
				"Profanar Arma"
			},
			{"%s"},
			{
				"Arma de prop. Sombria."
			},
			{
				"Chance de infligir"
			},
			{
				"Profanação ao atacar"
			}
		}
	},
	[EFST_IDs.EFST_RELIGIO] = {
		descript = {
			{"Religio"},
			{"%s"},
			{
				"Bônus de FEI,"
			},
			{
				"SAB e de STA"
			}
		}
	},
	[EFST_IDs.EFST_BENEDICTUM] = {
		descript = {
			{"Benedictum"},
			{"%s"},
			{
				"Bônus de POD,"
			},
			{
				"CRV e de CON"
			}
		}
	},
	[EFST_IDs.EFST_FIRST_BRAND] = {
		descript = {
			{"Estigma"},
			{"%s"},
			{"A 1ª marca"}
		}
	},
	[EFST_IDs.EFST_SECOND_BRAND] = {
		descript = {
			{"Condenação"},
			{"%s"},
			{"A 2ª marca"}
		}
	},
	[EFST_IDs.EFST_SECOND_JUDGE] = {
		descript = {
			{
				"Auréola do Juiz"
			},
			{"%s"},
			{
				"Permite usar Ruína e"
			},
			{
				"Combo Rápido sem"
			},
			{
				"consumir  Esferas."
			}
		}
	},
	[EFST_IDs.EFST_THIRD_EXOR_FLAME] = {
		descript = {
			{
				"Auréola das Chamas"
			},
			{"%s"},
			{
				"Permite usar Ruína,"
			},
			{
				"Combo Rápido e"
			},
			{
				"Garra de Tigre sem"
			},
			{
				"consumir Esferas."
			}
		}
	},
	[EFST_IDs.EFST_FIRST_FAITH_POWER] = {
		descript = {
			{
				"Auréola do Poder"
			},
			{"%s"},
			{
				"Permite usar Ruína"
			},
			{
				"sem consumir as"
			},
			{
				"Esferas Espirituais."
			}
		}
	},
	[EFST_IDs.EFST_AXE_STOMP] = {
		descript = {
			{
				"Machado Esmagador"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Fúria do Furacão"
			}
		}
	},
	[EFST_IDs.EFST_A_MACHINE] = {
		descript = {
			{
				"Robô Atacante"
			},
			{"%s"},
			{
				"Causa dano físico"
			},
			{
				"corpo a corpo ao redor"
			}
		}
	},
	[EFST_IDs.EFST_D_MACHINE] = {
		descript = {
			{
				"Robô Defensor"
			},
			{"%s"},
			{
				"DEF aumentada"
			},
			{
				"RES aumentado"
			}
		}
	},
	[EFST_IDs.EFST_SHADOW_STRIP] = {
		descript = {
			{
				"Remoção Sombria"
			},
			{"%s"},
			{
				"Sombrios removidos e não"
			},
			{
				"podem ser reequipados"
			}
		}
	},
	[EFST_IDs.EFST_ABYSS_DAGGER] = {
		descript = {
			{"Ameaça"},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Ofensiva Fatal"
			}
		}
	},
	[EFST_IDs.EFST_ABYSSFORCEWEAPON] = {
		descript = {
			{
				"Invocacão do Abismo"
			},
			{"%s"},
			{
				"Ao realizar ataques"
			},
			{
				"básicos, as Esferas"
			},
			{
				"causarão dano no alvo"
			}
		}
	},
	[EFST_IDs.EFST_ABYSS_SLAYER] = {
		descript = {
			{
				"Sangue-Frio"
			},
			{"%s"},
			{
				"Bônus de P.ATQ"
			},
			{
				"Bônus de S.ATQM"
			}
		}
	},
	[EFST_IDs.EFST_PROTECTSHADOWEQUIP] = {
		descript = {
			{
				"Proteção Sombria"
			},
			{"%s"},
			{
				"Impede os Sombrios"
			},
			{
				"de serem removidos"
			}
		}
	},
	[EFST_IDs.EFST_RESEARCHREPORT] = {
		descript = {
			{"Eureka"},
			{"%s"},
			{
				"Aumenta o dano"
			},
			{
				"de Atirar Ácido e"
			},
			{
				"contra Amorfo e"
			},
			{"Plantas"}
		}
	},
	[EFST_IDs.EFST_BO_HELL_DUSTY] = {
		descript = {
			{
				"Aroma Infernal"
			},
			{"%s"},
			{
				"Aumenta o dano físico"
			},
			{
				"à distância e o dano"
			},
			{
				"contra Amorfo e Planta"
			}
		}
	},
	[EFST_IDs.EFST_WINDSIGN] = {
		descript = {
			{
				"Sinal do Vento"
			},
			{"%s"},
			{
				"Marca feita por"
			},
			{
				"Falcões do Vento"
			}
		}
	},
	[EFST_IDs.EFST_CALAMITYGALE] = {
		descript = {
			{
				"Ventos Sinistros"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Tiro Crescente e faz"
			},
			{
				"com que Vendaval de"
			},
			{
				"Flechas cause críticos."
			},
			{
				"Tiro Crescente e"
			},
			{
				"Vendaval de Flechas"
			},
			{
				"Causam mais dano"
			},
			{
				"Contra Bruto e Peixe."
			}
		}
	},
	[EFST_IDs.EFST_MYSTIC_SYMPHONY] = {
		descript = {
			{
				"Sinfonia Mística"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"algumas músicas e"
			},
			{
				"contra as raças"
			},
			{
				"Peixe e Humanoide."
			}
		}
	},
	[EFST_IDs.EFST_KVASIR_SONATA] = {
		descript = {
			{
				"Sonata de Kvasir"
			},
			{"%s"},
			{
				"Permite usar"
			},
			{
				"Coral ou Dueto"
			},
			{
				"sem uma dupla."
			}
		}
	},
	[EFST_IDs.EFST_SOUNDBLEND] = {
		descript = {
			{
				"Desarranjo Musical"
			},
			{"%s"},
			{
				"Marca de algo"
			},
			{"desafinado"}
		}
	},
	[EFST_IDs.EFST_GEF_NOCTURN] = {
		descript = {
			{
				"Recital de Geffenia"
			},
			{"%s"},
			{
				"TENM reduzido"
			}
		}
	},
	[EFST_IDs.EFST_AIN_RHAPSODY] = {
		descript = {
			{
				"Rapsódia do Minerador"
			},
			{"%s"},
			{
				"TEN reduzido"
			}
		}
	},
	[EFST_IDs.EFST_MUSICAL_INTERLUDE] = {
		descript = {
			{"Interlúdio"},
			{"%s"},
			{
				"Bônus de TEN"
			}
		}
	},
	[EFST_IDs.EFST_JAWAII_SERENADE] = {
		descript = {
			{
				"Serenata de Jawaii"
			},
			{"%s"},
			{
				"Bônus de S.ATQM"
			},
			{
				"Acelera o movimento"
			}
		}
	},
	[EFST_IDs.EFST_PRON_MARCH] = {
		descript = {
			{
				"Marcha de Prontera"
			},
			{"%s"},
			{
				"Bônus de P.ATQ"
			},
			{
				"Acelera o movimento"
			}
		}
	},
	[EFST_IDs.EFST_SPELL_ENCHANTING] = {
		descript = {
			{"Enfeitiçar"},
			{"%s"},
			{
				"Bônus de S.ATQM"
			}
		}
	},
	[EFST_IDs.EFST_HOMUN_TIME] = {
		descript = {
			{"Homunculus"},
			{"%s"},
			{"Homunculus"},
			{"despertado"}
		}
	},
	[EFST_IDs.EFST_POISON_MIST] = {
		descript = {
			{
				"Névoa Venenosa"
			},
			{"%s"},
			{
				"Reduz Esquiva"
			}
		}
	},
	[EFST_IDs.EFST_STONE_WALL] = {
		descript = {
			{
				"Muro de Rocha"
			},
			{"%s"},
			{
				"DEF aumentada"
			},
			{
				"DEFM aumentada"
			}
		}
	},
	[EFST_IDs.EFST_OVERBRANDREADY] = {
		descript = {
			{
				"Lança Lunar"
			},
			{"%s"},
			{
				"Aumenta o dano de"
			},
			{
				"Lança do Destino"
			}
		}
	},
	[EFST_IDs.EFST_SHIELDSPELL] = {
		descript = {
			{
				"Aegis Domini"
			},
			{"%s"},
			{
				"Escudo encantado"
			}
		}
	},
	[EFST_IDs.EFST_CLOUD_POISON] = {
		descript = {
			{"Infecção"},
			{"%s"},
			{
				"Reduz a resist."
			},
			{
				"à prop. Veneno"
			}
		}
	},
	[EFST_IDs.EFST_SPORE_EXPLOSION_DEBUFF] = {
		descript = {
			{"Esporão"},
			{"%s"},
			{
				"Recebe mais dano"
			},
			{
				"físico à distância."
			}
		}
	},
	[EFST_IDs.EFST_DEFSCROLL] = {
		descript = {
			{
				"Perg. de Defesa"
			},
			{"%s"},
			{
				"DEF aumentada"
			}
		}
	},
	[EFST_IDs.EFST_MASSIVE_F_BLASTER] = {
		descript = {
			{
				"Fervor Espiritual"
			},
			{"%s"},
			{
				"Pode usar Explosão Espiritual"
			},
			{
				"e Tempestade Espiritual sem"
			},
			{
				"consumir Esferas Espirituais"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPWEAPON2] = {
		descript = {
			{
				"Remover Arma Sombria"
			},
			{"%s"},
			{
				"Arma Sombria removida e"
			},
			{
				"não pode ser reequipada"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPARMOR2] = {
		descript = {
			{
				"Remover Armadura Sombria"
			},
			{"%s"},
			{
				"Armadura Sombria removida"
			},
			{
				"e não pode ser reequipada"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPSHIELD2] = {
		descript = {
			{
				"Remover Escudo Sombrio"
			},
			{"%s"},
			{
				"Escudo Sombrio removido"
			},
			{
				"e não pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPSHOES2] = {
		descript = {
			{
				"Remover Greva Sombria"
			},
			{"%s"},
			{
				"Greva Sombria removida e"
			},
			{
				"não pode ser reequipada"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPPENDANT2] = {
		descript = {
			{
				"Remover Colar Sombrio"
			},
			{"%s"},
			{
				"Colar Sombrio removido e"
			},
			{
				"não pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPEARING2] = {
		descript = {
			{
				"Remover Brinco Sombrio"
			},
			{"%s"},
			{
				"Colar Sombrio removido e"
			},
			{
				"não pode ser reequipado"
			}
		}
	},
	[EFST_IDs.EFST_NOEQUIPFULL2] = {
		descript = {
			{
				"Remoção Sombria Total"
			},
			{"%s"},
			{
				"Equip. Sombrios removidos e"
			},
			{
				"não podem ser reequipados"
			}
		}
	},
	[EFST_IDs.EFST_TOXIN_OF_MANDARA] = {
		descript = {
			{
				"Neurotoxina"
			},
			{"%s"},
			{"Reduz TEN."}
		}
	},
	[EFST_IDs.EFST_GOLDENE_TONE] = {
		descript = {
			{
				"Couro Dourado"
			},
			{"%s"},
			{
				"Bônus de TEN."
			},
			{
				"Bônus de TENM."
			}
		}
	},
	[EFST_IDs.EFST_TEMPERING] = {
		descript = {
			{"Têmpera"},
			{"%s"},
			{
				"Aumenta o P.ATQ"
			}
		}
	},
	[EFST_IDs.EFST_INTENSIVE_AIM] = {
		descript = {
			{
				"Mira Concentrada Ativa"
			},
			{
				"ATQ aumentado"
			},
			{
				"Precisão aumentada"
			},
			{
				"Crítico aumentado"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_1] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Água"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_2] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Vento"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_3] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Terra"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_4] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Fogo"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_5] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Sombrio"
			}
		}
	},
	[EFST_IDs.EFST_GRENADE_FRAGMENT_6] = {
		descript = {
			{
				"Fragmentação"
			},
			{"%s"},
			{
				"Ao atacar com granada, aplica propriedade Sagrado"
			}
		}
	},
	[EFST_IDs.EFST_AUTO_FIRING_LAUNCHEREFST] = {
		descript = {
			{
				"Lançador de Disparo Automático"
			},
			{"%s"},
			{
				"Estado de ativação do lançador automático de granadas"
			}
		}
	},
	[EFST_IDs.EFST_HIDDEN_CARD] = {
		descript = {
			{
				"Carta na Manga"
			},
			{"%s"},
			{
				"Dano físico à distância aumentado"
			},
			{
				"P.ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_NW_GRENADE_MASTERY] = {
		descript = {
			{
				"Maestria em Granada"
			},
			{"%s"},
			{
				"CON aumentado"
			},
			{
				"Dano de habilidades da linha Granada aumentado"
			}
		}
	},
	[EFST_IDs.EFST_TALISMAN_OF_PROTECTION] = {
		descript = {
			{
				"Talismã do Protetor"
			},
			{"%s"},
			{
				"Recuperação contínua de HP"
			}
		}
	},
	[EFST_IDs.EFST_TALISMAN_OF_WARRIOR] = {
		descript = {
			{
				"Talismã do Guerreiro"
			},
			{"%s"},
			{
				"P.ATQ aumentado"
			}
		}
	},
	[EFST_IDs.EFST_TALISMAN_OF_MAGICIAN] = {
		descript = {
			{
				"Talismã do Mágico"
			},
			{"%s"},
			{
				"S.ATQM aumentado"
			}
		}
	},
	[EFST_IDs.EFST_TALISMAN_OF_FIVE_ELEMENTS] = {
		descript = {
			{
				"Talismã dos Elementos"
			},
			{"%s"},
			{
				"Dano físico causado a inimigos de propriedade Água/Vento/Terra/Fogo/Neutro aumentado"
			},
			{
				"Dano mágico causado a inimigos de propriedade Água/Vento/Terra/Fogo/Neutro aumentado"
			}
		}
	},
	[EFST_IDs.EFST_T_FIRST_GOD] = {
		descript = {
			{
				"Bênção do Leste"
			},
			{"%s"},
			{
				"Estado de bênção do leste"
			}
		}
	},
	[EFST_IDs.EFST_T_SECOND_GOD] = {
		descript = {
			{
				"Bênção do Oeste"
			},
			{"%s"},
			{
				"Estado de bênção do oeste"
			}
		}
	},
	[EFST_IDs.EFST_T_THIRD_GOD] = {
		descript = {
			{
				"Bênção do Sul"
			},
			{"%s"},
			{
				"Estado de bênção do sul"
			}
		}
	},
	[EFST_IDs.EFST_T_FOURTH_GOD] = {
		descript = {
			{
				"Bênção do Norte"
			},
			{"%s"},
			{
				"Estado de bênção do norte"
			}
		}
	},
	[EFST_IDs.EFST_T_FIVETH_GOD] = {
		descript = {
			{
				"Bênção dos Quatro Pontos Cardeais e Cinco Elementos"
			},
			{"%s"},
			{
				"Estado de bênção dos quatro pontos cardeais e cinco elementos"
			}
		}
	},
	[EFST_IDs.EFST_HEAVEN_AND_EARTH] = {
		descript = {
			{
				"Reencarnação das Almas"
			},
			{"%s"},
			{
				"Dano físico corpo a corpo aumentado"
			},
			{
				"Dano físico à distância aumentado"
			},
			{
				"Dano mágico de todas as propriedades aumentado"
			}
		}
	},
	[EFST_IDs.EFST_HOGOGONG] = {
		descript = {
			{"Patada"},
			{"%s"},
			{
				"Estado de Patada"
			}
		}
	},
	[EFST_IDs.EFST_MARINE_FESTIVAL] = {
		descript = {
			{
				"Festival Marinho"
			},
			{"%s"},
			{
				"POW, CRT, CON aumentados"
			}
		}
	},
	[EFST_IDs.EFST_SANDY_FESTIVAL] = {
		descript = {
			{
				"Luau nas Areias"
			},
			{"%s"},
			{
				"SPL, WIS, STA aumentados"
			}
		}
	},
	[EFST_IDs.EFST_KI_SUL_RAMPAGE] = {
		descript = {
			{
				"Ira da Tartaruga"
			},
			{"%s"},
			{
				"Recupera AP"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_1] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Água às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_2] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Vento às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_3] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Terra às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_4] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Fogo às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_5] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Sombrio às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_6] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Aplica propriedade Sagrado às habilidades abaixo"
			},
			{
				"Meteoros de Nepeta"
			},
			{
				"Brisa Silvestre"
			},
			{
				"Lança Galhada"
			},
			{
				"Poder Espiritual da Corça Esmeralda"
			}
		}
	},
	[EFST_IDs.EFST_COLORS_OF_HYUN_ROK_BUFF] = {
		descript = {
			{
				"Cores do Arco-Íris"
			},
			{"%s"},
			{
				"Dano de Meteoros de Nepeta aumentado"
			}
		}
	},
	[EFST_IDs.EFST_TEMPORARY_COMMUNION] = {
		descript = {
			{
				"Aliança Animal"
			},
			{"%s"},
			{
				"P.ATQ, S.ATQM, HEAL PLUS aumentados"
			}
		}
	},
	[EFST_IDs.EFST_BLESSING_OF_M_CREATURES] = {
		descript = {
			{
				"Bênção do Espírito Animal"
			},
			{"%s"},
			{
				"P.ATQ, S.ATQM aumentados"
			}
		}
	},
	[EFST_IDs.EFST_BLESSING_OF_M_C_DEBUFF] = {
		descript = {
			{
				"Efeito colateral da bênção do espírito animal"
			},
			{"%s"},
			{
				"Não é possível receber a bênção do espírito animal por certo tempo"
			}
		}
	},
	[EFST_IDs.EFST_SHIELDCHAINRUSH] = {
		descript = {
			{
				"Choque Violento"
			},
			{"%s"},
			{
				"Velocidade de movimento reduzida"
			},
			{
				"Dano físico recebido aumentado"
			},
			{
				"Dano mágico recebido aumentado"
			}
		}
	},
	[EFST_IDs.EFST_MISTYFROST] = {
		descript = {
			{
				"Frio Extremo"
			},
			{"%s"},
			{
				"Resistência à propriedade Água -15%"
			}
		}
	},
	[EFST_IDs.EFST_GROUNDGRAVITY] = {
		descript = {
			{
				"Zona Gravitacional"
			},
			{"%s"},
			{
				"Velocidade de movimento reduzida"
			},
			{
				"Dano físico recebido aumentado"
			},
			{
				"Dano mágico recebido aumentado"
			}
		}
	},
	[EFST_IDs.EFST_BREAKINGLIMIT] = {
		descript = {
			{
				"Anjo da Prosperidade"
			},
			{"%s"},
			{
				"Dano de habilidades da linha de combate amplificado"
			}
		}
	},
	[EFST_IDs.EFST_RULEBREAK] = {
		descript = {
			{
				"Anjo da Libertação"
			},
			{"%s"},
			{
				"Dano de habilidades da linha mágica amplificado"
			}
		}
	},
	[EFST_IDs.EFST_RISING_SUN] = {
		descript = {
			{
				"Nascer do Sol"
			},
			{"%s"},
			{
				"Estado de nascer do sol"
			}
		}
	},
	[EFST_IDs.EFST_NOON_SUN] = {
		descript = {
			{"Meio-dia"},
			{"%s"},
			{
				"Estado de meio-dia"
			}
		}
	},
	[EFST_IDs.EFST_SUNSET_SUN] = {
		descript = {
			{"Pôr do Sol"},
			{"%s"},
			{
				"Estado de pôr do sol"
			}
		}
	},
	[EFST_IDs.EFST_RISING_MOON] = {
		descript = {
			{
				"Nascer da Lua"
			},
			{"%s"},
			{
				"Estado de nascer da lua"
			}
		}
	},
	[EFST_IDs.EFST_MIDNIGHT_MOON] = {
		descript = {
			{"Meia-noite"},
			{"%s"},
			{
				"Estado de meia-noite"
			}
		}
	},
	[EFST_IDs.EFST_DAWN_MOON] = {
		descript = {
			{"Pôr da Lua"},
			{"%s"},
			{
				"Estado de pôr da lua"
			}
		}
	},
	[EFST_IDs.EFST_STAR_BURST] = {
		descript = {
			{
				"Colapso Estelar"
			},
			{"%s"},
			{
				"Estado de Colapso Estelar"
			}
		}
	},
	[EFST_IDs.EFST_SKY_ENCHANT] = {
		descript = {
			{
				"União Celestial"
			},
			{"%s"},
			{
				"Estado de maximização do fluxo celestial"
			}
		}
	},
	[EFST_IDs.EFST_SHADOW_CLOCK] = {
		descript = {
			{
				"Furtividade Sombria"
			},
			{"%s"},
			{
				"Velocidade de movimento aumentada"
			},
			{
				"Dano físico recebido reduzido"
			},
			{
				"Dano mágico recebido reduzido"
			}
		}
	},
	[EFST_IDs.EFST_NIGHTMARE] = {
		descript = {
			{
				"Pedra do Pesadelo"
			},
			{"%s"},
			{
				"Marca de Pedra do Pesadelo"
			}
		}
	},
	[EFST_IDs.EFST_RUSH_QUAKE1] = {
		descript = {
			{"Sismo"},
			{"%s"},
			{
				"Recebe mais dano"
			},
			{
				"à distância e"
			},
			{
				"corpo a corpo."
			}
		}
	},
	[EFST_IDs.EFST_RUSH_QUAKE2] = {
		descript = {
			{"Avanço"},
			{"%s"},
			{
				"Causa mais dano"
			},
			{
				"à distância e"
			},
			{
				"corpo a corpo."
			}
		}
	},
	[EFST_IDs.EFST_C_BUFF_1] = {
		descript = {
			{
				"Estimulante"
			},
			{
				"Todos os Talentos +5"
			},
			{
				"P.ATQ e S.ATQM +10"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_C_BUFF_2] = {
		descript = {
			{
				"Elixir Celestial"
			},
			{
				"Vel.Atq +1."
			},
			{
				"Esquiva +50."
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_C_BUFF_3] = {
		descript = {
			{
				"Pluma Resistente"
			},
			{
				"HP máx. +5%"
			},
			{
				"SP máx. +5%"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_C_BUFF_4] = {
		descript = {
			{"Pena Leve"},
			{
				"Esquiva +25"
			},
			{
				"Precisão +25"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_C_BUFF_5] = {
		descript = {
			{
				"Pena Glamurosa"
			},
			{"CRIT + 10"},
			{
				"Vel.Atq + 1"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_C_BUFF_6] = {
		descript = {
			{
				"Pena Maciça"
			},
			{
				"ATQ da arma +7%"
			},
			{
				"Dano Mágico +7%"
			},
			{"%s"}
		}
	},
	[EFST_IDs.EFST_OVERSEA_BUFF_12] = {
		descript = {
			{"Escrituras"},
			{"%s"},
			{
				"Bônus de DEF"
			},
			{
				"Bônus de DEFM"
			}
		}
	},
	[EFST_IDs.EFST_OVERSEA_BUFF_30] = {
		descript = {
			{
				"Música de Samba"
			},
			{"%s"},
			{
				"Precisão perfeita +30."
			},
			{
				"5% de chance de converter"
			},
			{
				"5% do dano físico em SP."
			}
		}
	},
	[EFST_IDs.EFST_OVERSEA_BUFF_31] = {
		descript = {
			{
				"Glitter de Carnaval"
			},
			{"%s"},
			{
				"Pós-conjuração -15%."
			},
			{
				"Efetividade de cura +30%."
			}
		}
	}
}
