SkillInfoList_string = {
	[SKID.NV_BASIC] = {
		SkillName = "Habilidades Básicas"
	},
	[SKID.SM_SWORD] = {
		SkillName = "Perícia com Espada"
	},
	[SKID.SM_TWOHAND] = {
		SkillName = "Perícia com Espada de Duas Mãos"
	},
	[SKID.SM_RECOVERY] = {
		SkillName = "Aumentar Recuperação de HP"
	},
	[SKID.SM_BASH] = {
		SkillName = "Golpe Fulminante"
	},
	[SKID.SM_PROVOKE] = {SkillName = "Provocar"},
	[SKID.SM_MAGNUM] = {
		SkillName = "Impacto Explosivo"
	},
	[SKID.SM_ENDURE] = {SkillName = "Vigor"},
	[SKID.MG_SRECOVERY] = {
		SkillName = "Aumentar Recuperação de SP"
	},
	[SKID.MG_SIGHT] = {
		SkillName = "Chama Reveladora"
	},
	[SKID.MG_NAPALMBEAT] = {
		SkillName = "Ataque Espiritual"
	},
	[SKID.MG_SAFETYWALL] = {
		SkillName = "Escudo Mágico"
	},
	[SKID.MG_SOULSTRIKE] = {
		SkillName = "Espíritos Anciões"
	},
	[SKID.MG_COLDBOLT] = {
		SkillName = "Lanças de Gelo"
	},
	[SKID.MG_FROSTDIVER] = {
		SkillName = "Rajada Congelante"
	},
	[SKID.MG_STONECURSE] = {SkillName = "Petrificar"},
	[SKID.MG_FIREBALL] = {
		SkillName = "Bolas de Fogo"
	},
	[SKID.MG_FIREWALL] = {
		SkillName = "Barreira de Fogo"
	},
	[SKID.MG_FIREBOLT] = {
		SkillName = "Lanças de Fogo"
	},
	[SKID.MG_LIGHTNINGBOLT] = {SkillName = "Relâmpago"},
	[SKID.MG_THUNDERSTORM] = {
		SkillName = "Tempestade de Raios"
	},
	[SKID.AL_DP] = {
		SkillName = "Proteção Divina"
	},
	[SKID.AL_DEMONBANE] = {
		SkillName = "Flagelo do Mal"
	},
	[SKID.AL_RUWACH] = {SkillName = "Revelação"},
	[SKID.AL_PNEUMA] = {
		SkillName = "Escudo Sagrado"
	},
	[SKID.AL_TELEPORT] = {SkillName = "Teleporte"},
	[SKID.AL_WARP] = {SkillName = "Portal"},
	[SKID.AL_HEAL] = {SkillName = "Curar"},
	[SKID.AL_INCAGI] = {
		SkillName = "Aumentar Agilidade"
	},
	[SKID.AL_DECAGI] = {
		SkillName = "Diminuir Agilidade"
	},
	[SKID.AL_HOLYWATER] = {
		SkillName = "Aqua Benedicta"
	},
	[SKID.AL_CRUCIS] = {
		SkillName = "Signum Crucis"
	},
	[SKID.AL_ANGELUS] = {SkillName = "Angelus"},
	[SKID.AL_BLESSING] = {SkillName = "Bênção"},
	[SKID.AL_CURE] = {SkillName = "Medicar"},
	[SKID.MC_INCCARRY] = {
		SkillName = "Aumentar Capacidade de Carga"
	},
	[SKID.MC_DISCOUNT] = {SkillName = "Desconto"},
	[SKID.MC_OVERCHARGE] = {
		SkillName = "Superfaturar"
	},
	[SKID.MC_PUSHCART] = {
		SkillName = "Usar Carrinho"
	},
	[SKID.MC_IDENTIFY] = {
		SkillName = "Identificar Item"
	},
	[SKID.MC_VENDING] = {SkillName = "Comércio"},
	[SKID.MC_MAMMONITE] = {SkillName = "Mammonita"},
	[SKID.AC_OWL] = {
		SkillName = "Olhos de Coruja"
	},
	[SKID.AC_VULTURE] = {
		SkillName = "Olhos de Águia"
	},
	[SKID.AC_CONCENTRATION] = {SkillName = "Concentrar"},
	[SKID.AC_DOUBLE] = {
		SkillName = "Rajada de Flechas"
	},
	[SKID.AC_SHOWER] = {
		SkillName = "Chuva de Flechas"
	},
	[SKID.TF_DOUBLE] = {
		SkillName = "Ataque Duplo"
	},
	[SKID.TF_MISS] = {
		SkillName = "Perícia em Esquiva"
	},
	[SKID.TF_STEAL] = {SkillName = "Furto"},
	[SKID.TF_HIDING] = {
		SkillName = "Esconderijo"
	},
	[SKID.TF_POISON] = {SkillName = "Envenenar"},
	[SKID.TF_DETOXIFY] = {
		SkillName = "Desintoxicar"
	},
	[SKID.ALL_RESURRECTION] = {
		SkillName = "Ressuscitar"
	},
	[SKID.KN_SPEARMASTERY] = {
		SkillName = "Perícia com Lança"
	},
	[SKID.KN_PIERCE] = {SkillName = "Perfurar"},
	[SKID.KN_BRANDISHSPEAR] = {
		SkillName = "Brandir Lança"
	},
	[SKID.KN_SPEARSTAB] = {SkillName = "Estocada"},
	[SKID.KN_SPEARBOOMERANG] = {
		SkillName = "Lança Bumerangue"
	},
	[SKID.KN_TWOHANDQUICKEN] = {
		SkillName = "Rapidez com Duas Mãos"
	},
	[SKID.KN_AUTOCOUNTER] = {
		SkillName = "Contra-Ataque"
	},
	[SKID.KN_BOWLINGBASH] = {
		SkillName = "Impacto de Tyr"
	},
	[SKID.KN_RIDING] = {SkillName = "Montaria"},
	[SKID.KN_CAVALIERMASTERY] = {
		SkillName = "Perícia em Montaria"
	},
	[SKID.PR_MACEMASTERY] = {
		SkillName = "Perícia com Maça"
	},
	[SKID.PR_IMPOSITIO] = {
		SkillName = "Impositio Manus"
	},
	[SKID.PR_SUFFRAGIUM] = {SkillName = "Suffragium"},
	[SKID.PR_ASPERSIO] = {SkillName = "Aspersio"},
	[SKID.PR_BENEDICTIO] = {
		SkillName = "B.S Sacramenti"
	},
	[SKID.PR_SANCTUARY] = {SkillName = "Santuário"},
	[SKID.PR_SLOWPOISON] = {
		SkillName = "Retardar Veneno"
	},
	[SKID.PR_STRECOVERY] = {
		SkillName = "Graça Divina"
	},
	[SKID.PR_KYRIE] = {
		SkillName = "Kyrie Eleison"
	},
	[SKID.PR_MAGNIFICAT] = {SkillName = "Magnificat"},
	[SKID.PR_GLORIA] = {SkillName = "Glória"},
	[SKID.PR_LEXDIVINA] = {SkillName = "Lex Divina"},
	[SKID.PR_TURNUNDEAD] = {SkillName = "Esconjurar"},
	[SKID.PR_LEXAETERNA] = {
		SkillName = "Lex Aeterna"
	},
	[SKID.PR_MAGNUS] = {
		SkillName = "Magnus Exorcismus"
	},
	[SKID.WZ_FIREPILLAR] = {
		SkillName = "Coluna de Fogo"
	},
	[SKID.WZ_SIGHTRASHER] = {SkillName = "Supernova"},
	[SKID.WZ_FIREIVY] = {
		SkillName = "Hera de Fogo"
	},
	[SKID.WZ_METEOR] = {
		SkillName = "Chuva de Meteoros"
	},
	[SKID.WZ_JUPITEL] = {
		SkillName = "Trovão de Júpiter"
	},
	[SKID.WZ_VERMILION] = {
		SkillName = "Ira de Thor"
	},
	[SKID.WZ_WATERBALL] = {
		SkillName = "Esfera d'Água"
	},
	[SKID.WZ_ICEWALL] = {
		SkillName = "Barreira de Gelo"
	},
	[SKID.WZ_FROSTNOVA] = {SkillName = "Congelar"},
	[SKID.WZ_STORMGUST] = {SkillName = "Nevasca"},
	[SKID.WZ_EARTHSPIKE] = {
		SkillName = "Coluna de Pedra"
	},
	[SKID.WZ_HEAVENDRIVE] = {
		SkillName = "Fúria da Terra"
	},
	[SKID.WZ_QUAGMIRE] = {
		SkillName = "Pântano dos Mortos"
	},
	[SKID.WZ_ESTIMATION] = {
		SkillName = "Sentido Sobrenatural"
	},
	[SKID.BS_IRON] = {
		SkillName = "Trabalhar Ferro"
	},
	[SKID.BS_STEEL] = {
		SkillName = "Trabalhar Aço"
	},
	[SKID.BS_ENCHANTEDSTONE] = {
		SkillName = "Produzir Pedra Fundamental"
	},
	[SKID.BS_ORIDEOCON] = {
		SkillName = "Perícia com Oridecon"
	},
	[SKID.BS_DAGGER] = {
		SkillName = "Forjar Adaga"
	},
	[SKID.BS_SWORD] = {
		SkillName = "Forjar Espada"
	},
	[SKID.BS_TWOHANDSWORD] = {
		SkillName = "Forjar Espada de Duas Mãos"
	},
	[SKID.BS_AXE] = {
		SkillName = "Forjar Machado"
	},
	[SKID.BS_MACE] = {
		SkillName = "Forjar Maça"
	},
	[SKID.BS_KNUCKLE] = {
		SkillName = "Forjar Soqueira"
	},
	[SKID.BS_SPEAR] = {
		SkillName = "Forjar Lança"
	},
	[SKID.BS_HILTBINDING] = {
		SkillName = "Punho Firme"
	},
	[SKID.BS_FINDINGORE] = {
		SkillName = "Encontrar Minério"
	},
	[SKID.BS_WEAPONRESEARCH] = {
		SkillName = "Perícia em Armamento"
	},
	[SKID.BS_REPAIRWEAPON] = {
		SkillName = "Consertar Armas"
	},
	[SKID.BS_SKINTEMPER] = {
		SkillName = "Resistência ao Fogo"
	},
	[SKID.BS_HAMMERFALL] = {
		SkillName = "Martelo de Thor"
	},
	[SKID.BS_ADRENALINE] = {
		SkillName = "Adrenalina Pura"
	},
	[SKID.BS_WEAPONPERFECT] = {
		SkillName = "Manejo Perfeito"
	},
	[SKID.BS_OVERTHRUST] = {
		SkillName = "Força Violenta"
	},
	[SKID.BS_MAXIMIZE] = {
		SkillName = "Amplificar Poder"
	},
	[SKID.HT_SKIDTRAP] = {
		SkillName = "Armadilha Escorregadia"
	},
	[SKID.HT_LANDMINE] = {
		SkillName = "Armadilha Atordoante"
	},
	[SKID.HT_ANKLESNARE] = {
		SkillName = "Instalar Armadilha"
	},
	[SKID.HT_SHOCKWAVE] = {
		SkillName = "Armadilha Extenuante"
	},
	[SKID.HT_SANDMAN] = {
		SkillName = "Armadilha Sonífera"
	},
	[SKID.HT_FLASHER] = {
		SkillName = "Armadilha Luminosa"
	},
	[SKID.HT_FREEZINGTRAP] = {
		SkillName = "Armadilha Congelante"
	},
	[SKID.HT_BLASTMINE] = {
		SkillName = "Instalar Mina"
	},
	[SKID.HT_CLAYMORETRAP] = {
		SkillName = "Armadilha Explosiva"
	},
	[SKID.HT_REMOVETRAP] = {
		SkillName = "Remover Armadilha"
	},
	[SKID.HT_TALKIEBOX] = {
		SkillName = "Mensagem Secreta"
	},
	[SKID.HT_BEASTBANE] = {
		SkillName = "Flagelo das Feras"
	},
	[SKID.HT_FALCON] = {
		SkillName = "Adestrar Ave"
	},
	[SKID.HT_STEELCROW] = {
		SkillName = "Garras de Aço"
	},
	[SKID.HT_BLITZBEAT] = {
		SkillName = "Ataque Aéreo"
	},
	[SKID.HT_DETECTING] = {SkillName = "Alerta"},
	[SKID.HT_SPRINGTRAP] = {
		SkillName = "Desativar Armadilha"
	},
	[SKID.AS_RIGHT] = {
		SkillName = "Perícia com Mão Direita"
	},
	[SKID.AS_LEFT] = {
		SkillName = "Perícia com Mão Esquerda"
	},
	[SKID.AS_KATAR] = {
		SkillName = "Perícia com Katar"
	},
	[SKID.AS_CLOAKING] = {
		SkillName = "Furtividade"
	},
	[SKID.AS_SONICBLOW] = {
		SkillName = "Lâminas Destruidoras"
	},
	[SKID.AS_GRIMTOOTH] = {SkillName = "Tocaia"},
	[SKID.AS_ENCHANTPOISON] = {
		SkillName = "Envenenar Arma"
	},
	[SKID.AS_POISONREACT] = {
		SkillName = "Refletir Veneno"
	},
	[SKID.AS_VENOMDUST] = {
		SkillName = "Névoa Tóxica"
	},
	[SKID.AS_SPLASHER] = {
		SkillName = "Explosão Tóxica"
	},
	[SKID.NV_FIRSTAID] = {
		SkillName = "Primeiros Socorros"
	},
	[SKID.NV_TRICKDEAD] = {
		SkillName = "Fingir de Morto"
	},
	[SKID.SM_MOVINGRECOVERY] = {
		SkillName = "Recuperar HP em Movimento"
	},
	[SKID.SM_FATALBLOW] = {
		SkillName = "Ataque Fatal"
	},
	[SKID.SM_AUTOBERSERK] = {
		SkillName = "Instinto de Sobrevivência"
	},
	[SKID.AC_MAKINGARROW] = {
		SkillName = "Fabricar Flechas"
	},
	[SKID.AC_CHARGEARROW] = {
		SkillName = "Disparo Violento"
	},
	[SKID.TF_SPRINKLESAND] = {
		SkillName = "Chutar Areia"
	},
	[SKID.TF_BACKSLIDING] = {SkillName = "Recuar"},
	[SKID.TF_PICKSTONE] = {
		SkillName = "Procurar Pedras"
	},
	[SKID.TF_THROWSTONE] = {
		SkillName = "Arremessar Pedra"
	},
	[SKID.MC_CARTREVOLUTION] = {
		SkillName = "Cavalo-de-Pau"
	},
	[SKID.MC_CHANGECART] = {
		SkillName = "Personalizar Carrinho"
	},
	[SKID.MC_LOUD] = {
		SkillName = "Grito de Guerra"
	},
	[SKID.AL_HOLYLIGHT] = {SkillName = "Luz Divina"},
	[SKID.MG_ENERGYCOAT] = {
		SkillName = "Proteção Arcana"
	},
	[SKID.NPC_DEFENDER] = {
		SkillName = "Aura Sagrada"
	},
	[SKID.RG_SNATCHER] = {SkillName = "Mãos Leves"},
	[SKID.RG_STEALCOIN] = {SkillName = "Afanar"},
	[SKID.RG_BACKSTAP] = {SkillName = "Apunhalar"},
	[SKID.RG_TUNNELDRIVE] = {
		SkillName = "Túnel de Fuga"
	},
	[SKID.RG_RAID] = {
		SkillName = "Ataque Surpresa"
	},
	[SKID.RG_STRIPWEAPON] = {
		SkillName = "Remover Arma"
	},
	[SKID.RG_STRIPSHIELD] = {
		SkillName = "Remover Escudo"
	},
	[SKID.RG_STRIPARMOR] = {
		SkillName = "Remover Armadura"
	},
	[SKID.RG_STRIPHELM] = {
		SkillName = "Remover Capacete"
	},
	[SKID.RG_INTIMIDATE] = {SkillName = "Rapto"},
	[SKID.RG_GRAFFITI] = {SkillName = "Graffiti"},
	[SKID.RG_FLAGGRAFFITI] = {SkillName = "Pichar"},
	[SKID.RG_CLEANER] = {SkillName = "Faxina"},
	[SKID.RG_GANGSTER] = {
		SkillName = "Malandragem"
	},
	[SKID.RG_COMPULSION] = {SkillName = "Extorquir"},
	[SKID.RG_PLAGIARISM] = {SkillName = "Plágio"},
	[SKID.AM_AXEMASTERY] = {
		SkillName = "Perícia com Machado e Espada"
	},
	[SKID.AM_LEARNINGPOTION] = {
		SkillName = "Pesquisa de Poções"
	},
	[SKID.AM_PHARMACY] = {
		SkillName = "Preparar Poção"
	},
	[SKID.AM_DEMONSTRATION] = {SkillName = "Fogo Grego"},
	[SKID.AM_ACIDTERROR] = {
		SkillName = "Terror Ácido"
	},
	[SKID.AM_POTIONPITCHER] = {
		SkillName = "Arremessar Poção"
	},
	[SKID.AM_CANNIBALIZE] = {
		SkillName = "Criar Monstro Planta"
	},
	[SKID.AM_SPHEREMINE] = {
		SkillName = "Criar Esfera Marinha"
	},
	[SKID.AM_CP_WEAPON] = {
		SkillName = "Revestir Arma"
	},
	[SKID.AM_CP_SHIELD] = {
		SkillName = "Revestir Escudo"
	},
	[SKID.AM_CP_ARMOR] = {
		SkillName = "Revestir Armadura"
	},
	[SKID.AM_CP_HELM] = {
		SkillName = "Revestir Capacete"
	},
	[SKID.AM_BIOETHICS] = {SkillName = "Bioética"},
	[SKID.AM_BIOTECHNOLOGY] = {
		SkillName = "Biotecnologia"
	},
	[SKID.AM_CREATECREATURE] = {
		SkillName = "Criar Criatura"
	},
	[SKID.AM_CULTIVATION] = {SkillName = "Cultivo"},
	[SKID.AM_FLAMECONTROL] = {
		SkillName = "Controle das Chamas"
	},
	[SKID.AM_CALLHOMUN] = {
		SkillName = "Despertar Homunculus"
	},
	[SKID.AM_REST] = {SkillName = "Vaporizar"},
	[SKID.AM_DRILLMASTER] = {SkillName = "Semeadeira"},
	[SKID.AM_HEALHOMUN] = {
		SkillName = "Curar Homunculus"
	},
	[SKID.AM_RESURRECTHOMUN] = {
		SkillName = "Ressuscitar Homunculus"
	},
	[SKID.CR_TRUST] = {SkillName = "Fé"},
	[SKID.CR_AUTOGUARD] = {SkillName = "Bloqueio"},
	[SKID.CR_SHIELDCHARGE] = {
		SkillName = "Punição Divina"
	},
	[SKID.CR_SHIELDBOOMERANG] = {
		SkillName = "Escudo Bumerangue"
	},
	[SKID.CR_REFLECTSHIELD] = {
		SkillName = "Escudo Refletor"
	},
	[SKID.CR_HOLYCROSS] = {
		SkillName = "Crux Divinum"
	},
	[SKID.CR_GRANDCROSS] = {
		SkillName = "Crux Magnum"
	},
	[SKID.CR_DEVOTION] = {SkillName = "Redenção"},
	[SKID.CR_PROVIDENCE] = {
		SkillName = "Divina Providência"
	},
	[SKID.CR_DEFENDER] = {
		SkillName = "Aura Sagrada"
	},
	[SKID.CR_SPEARQUICKEN] = {
		SkillName = "Rapidez com Lança"
	},
	[SKID.MO_IRONHAND] = {
		SkillName = "Punhos de Ferro"
	},
	[SKID.MO_SPIRITSRECOVERY] = {SkillName = "Meditação"},
	[SKID.MO_CALLSPIRITS] = {
		SkillName = "Invocar Esfera Espiritual"
	},
	[SKID.MO_ABSORBSPIRITS] = {
		SkillName = "Absorver Esferas Espirituais"
	},
	[SKID.MO_TRIPLEATTACK] = {
		SkillName = "Combo Triplo"
	},
	[SKID.MO_BODYRELOCATION] = {
		SkillName = "Passo Etéreo"
	},
	[SKID.MO_DODGE] = {
		SkillName = "Cair das Pétalas"
	},
	[SKID.MO_INVESTIGATE] = {
		SkillName = "Impacto Psíquico"
	},
	[SKID.MO_FINGEROFFENSIVE] = {
		SkillName = "Disparo de Esferas Espirituais"
	},
	[SKID.MO_STEELBODY] = {
		SkillName = "Corpo Fechado"
	},
	[SKID.MO_BLADESTOP] = {SkillName = "Dilema"},
	[SKID.MO_EXPLOSIONSPIRITS] = {
		SkillName = "Fúria Interior"
	},
	[SKID.MO_EXTREMITYFIST] = {
		SkillName = "Punho Supremo de Asura"
	},
	[SKID.MO_CHAINCOMBO] = {
		SkillName = "Combo Quádruplo"
	},
	[SKID.MO_COMBOFINISH] = {
		SkillName = "O Último Dragão"
	},
	[SKID.SA_ADVANCEDBOOK] = {
		SkillName = "Estudo de Livros"
	},
	[SKID.SA_CASTCANCEL] = {
		SkillName = "Cancelar magia"
	},
	[SKID.SA_MAGICROD] = {
		SkillName = "Espelho Mágico"
	},
	[SKID.SA_SPELLBREAKER] = {
		SkillName = "Desconcentrar"
	},
	[SKID.SA_FREECAST] = {
		SkillName = "Conjuração Livre"
	},
	[SKID.SA_AUTOSPELL] = {
		SkillName = "Desejo Arcano"
	},
	[SKID.SA_FLAMELAUNCHER] = {
		SkillName = "Encantar com Chama"
	},
	[SKID.SA_FROSTWEAPON] = {
		SkillName = "Encantar com Geada"
	},
	[SKID.SA_LIGHTNINGLOADER] = {
		SkillName = "Encantar com Ventania"
	},
	[SKID.SA_SEISMICWEAPON] = {
		SkillName = "Encantar com Terremoto"
	},
	[SKID.SA_DRAGONOLOGY] = {
		SkillName = "Dragonologia"
	},
	[SKID.SA_VOLCANO] = {SkillName = "Vulcão"},
	[SKID.SA_DELUGE] = {SkillName = "Dilúvio"},
	[SKID.SA_VIOLENTGALE] = {SkillName = "Furacão"},
	[SKID.SA_LANDPROTECTOR] = {
		SkillName = "Proteger Terreno"
	},
	[SKID.SA_DISPELL] = {
		SkillName = "Desencantar"
	},
	[SKID.SA_ABRACADABRA] = {
		SkillName = "Abracadabra"
	},
	[SKID.SA_MONOCELL] = {SkillName = "Monocélula"},
	[SKID.SA_CLASSCHANGE] = {
		SkillName = "Transformação"
	},
	[SKID.SA_SUMMONMONSTER] = {
		SkillName = "Invocar Monstro"
	},
	[SKID.SA_REVERSEORCISH] = {
		SkillName = "Transformação Orc"
	},
	[SKID.SA_DEATH] = {SkillName = "Extermínio"},
	[SKID.SA_FORTUNE] = {
		SkillName = "Toque de Midas"
	},
	[SKID.SA_TAMINGMONSTER] = {SkillName = "Hipnose"},
	[SKID.SA_QUESTION] = {
		SkillName = "Interrogatório"
	},
	[SKID.SA_GRAVITY] = {SkillName = "Gravity"},
	[SKID.SA_LEVELUP] = {
		SkillName = "Subir de Nível"
	},
	[SKID.SA_INSTANTDEATH] = {SkillName = "Suicídio"},
	[SKID.SA_FULLRECOVERY] = {
		SkillName = "Rejuvenescer"
	},
	[SKID.SA_COMA] = {SkillName = "Coma"},
	[SKID.BD_ADAPTATION] = {
		SkillName = "Aquecimento"
	},
	[SKID.BD_ENCORE] = {SkillName = "Bis"},
	[SKID.BD_LULLABY] = {
		SkillName = "Cantiga de Ninar"
	},
	[SKID.BD_RICHMANKIM] = {
		SkillName = "Banquete de Njord"
	},
	[SKID.BD_ETERNALCHAOS] = {
		SkillName = "Ritmo Caótico"
	},
	[SKID.BD_DRUMBATTLEFIELD] = {
		SkillName = "Rufar dos Tambores"
	},
	[SKID.BD_RINGNIBELUNGEN] = {
		SkillName = "Anel dos Nibelungos"
	},
	[SKID.BD_ROKISWEIL] = {
		SkillName = "Lamento de Loki"
	},
	[SKID.BD_INTOABYSS] = {
		SkillName = "Canção Preciosa"
	},
	[SKID.BD_SIEGFRIED] = {
		SkillName = "Ode a Siegfried"
	},
	[SKID.BD_RAGNAROK] = {SkillName = "Ragnarök"},
	[SKID.BA_MUSICALLESSON] = {
		SkillName = "Lições de Música"
	},
	[SKID.BA_MUSICALSTRIKE] = {
		SkillName = "Flecha Melódica"
	},
	[SKID.BA_DISSONANCE] = {
		SkillName = "Dissonância"
	},
	[SKID.BA_FROSTJOKE] = {
		SkillName = "Piada Infame"
	},
	[SKID.BA_WHISTLE] = {SkillName = "Assovio"},
	[SKID.BA_ASSASSINCROSS] = {
		SkillName = "Crepúsculo Sangrento"
	},
	[SKID.BA_POEMBRAGI] = {
		SkillName = "Poema de Bragi"
	},
	[SKID.BA_APPLEIDUN] = {
		SkillName = "Maçãs de Idun"
	},
	[SKID.DC_DANCINGLESSON] = {
		SkillName = "Lições de Dança"
	},
	[SKID.DC_THROWARROW] = {SkillName = "Estilingue"},
	[SKID.DC_UGLYDANCE] = {
		SkillName = "Dança do Ventre"
	},
	[SKID.DC_SCREAM] = {SkillName = "Escândalo"},
	[SKID.DC_HUMMING] = {SkillName = "Sibilo"},
	[SKID.DC_DONTFORGETME] = {
		SkillName = "Não Me Abandones"
	},
	[SKID.DC_FORTUNEKISS] = {
		SkillName = "Beijo da Sorte"
	},
	[SKID.DC_SERVICEFORYOU] = {
		SkillName = "Dança Cigana"
	},
	[SKID.WE_MALE] = {
		SkillName = "Amor Verdadeiro"
	},
	[SKID.WE_FEMALE] = {
		SkillName = "Amor Eterno"
	},
	[SKID.WE_CALLPARTNER] = {
		SkillName = "Saudades de Você"
	},
	[SKID.ITM_TOMAHAWK] = {
		SkillName = "Arremessar Tomahawk"
	},
	[SKID.LK_AURABLADE] = {
		SkillName = "Lâmina de Aura"
	},
	[SKID.LK_PARRYING] = {
		SkillName = "Aparar Golpe"
	},
	[SKID.LK_CONCENTRATION] = {SkillName = "Dedicação"},
	[SKID.LK_TENSIONRELAX] = {SkillName = "Relaxar"},
	[SKID.LK_BERSERK] = {SkillName = "Frenesi"},
	[SKID.HP_ASSUMPTIO] = {SkillName = "Assumptio"},
	[SKID.HP_BASILICA] = {SkillName = "Basílica"},
	[SKID.HP_MEDITATIO] = {SkillName = "Meditatio"},
	[SKID.HW_SOULDRAIN] = {
		SkillName = "Dreno de Alma"
	},
	[SKID.HW_MAGICCRASHER] = {
		SkillName = "Esmagamento Mágico"
	},
	[SKID.HW_MAGICPOWER] = {
		SkillName = "Amplificação Mística"
	},
	[SKID.PA_PRESSURE] = {
		SkillName = "Gloria Domini"
	},
	[SKID.PA_SACRIFICE] = {
		SkillName = "Sacrifício do Mártir"
	},
	[SKID.PA_GOSPEL] = {
		SkillName = "Canto de Batalha"
	},
	[SKID.CH_PALMSTRIKE] = {
		SkillName = "Golpe da Palma em Fúria"
	},
	[SKID.CH_TIGERFIST] = {
		SkillName = "Punho do Tigre"
	},
	[SKID.CH_CHAINCRUSH] = {
		SkillName = "Combo Esmagador"
	},
	[SKID.PF_HPCONVERSION] = {SkillName = "Indulgir"},
	[SKID.PF_SOULCHANGE] = {
		SkillName = "Exalar Alma"
	},
	[SKID.PF_SOULBURN] = {
		SkillName = "Sifão de Alma"
	},
	[SKID.ASC_KATAR] = {
		SkillName = "Perícia com Katar Avançada"
	},
	[SKID.ASC_EDP] = {
		SkillName = "Encantar com Veneno Mortal"
	},
	[SKID.ASC_BREAKER] = {
		SkillName = "Destruidor de Almas"
	},
	[SKID.SN_SIGHT] = {SkillName = "Visão Real"},
	[SKID.SN_FALCONASSAULT] = {
		SkillName = "Assalto do Falcão"
	},
	[SKID.SN_SHARPSHOOTING] = {
		SkillName = "Tiro Preciso"
	},
	[SKID.SN_WINDWALK] = {
		SkillName = "Caminho do Vento"
	},
	[SKID.WS_MELTDOWN] = {
		SkillName = "Golpe Estilhaçante"
	},
	[SKID.WS_CREATECOIN] = {
		SkillName = "Criar Moeda"
	},
	[SKID.WS_CREATENUGGET] = {
		SkillName = "Criar Pepita"
	},
	[SKID.WS_CARTBOOST] = {
		SkillName = "Impulso no Carrinho"
	},
	[SKID.WS_SYSTEMCREATE] = {
		SkillName = "Criar Máquina Automática de Ataque"
	},
	[SKID.ST_CHASEWALK] = {SkillName = "Espreitar"},
	[SKID.ST_REJECTSWORD] = {
		SkillName = "Instinto de Defesa"
	},
	[SKID.ST_STEALBACKPACK] = {
		SkillName = "Roubar Bolso"
	},
	[SKID.CR_ALCHEMY] = {SkillName = "Alquimia"},
	[SKID.CR_SYNTHESISPOTION] = {
		SkillName = "Síntese de Poção"
	},
	[SKID.CG_ARROWVULCAN] = {
		SkillName = "Vulcão de Flechas"
	},
	[SKID.CG_MOONLIT] = {
		SkillName = "Bênção Protetora"
	},
	[SKID.CG_MARIONETTE] = {
		SkillName = "Controle de Marionete"
	},
	[SKID.LK_SPIRALPIERCE] = {
		SkillName = "Perfurar em Espiral"
	},
	[SKID.LK_HEADCRUSH] = {
		SkillName = "Golpe Traumático"
	},
	[SKID.LK_JOINTBEAT] = {
		SkillName = "Ataque Vital"
	},
	[SKID.HW_NAPALMVULCAN] = {
		SkillName = "Vulcão Napalm"
	},
	[SKID.CH_SOULCOLLECT] = {SkillName = "Zen"},
	[SKID.PF_MINDBREAKER] = {
		SkillName = "Enlouquecedor"
	},
	[SKID.PF_MEMORIZE] = {
		SkillName = "Presciência"
	},
	[SKID.PF_FOGWALL] = {
		SkillName = "Bruma Ofuscante"
	},
	[SKID.PF_SPIDERWEB] = {
		SkillName = "Prisão de Teia"
	},
	[SKID.ASC_METEORASSAULT] = {
		SkillName = "Impacto Meteoro"
	},
	[SKID.ASC_CDP] = {
		SkillName = "Criar Veneno Mortal"
	},
	[SKID.WE_BABY] = {
		SkillName = "Mãe, Pai, amo vocês!"
	},
	[SKID.WE_CALLPARENT] = {
		SkillName = "Mãe, Pai, cadê vocês?"
	},
	[SKID.WE_CALLBABY] = {
		SkillName = "Vem cá, filhote!"
	},
	[SKID.TK_RUN] = {SkillName = "Corrida"},
	[SKID.TK_READYSTORM] = {
		SkillName = "Postura do Tornado"
	},
	[SKID.TK_STORMKICK] = {
		SkillName = "Chute do Tornado"
	},
	[SKID.TK_READYDOWN] = {
		SkillName = "Postura da Patada Voadora"
	},
	[SKID.TK_DOWNKICK] = {
		SkillName = "Patada Voadora"
	},
	[SKID.TK_READYTURN] = {
		SkillName = "Postura da Rasteira"
	},
	[SKID.TK_TURNKICK] = {SkillName = "Rasteira"},
	[SKID.TK_READYCOUNTER] = {
		SkillName = "Postura do Contrachute"
	},
	[SKID.TK_COUNTER] = {
		SkillName = "Contrachute"
	},
	[SKID.TK_DODGE] = {SkillName = "Cambalhota"},
	[SKID.TK_JUMPKICK] = {
		SkillName = "Chute Aéreo"
	},
	[SKID.TK_HPTIME] = {
		SkillName = "Trégua Rápida"
	},
	[SKID.TK_SPTIME] = {
		SkillName = "Retiro Rápido"
	},
	[SKID.TK_POWER] = {SkillName = "Kihop"},
	[SKID.TK_SEVENWIND] = {SkillName = "Brisa Leve"},
	[SKID.TK_HIGHJUMP] = {SkillName = "Salto"},
	[SKID.SG_FEEL] = {
		SkillName = "Percepção Solar, Lunar e Estelar"
	},
	[SKID.SG_SUN_WARM] = {
		SkillName = "Calor Solar"
	},
	[SKID.SG_MOON_WARM] = {
		SkillName = "Calor Lunar"
	},
	[SKID.SG_STAR_WARM] = {
		SkillName = "Calor Estelar"
	},
	[SKID.SG_SUN_COMFORT] = {
		SkillName = "Proteção Solar"
	},
	[SKID.SG_MOON_COMFORT] = {
		SkillName = "Proteção Lunar"
	},
	[SKID.SG_STAR_COMFORT] = {
		SkillName = "Proteção Estelar"
	},
	[SKID.SG_HATE] = {
		SkillName = "Oposição Solar, Lunar e Estelar"
	},
	[SKID.SG_SUN_ANGER] = {
		SkillName = "Fúria Solar"
	},
	[SKID.SG_MOON_ANGER] = {
		SkillName = "Fúria Lunar"
	},
	[SKID.SG_STAR_ANGER] = {
		SkillName = "Fúria Estelar"
	},
	[SKID.SG_SUN_BLESS] = {
		SkillName = "Bênção Solar"
	},
	[SKID.SG_MOON_BLESS] = {
		SkillName = "Bênção Lunar"
	},
	[SKID.SG_STAR_BLESS] = {
		SkillName = "Bênção Estelar"
	},
	[SKID.SG_DEVIL] = {
		SkillName = "Sombra Solar, Lunar e Estelar"
	},
	[SKID.SG_FRIEND] = {
		SkillName = "Auxílio Solar, Lunar e Estelar"
	},
	[SKID.SG_KNOWLEDGE] = {
		SkillName = "Transmissão Solar, Lunar e Estelar"
	},
	[SKID.SG_FUSION] = {
		SkillName = "União Solar, Lunar e Estelar"
	},
	[SKID.SL_ALCHEMIST] = {
		SkillName = "Espírito do Alquimista"
	},
	[SKID.AM_BERSERKPITCHER] = {
		SkillName = "Arremessar Poção da Fúria Selvagem"
	},
	[SKID.SL_MONK] = {
		SkillName = "Espírito do Monge"
	},
	[SKID.SL_STAR] = {
		SkillName = "Espírito do Mestre Taekwon"
	},
	[SKID.SL_SAGE] = {
		SkillName = "Espírito do Sábio"
	},
	[SKID.SL_CRUSADER] = {
		SkillName = "Espírito do Templário"
	},
	[SKID.SL_SUPERNOVICE] = {
		SkillName = "Espírito do Superaprendiz"
	},
	[SKID.SL_KNIGHT] = {
		SkillName = "Espírito do Cavaleiro"
	},
	[SKID.SL_WIZARD] = {
		SkillName = "Espírito do Bruxo"
	},
	[SKID.SL_PRIEST] = {
		SkillName = "Espírito do Sacerdote"
	},
	[SKID.SL_BARDDANCER] = {
		SkillName = "Espírito dos Artistas"
	},
	[SKID.SL_ROGUE] = {
		SkillName = "Espírito do Arruaceiro"
	},
	[SKID.SL_ASSASIN] = {
		SkillName = "Espírito do Mercenário"
	},
	[SKID.SL_BLACKSMITH] = {
		SkillName = "Espírito do Ferreiro"
	},
	[SKID.BS_ADRENALINE2] = {
		SkillName = "Adrenalina Concentrada"
	},
	[SKID.SL_HUNTER] = {
		SkillName = "Espírito do Caçador"
	},
	[SKID.SL_SOULLINKER] = {
		SkillName = "Espírito do Espiritualista"
	},
	[SKID.SL_KAIZEL] = {SkillName = "Kaizel"},
	[SKID.SL_KAAHI] = {SkillName = "Kaahi"},
	[SKID.SL_KAUPE] = {SkillName = "Kaupe"},
	[SKID.SL_KAITE] = {SkillName = "Kaite"},
	[SKID.SL_KAINA] = {SkillName = "Kaina"},
	[SKID.SL_STIN] = {SkillName = "Estin"},
	[SKID.SL_STUN] = {SkillName = "Estun"},
	[SKID.SL_SMA] = {SkillName = "Esma"},
	[SKID.SL_SWOO] = {SkillName = "Eswoo"},
	[SKID.SL_SKE] = {SkillName = "Eske"},
	[SKID.SL_SKA] = {SkillName = "Eska"},
	[SKID.ST_PRESERVE] = {SkillName = "Preservar"},
	[SKID.ST_FULLSTRIP] = {
		SkillName = "Remoção Total"
	},
	[SKID.WS_WEAPONREFINE] = {
		SkillName = "Aprimorar Armamento"
	},
	[SKID.CR_SLIMPITCHER] = {
		SkillName = "Arremessar Poção Compacta"
	},
	[SKID.CR_FULLPROTECTION] = {
		SkillName = "Proteção Química Total"
	},
	[SKID.PA_SHIELDCHAIN] = {
		SkillName = "Choque Rápido"
	},
	[SKID.HP_MANARECHARGE] = {
		SkillName = "Riqueza Espírito"
	},
	[SKID.PF_DOUBLECASTING] = {
		SkillName = "Lanças Duplas"
	},
	[SKID.HW_GANBANTEIN] = {SkillName = "Ganbantein"},
	[SKID.HW_GRAVITATION] = {
		SkillName = "Campo Gravitacional"
	},
	[SKID.WS_CARTTERMINATION] = {
		SkillName = "Choque de Carrinho"
	},
	[SKID.WS_OVERTHRUSTMAX] = {
		SkillName = "Força Violentíssima"
	},
	[SKID.CG_LONGINGFREEDOM] = {
		SkillName = "Grito de Liberdade"
	},
	[SKID.CG_HERMODE] = {
		SkillName = "Bastão de Hermod"
	},
	[SKID.CG_TAROTCARD] = {
		SkillName = "Destino nas Cartas"
	},
	[SKID.CR_ACIDDEMONSTRATION] = {
		SkillName = "Bomba Ácida"
	},
	[SKID.CR_CULTIVATION] = {
		SkillName = "Cultivar Planta"
	},
	[SKID.TK_MISSION] = {
		SkillName = "Missão Taekwon"
	},
	[SKID.SL_HIGH] = {
		SkillName = "Espírito dos Transcendentais"
	},
	[SKID.KN_ONEHAND] = {
		SkillName = "Rapidez com Uma Mão"
	},
	[SKID.AM_TWILIGHT1] = {
		SkillName = "Criação Espiritual de Poções I"
	},
	[SKID.AM_TWILIGHT2] = {
		SkillName = "Criação Espiritual de Poções II"
	},
	[SKID.AM_TWILIGHT3] = {
		SkillName = "Criação Espiritual de Poções III"
	},
	[SKID.HT_POWER] = {
		SkillName = "Ataque da Fera"
	},
	[SKID.GS_GLITTERING] = {
		SkillName = "Cara ou Coroa"
	},
	[SKID.GS_FLING] = {
		SkillName = "Atirar Moedas"
	},
	[SKID.GS_TRIPLEACTION] = {
		SkillName = "Ataque Triplo"
	},
	[SKID.GS_BULLSEYE] = {
		SkillName = "Ataque Certeiro"
	},
	[SKID.GS_MADNESSCANCEL] = {
		SkillName = "Resistência Final"
	},
	[SKID.GS_ADJUSTMENT] = {
		SkillName = "Pânico do Justiceiro"
	},
	[SKID.GS_INCREASING] = {
		SkillName = "Aumentar Precisão"
	},
	[SKID.GS_MAGICALBULLET] = {
		SkillName = "Bala Mágica"
	},
	[SKID.GS_CRACKER] = {
		SkillName = "Tiro Bombinha"
	},
	[SKID.GS_SINGLEACTION] = {
		SkillName = "Ataque Concentrado"
	},
	[SKID.GS_SNAKEEYE] = {
		SkillName = "Olhos de Serpente"
	},
	[SKID.GS_CHAINACTION] = {
		SkillName = "Reação em Cadeia"
	},
	[SKID.GS_TRACKING] = {
		SkillName = "Rastrear o Alvo"
	},
	[SKID.GS_DISARM] = {SkillName = "Desarmar"},
	[SKID.GS_PIERCINGSHOT] = {SkillName = "Ferir Alvo"},
	[SKID.GS_RAPIDSHOWER] = {
		SkillName = "Rajada Certeira"
	},
	[SKID.GS_DESPERADO] = {SkillName = "Desperado"},
	[SKID.GS_GATLINGFEVER] = {
		SkillName = "Ataque Gatling"
	},
	[SKID.GS_DUST] = {
		SkillName = "Controle de Multidão"
	},
	[SKID.GS_FULLBUSTER] = {
		SkillName = "Ataque Total"
	},
	[SKID.GS_SPREADATTACK] = {
		SkillName = "Disparo Espalhado"
	},
	[SKID.GS_GROUNDDRIFT] = {
		SkillName = "Mina do Justiceiro"
	},
	[SKID.NJ_TOBIDOUGU] = {
		SkillName = "Perícia com Shuriken"
	},
	[SKID.NJ_SYURIKEN] = {
		SkillName = "Arremessar Shuriken"
	},
	[SKID.NJ_KUNAI] = {
		SkillName = "Arremessar Kunai"
	},
	[SKID.NJ_HUUMA] = {
		SkillName = "Arremessar Huuma"
	},
	[SKID.NJ_ZENYNAGE] = {
		SkillName = "Chuva de Moedas"
	},
	[SKID.NJ_TATAMIGAESHI] = {
		SkillName = "Virar Tatami"
	},
	[SKID.NJ_KASUMIKIRI] = {
		SkillName = "Corte da Névoa"
	},
	[SKID.NJ_SHADOWJUMP] = {
		SkillName = "Salto das Sombras"
	},
	[SKID.NJ_KIRIKAGE] = {
		SkillName = "Corte das Sombras"
	},
	[SKID.NJ_UTSUSEMI] = {
		SkillName = "Troca de Pele"
	},
	[SKID.NJ_BUNSINJYUTSU] = {
		SkillName = "Imagem Falsa"
	},
	[SKID.NJ_NINPOU] = {
		SkillName = "Perícia Ninja"
	},
	[SKID.NJ_KOUENKA] = {
		SkillName = "Pétalas Flamejantes"
	},
	[SKID.NJ_KAENSIN] = {
		SkillName = "Escudo de Chamas"
	},
	[SKID.NJ_BAKUENRYU] = {
		SkillName = "Dragão Explosivo"
	},
	[SKID.NJ_HYOUSENSOU] = {
		SkillName = "Lança Congelante"
	},
	[SKID.NJ_SUITON] = {
		SkillName = "Evasão Aquática"
	},
	[SKID.NJ_HYOUSYOURAKU] = {
		SkillName = "Grande Floco de Neve"
	},
	[SKID.NJ_HUUJIN] = {
		SkillName = "Lâmina de Vento"
	},
	[SKID.NJ_RAIGEKISAI] = {
		SkillName = "Descarga Elétrica"
	},
	[SKID.NJ_KAMAITACHI] = {
		SkillName = "Brisa Cortante"
	},
	[SKID.NJ_NEN] = {SkillName = "Aura Ninja"},
	[SKID.NJ_ISSEN] = {
		SkillName = "Ataque Mortal"
	},
	[SKID.MB_FIGHTING] = {SkillName = "Luta Munak"},
	[SKID.MB_NEUTRAL] = {
		SkillName = "Bongun Neutro"
	},
	[SKID.MB_TAIMING_PUTI] = {
		SkillName = "Bichinho de Estimação"
	},
	[SKID.MB_WHITEPOTION] = {
		SkillName = "Entrega de Poção Branca"
	},
	[SKID.MB_MENTAL] = {
		SkillName = "Entrega Mental"
	},
	[SKID.MB_CARDPITCHER] = {
		SkillName = "Lançar Carta"
	},
	[SKID.MB_PETPITCHER] = {
		SkillName = "Lançar Bichinho"
	},
	[SKID.MB_BODYSTUDY] = {SkillName = "Anatomia"},
	[SKID.MB_BODYALTER] = {
		SkillName = "Fisicultura"
	},
	[SKID.MB_PETMEMORY] = {
		SkillName = "Lembrança do Bichinho"
	},
	[SKID.MB_M_TELEPORT] = {
		SkillName = "Teleporte Munak"
	},
	[SKID.MB_B_GAIN] = {
		SkillName = "Recompensa Bongun"
	},
	[SKID.MB_M_GAIN] = {
		SkillName = "Recompensa Munak"
	},
	[SKID.MB_MISSION] = {
		SkillName = "Missão de Adestramento"
	},
	[SKID.MB_MUNAKKNOWLEDGE] = {
		SkillName = "Perícia em Adestramento"
	},
	[SKID.MB_MUNAKBALL] = {
		SkillName = "Bola da Munak"
	},
	[SKID.MB_SCROLL] = {
		SkillName = "Pergaminho do Bongun"
	},
	[SKID.MB_B_GATHERING] = {
		SkillName = "Reunião de Bongun"
	},
	[SKID.MB_M_GATHERING] = {
		SkillName = "Reunião de Munak"
	},
	[SKID.MB_B_EXCLUDE] = {
		SkillName = "Excluir Bongun"
	},
	[SKID.MB_B_DRIFT] = {
		SkillName = "Manobra Bongun"
	},
	[SKID.MB_B_WALLRUSH] = {
		SkillName = "Bongun Corredor"
	},
	[SKID.MB_M_WALLRUSH] = {
		SkillName = "Munak Corredora"
	},
	[SKID.MB_B_WALLSHIFT] = {
		SkillName = "Parede Bongun"
	},
	[SKID.MB_M_WALLCRASH] = {
		SkillName = "Parede Munak"
	},
	[SKID.MB_M_REINCARNATION] = {
		SkillName = "Ressuscitar Munak"
	},
	[SKID.MB_B_EQUIP] = {
		SkillName = "Equipar Bongun"
	},
	[SKID.SL_DEATHKNIGHT] = {
		SkillName = "Espírito do Cavaleiro da Morte"
	},
	[SKID.SL_COLLECTOR] = {
		SkillName = "Espírito do Coletor Sombrio"
	},
	[SKID.SL_NINJA] = {
		SkillName = "Espírito do Ninja"
	},
	[SKID.SL_GUNNER] = {
		SkillName = "Espírito do Justiceiro"
	},
	[SKID.AM_TWILIGHT4] = {
		SkillName = "Magia do Crepúsculo"
	},
	[SKID.DE_BERSERKAIZER] = {SkillName = "Frenético"},
	[SKID.DA_DARKPOWER] = {
		SkillName = "Espírito Negro"
	},
	[SKID.DE_PASSIVE] = {
		SkillName = "Morte Lenta"
	},
	[SKID.DE_PATTACK] = {
		SkillName = "Ataque da Morte"
	},
	[SKID.DE_PSPEED] = {
		SkillName = "Morte Rápida"
	},
	[SKID.DE_PDEFENSE] = {
		SkillName = "Defesa Mortal"
	},
	[SKID.DE_PCRITICAL] = {
		SkillName = "Morte Crítica"
	},
	[SKID.DE_PHP] = {
		SkillName = "Recuperação pela Morte"
	},
	[SKID.DE_PSP] = {
		SkillName = "Magia Mortal"
	},
	[SKID.DE_RESET] = {
		SkillName = "Otimização da Morte"
	},
	[SKID.DE_RANKING] = {
		SkillName = "Ranking da Morte"
	},
	[SKID.DE_PTRIPLE] = {
		SkillName = "Morte Tripla"
	},
	[SKID.DE_ENERGY] = {
		SkillName = "Energia da Morte"
	},
	[SKID.DE_NIGHTMARE] = {
		SkillName = "Sonhar com Pesadelo"
	},
	[SKID.DE_SLASH] = {
		SkillName = "Corte Mortal"
	},
	[SKID.DE_COIL] = {
		SkillName = "Moeda Mortal"
	},
	[SKID.DE_WAVE] = {
		SkillName = "Onda da Morte"
	},
	[SKID.DE_REBIRTH] = {
		SkillName = "Energia Reversa"
	},
	[SKID.DE_AURA] = {
		SkillName = "Aura Maligna"
	},
	[SKID.DE_FREEZER] = {
		SkillName = "Congelamento Mórbido"
	},
	[SKID.DE_CHANGEATTACK] = {
		SkillName = "Troca de Ataque da Morte"
	},
	[SKID.DE_PUNISH] = {
		SkillName = "Punição pela Morte"
	},
	[SKID.DE_POISON] = {
		SkillName = "Envenenamento Moral"
	},
	[SKID.DE_INSTANT] = {
		SkillName = "Barreira Mortal"
	},
	[SKID.DE_WARNING] = {
		SkillName = "Juramento de Morte"
	},
	[SKID.DE_RANKEDKNIFE] = {
		SkillName = "Adaga Mortífera"
	},
	[SKID.DE_RANKEDGRADIUS] = {
		SkillName = "Gladius Mortífera"
	},
	[SKID.DE_GAUGE] = {SkillName = "Indicação"},
	[SKID.DE_GTIME] = {
		SkillName = "Troca do Tempo"
	},
	[SKID.DE_GPAIN] = {
		SkillName = "Troca de Dor"
	},
	[SKID.DE_GSKILL] = {
		SkillName = "Troca de Habilidade"
	},
	[SKID.DE_GKILL] = {
		SkillName = "Troca de Morte"
	},
	[SKID.DE_ACCEL] = {
		SkillName = "Aceleração Mortal"
	},
	[SKID.DE_BLOCKDOUBLE] = {
		SkillName = "Bloqueio Duplo"
	},
	[SKID.DE_BLOCKMELEE] = {
		SkillName = "Bloqueio Próximo"
	},
	[SKID.DE_BLOCKFAR] = {
		SkillName = "Bloqueio Distante"
	},
	[SKID.DE_FRONTATTACK] = {
		SkillName = "Ataque Rápido Mortal"
	},
	[SKID.DE_DANGERATTACK] = {
		SkillName = "Ataque Perigoso"
	},
	[SKID.DE_TWINATTACK] = {
		SkillName = "Ataque Gêmeo"
	},
	[SKID.DE_WINDATTACK] = {
		SkillName = "Ataque Tempestuoso"
	},
	[SKID.DE_WATERATTACK] = {
		SkillName = "Ataque de Água"
	},
	[SKID.DA_ENERGY] = {
		SkillName = "Energia Sombria"
	},
	[SKID.DA_CLOUD] = {
		SkillName = "Nuvem Sombria"
	},
	[SKID.DA_FIRSTSLOT] = {
		SkillName = "Fantasia Sombria"
	},
	[SKID.DA_HEADDEF] = {
		SkillName = "Cabeçada Sombria"
	},
	[SKID.DA_SPACE] = {
		SkillName = "Espaço Sombrio"
	},
	[SKID.DA_TRANSFORM] = {
		SkillName = "Transformação Sombria"
	},
	[SKID.DA_EXPLOSION] = {
		SkillName = "Explosão Sombria"
	},
	[SKID.DA_REWARD] = {
		SkillName = "Recompensa Sombria"
	},
	[SKID.DA_CRUSH] = {
		SkillName = "Esmagar Sombriamente"
	},
	[SKID.DA_ITEMREBUILD] = {
		SkillName = "Reconstruir Item"
	},
	[SKID.DA_ILLUSION] = {
		SkillName = "Ilusão Sombria"
	},
	[SKID.DA_NUETRALIZE] = {
		SkillName = "Neutralização Sombria"
	},
	[SKID.DA_RUNNER] = {
		SkillName = "Corredor Sombrio"
	},
	[SKID.DA_TRANSFER] = {
		SkillName = "Transferência Sombria"
	},
	[SKID.DA_WALL] = {
		SkillName = "Parede Sombria"
	},
	[SKID.DA_REVENGE] = {
		SkillName = "Vigança Sombria"
	},
	[SKID.DA_EARPLUG] = {
		SkillName = "Orelha Sombria"
	},
	[SKID.DA_CONTRACT] = {
		SkillName = "Contrato de Gema Sombria"
	},
	[SKID.DA_BLACK] = {
		SkillName = "Magia de Gema Sombria"
	},
	[SKID.DA_DREAM] = {
		SkillName = "Sonho de Gema Sombria"
	},
	[SKID.DA_MAGICCART] = {
		SkillName = "Carrinho Mágico"
	},
	[SKID.DA_COPY] = {SkillName = "Copiar"},
	[SKID.DA_CRYSTAL] = {SkillName = "Cristal"},
	[SKID.DA_EXP] = {
		SkillName = "Experiência"
	},
	[SKID.DA_CARTSWING] = {
		SkillName = "Balanceio de Carrinho"
	},
	[SKID.DA_REBUILD] = {
		SkillName = "Reconstrução Humana"
	},
	[SKID.DA_JOBCHANGE] = {
		SkillName = "Mudança de Classe"
	},
	[SKID.DA_EDARKNESS] = {
		SkillName = "Emperium Sombrio"
	},
	[SKID.DA_EGUARDIAN] = {
		SkillName = "Guardião do Emperium"
	},
	[SKID.DA_TIMEOUT] = {
		SkillName = "Tempo Esgotado"
	},
	[SKID.ALL_TIMEIN] = {
		SkillName = "Tempo Iniciado"
	},
	[SKID.DA_ZENYRANK] = {
		SkillName = "Ranking de Zeny"
	},
	[SKID.DA_ACCESSORYMIX] = {
		SkillName = "Combinar Acessórios"
	},
	[SKID.NPC_EARTHQUAKE] = {SkillName = "Terremoto"},
	[SKID.NPC_DRAGONFEAR] = {
		SkillName = "Terror Draconiano"
	},
	[SKID.NPC_PULSESTRIKE] = {
		SkillName = "Golpe Avassalador"
	},
	[SKID.NPC_HELLJUDGEMENT] = {
		SkillName = "Julgamento Infernal"
	},
	[SKID.NPC_WIDESILENCE] = {
		SkillName = "Som do Silêncio"
	},
	[SKID.NPC_WIDEFREEZE] = {
		SkillName = "Beijo do Inverno"
	},
	[SKID.NPC_WIDEBLEEDING] = {SkillName = "Sangria"},
	[SKID.NPC_WIDESTONE] = {
		SkillName = "Olhar da Medusa"
	},
	[SKID.NPC_WIDECONFUSE] = {
		SkillName = "Toque de Insanidade"
	},
	[SKID.NPC_WIDESLEEP] = {
		SkillName = "Sussurro de Morfeu"
	},
	[SKID.NPC_WIDESIGHT] = {SkillName = "Visão"},
	[SKID.NPC_EVILLAND] = {
		SkillName = "Terra Amaldiçoada"
	},
	[SKID.NPC_MAGICMIRROR] = {
		SkillName = "Reflexão Mágica"
	},
	[SKID.NPC_SLOWCAST] = {
		SkillName = "Conjuração Lenta"
	},
	[SKID.NPC_CRITICALWOUND] = {
		SkillName = "Ferimento Mortal"
	},
	[SKID.NPC_STONESKIN] = {
		SkillName = "Pele Rochosa"
	},
	[SKID.NPC_ANTIMAGIC] = {SkillName = "Antimagia"},
	[SKID.NPC_WIDECURSE] = {
		SkillName = "Espírito dos Amaldiçoados"
	},
	[SKID.NPC_WIDESTUN] = {
		SkillName = "Atordoamento Titânico"
	},
	[SKID.NPC_VAMPIRE_GIFT] = {
		SkillName = "Beijo do Vampiro"
	},
	[SKID.NPC_WIDESOULDRAIN] = {SkillName = "Drenar SP"},
	[SKID.ALL_INCCARRY] = {
		SkillName = "Aumentar Capacidade de Carga Especial"
	},
	[SKID.NPC_HELLPOWER] = {
		SkillName = "Poder Infernal"
	},
	[SKID.NPC_ALLHEAL] = {
		SkillName = "Corrente da Vida"
	},
	[SKID.GM_SANDMAN] = {
		SkillName = "Durma bem, minha criança"
	},
	[SKID.ALL_CATCRY] = {
		SkillName = "Rugido da Fera"
	},
	[SKID.ALL_PARTYFLEE] = {
		SkillName = "Vento Uivante"
	},
	[SKID.ALL_ANGEL_PROTECT] = {
		SkillName = "Muito Obrigado"
	},
	[SKID.ALL_DREAM_SUMMERNIGHT] = {
		SkillName = "Sonho de Verão"
	},
	[SKID.ALL_REVERSEORCISH] = {
		SkillName = "Transformação Orc"
	},
	[SKID.ALL_WEWISH] = {
		SkillName = "Noite Feliz"
	},
	[SKID.NPC_WIDEHEALTHFEAR] = {
		SkillName = "Grito Ameaçador"
	},
	[SKID.NPC_WIDEBODYBURNNING] = {SkillName = "Combustão"},
	[SKID.NPC_WIDEFROSTMISTY] = {SkillName = "Avalanche"},
	[SKID.NPC_WIDECOLD] = {
		SkillName = "Grande Geada"
	},
	[SKID.NPC_WIDE_DEEP_SLEEP] = {
		SkillName = "Magia Malévola"
	},
	[SKID.NPC_WIDESIREN] = {
		SkillName = "Flecha do Cupido"
	},
	[SKID.NPC_VENOMFOG] = {
		SkillName = "Neblina de Veneno"
	},
	[SKID.NPC_MILLENNIUMSHIELD] = {
		SkillName = "Escudos Milenares"
	},
	[SKID.NPC_COMET] = {SkillName = "Cometa"},
	[SKID.NPC_ICEMINE] = {
		SkillName = "Mina de Gelo"
	},
	[SKID.NPC_FLAMECROSS] = {
		SkillName = "Cruz de Chamas"
	},
	[SKID.NPC_PULSESTRIKE2] = {
		SkillName = "Golpe Avassalador"
	},
	[SKID.NPC_MAXPAIN] = {SkillName = "Dor Máxima"},
	[SKID.NPC_JACKFROST] = {
		SkillName = "Esquife de Gelo"
	},
	[SKID.NPC_WIDEWEB] = {
		SkillName = "Campo de Teia"
	},
	[SKID.NPC_WIDESUCK] = {
		SkillName = "Usurpar Almas"
	},
	[SKID.NPC_STORMGUST2] = {SkillName = "Nevasca"},
	[SKID.NPC_FIRESTORM] = {
		SkillName = "Tempestade de Fogo"
	},
	[SKID.NPC_REVERBERATION] = {
		SkillName = "Ressonância"
	},
	[SKID.NPC_LEX_AETERNA] = {SkillName = "Lex Regio"},
	[SKID.NPC_ARROWSTORM] = {
		SkillName = "Tempestade de Flechas"
	},
	[SKID.NPC_SR_CURSEDCIRCLE] = {
		SkillName = "Campo Amaldiçoado"
	},
	[SKID.NPC_DRAGONBREATH] = {
		SkillName = "Sopro do Dragão"
	},
	[SKID.NPC_FATALMENACE] = {
		SkillName = "Ofensiva Fatal"
	},
	[SKID.NPC_MAGMA_ERUPTION] = {
		SkillName = "Erupção de Magma"
	},
	[SKID.NPC_MANDRAGORA] = {
		SkillName = "Grito da Mandrágora"
	},
	[SKID.NPC_PSYCHIC_WAVE] = {
		SkillName = "Onda Psíquica"
	},
	[SKID.NPC_RAYOFGENESIS] = {
		SkillName = "Luz da Criação"
	},
	[SKID.NPC_CLOUD_KILL] = {
		SkillName = "Maldição de Jormungand"
	},
	[SKID.NPC_IGNITIONBREAK] = {
		SkillName = "Impacto Flamejante"
	},
	[SKID.NPC_HALLUCINATIONWALK] = {
		SkillName = "Passos da Ilusão"
	},
	[SKID.NPC_ELECTRICWALK] = {
		SkillName = "Passos de Sílfide"
	},
	[SKID.NPC_FIREWALK] = {
		SkillName = "Passos de Salamandra"
	},
	[SKID.NPC_LEASH] = {SkillName = "Puxar"},
	[SKID.NPC_WIDELEASH] = {SkillName = "Puxão"},
	[SKID.NPC_WIDECRITICALWOUND] = {
		SkillName = "Ferida Mortal"
	},
	[SKID.NPC_EARTHQUAKE_K] = {SkillName = "Terremoto"},
	[SKID.NPC_IMMUNE_PROPERTY] = {
		SkillName = "Imunidade Elemental"
	},
	[SKID.NPC_WIDEBLEEDING2] = {SkillName = "Sangria"},
	[SKID.NPC_WIDESILENCE2] = {
		SkillName = "Som do Silêncio"
	},
	[SKID.NPC_WIDESTUN2] = {
		SkillName = "Atordoamento Titânico"
	},
	[SKID.NPC_WIDESTONE2] = {
		SkillName = "Olhar da Medusa"
	},
	[SKID.NPC_WIDESLEEP2] = {
		SkillName = "Sussurro de Morfeu"
	},
	[SKID.NPC_WIDECURSE2] = {
		SkillName = "Espírito dos Amaldiçoados"
	},
	[SKID.NPC_WIDECONFUSE2] = {
		SkillName = "Toque de Insanidade"
	},
	[SKID.NPC_WIDEFREEZE2] = {
		SkillName = "Beijo do Inverno"
	},
	[SKID.NPC_EVILLAND2] = {
		SkillName = "Terra Amaldiçoada"
	},
	[SKID.NPC_HELLJUDGEMENT2] = {
		SkillName = "Julgamento Infernal"
	},
	[SKID.NPC_RAINOFMETEOR] = {
		SkillName = "RAIN OF METEOR"
	},
	[SKID.NPC_GROUNDDRIVE] = {
		SkillName = "GROUND DRIVE"
	},
	[SKID.NPC_DEADLYCURSE2] = {
		SkillName = "Praga Mortal"
	},
	[SKID.KN_CHARGEATK] = {
		SkillName = "Avanço Ofensivo"
	},
	[SKID.CR_SHRINK] = {SkillName = "Submissão"},
	[SKID.AS_SONICACCEL] = {
		SkillName = "Lâminas Aceleradas"
	},
	[SKID.AS_VENOMKNIFE] = {
		SkillName = "Faca Envenenada"
	},
	[SKID.RG_CLOSECONFINE] = {
		SkillName = "Confinamento"
	},
	[SKID.WZ_SIGHTBLASTER] = {
		SkillName = "Explosão Protetora"
	},
	[SKID.SA_CREATECON] = {
		SkillName = "Criar Conversor Elemental"
	},
	[SKID.SA_ELEMENTWATER] = {
		SkillName = "Mudança Elemental - Água"
	},
	[SKID.HT_PHANTASMIC] = {
		SkillName = "Flecha Ilusória"
	},
	[SKID.BA_PANGVOICE] = {
		SkillName = "Voz Dolorosa"
	},
	[SKID.DC_WINKCHARM] = {SkillName = "Piscadela"},
	[SKID.BS_UNFAIRLYTRICK] = {
		SkillName = "Venda Duvidosa"
	},
	[SKID.BS_GREED] = {SkillName = "Ganância"},
	[SKID.PR_REDEMPTIO] = {SkillName = "Martírio"},
	[SKID.MO_KITRANSLATION] = {
		SkillName = "Concessão Espiritual"
	},
	[SKID.MO_BALKYOUNG] = {
		SkillName = "Punhos Intensos"
	},
	[SKID.SA_ELEMENTGROUND] = {
		SkillName = "Mudança Elemental - Terra"
	},
	[SKID.SA_ELEMENTFIRE] = {
		SkillName = "Mudança Elemental - Fogo"
	},
	[SKID.SA_ELEMENTWIND] = {
		SkillName = "Mudança Elemental - Vento"
	},
	[SKID.RK_ENCHANTBLADE] = {
		SkillName = "Encantar Lâmina"
	},
	[SKID.RK_SONICWAVE] = {
		SkillName = "Onda de Choque"
	},
	[SKID.RK_DEATHBOUND] = {
		SkillName = "Revidar Dano"
	},
	[SKID.RK_HUNDREDSPEAR] = {
		SkillName = "Lança das Mil Pontas"
	},
	[SKID.RK_WINDCUTTER] = {
		SkillName = "Vento Cortante"
	},
	[SKID.RK_IGNITIONBREAK] = {
		SkillName = "Impacto Flamejante"
	},
	[SKID.RK_DRAGONTRAINING] = {
		SkillName = "Adestrar Dragão"
	},
	[SKID.RK_DRAGONBREATH] = {
		SkillName = "Sopro do Dragão"
	},
	[SKID.RK_DRAGONHOWLING] = {
		SkillName = "Rugido do Dragão"
	},
	[SKID.RK_RUNEMASTERY] = {
		SkillName = "Perícia em Runas"
	},
	[SKID.RK_MILLENNIUMSHIELD] = {
		SkillName = "Escudos Milenares"
	},
	[SKID.RK_CRUSHSTRIKE] = {
		SkillName = "Golpe Titânico"
	},
	[SKID.RK_REFRESH] = {
		SkillName = "Purificação"
	},
	[SKID.RK_GIANTGROWTH] = {
		SkillName = "Força Titânica"
	},
	[SKID.RK_STONEHARDSKIN] = {
		SkillName = "Escamas Rochosas"
	},
	[SKID.RK_VITALITYACTIVATION] = {
		SkillName = "Vitalidade Rúnica"
	},
	[SKID.RK_STORMBLAST] = {
		SkillName = "Explosão Rúnica"
	},
	[SKID.RK_FIGHTINGSPIRIT] = {
		SkillName = "Aura de Combate"
	},
	[SKID.RK_ABUNDANCE] = {
		SkillName = "Regeneração Espiritual"
	},
	[SKID.RK_PHANTOMTHRUST] = {SkillName = "Arpão"},
	[SKID.GC_VENOMIMPRESS] = {
		SkillName = "Potencializar Veneno"
	},
	[SKID.GC_CROSSIMPACT] = {
		SkillName = "Lâminas Retalhadoras"
	},
	[SKID.GC_DARKILLUSION] = {
		SkillName = "Passo Sombrio"
	},
	[SKID.GC_RESEARCHNEWPOISON] = {
		SkillName = "Pesquisa de Toxinas"
	},
	[SKID.GC_CREATENEWPOISON] = {
		SkillName = "Criar Toxina"
	},
	[SKID.GC_ANTIDOTE] = {SkillName = "Antídoto"},
	[SKID.GC_POISONINGWEAPON] = {
		SkillName = "Aplicar Toxina"
	},
	[SKID.GC_WEAPONBLOCKING] = {
		SkillName = "Reflexo de Combate"
	},
	[SKID.GC_COUNTERSLASH] = {SkillName = "Retaliação"},
	[SKID.GC_WEAPONCRUSH] = {
		SkillName = "Estilhaçar Arma"
	},
	[SKID.GC_VENOMPRESSURE] = {SkillName = "Intoxicar"},
	[SKID.GC_POISONSMOKE] = {
		SkillName = "Nevoeiro Tóxico"
	},
	[SKID.GC_CLOAKINGEXCEED] = {SkillName = "Ocultação"},
	[SKID.GC_PHANTOMMENACE] = {
		SkillName = "Ameaça Fantasma"
	},
	[SKID.GC_HALLUCINATIONWALK] = {
		SkillName = "Passos da Ilusão"
	},
	[SKID.GC_ROLLINGCUTTER] = {
		SkillName = "Lâminas de Loki"
	},
	[SKID.GC_CROSSRIPPERSLASHER] = {
		SkillName = "Castigo de Loki"
	},
	[SKID.AB_JUDEX] = {SkillName = "Judex"},
	[SKID.AB_ANCILLA] = {
		SkillName = "Criar Ancilla"
	},
	[SKID.AB_ADORAMUS] = {SkillName = "Adoramus"},
	[SKID.AB_CLEMENTIA] = {SkillName = "Clementia"},
	[SKID.AB_CANTO] = {
		SkillName = "Canto Candidus"
	},
	[SKID.AB_CHEAL] = {
		SkillName = "Sopro Divino"
	},
	[SKID.AB_EPICLESIS] = {SkillName = "Epiclesis"},
	[SKID.AB_PRAEFATIO] = {SkillName = "Praefatio"},
	[SKID.AB_ORATIO] = {SkillName = "Oratio"},
	[SKID.AB_LAUDAAGNUS] = {
		SkillName = "Lauda Agnus"
	},
	[SKID.AB_LAUDARAMUS] = {
		SkillName = "Lauda Ramus"
	},
	[SKID.AB_EUCHARISTICA] = {SkillName = "Eucaristia"},
	[SKID.AB_RENOVATIO] = {SkillName = "Renovatio"},
	[SKID.AB_HIGHNESSHEAL] = {SkillName = "Curatio"},
	[SKID.AB_CLEARANCE] = {SkillName = "Gênese"},
	[SKID.AB_EXPIATIO] = {SkillName = "Expiatio"},
	[SKID.AB_DUPLELIGHT] = {
		SkillName = "Gemini Lumen"
	},
	[SKID.AB_DUPLELIGHT_MELEE] = {
		SkillName = "Magni Lumen"
	},
	[SKID.AB_DUPLELIGHT_MAGIC] = {SkillName = "Modi Lumen"},
	[SKID.AB_SILENTIUM] = {SkillName = "Silentium"},
	[SKID.WL_WHITEIMPRISON] = {SkillName = "Exílio"},
	[SKID.WL_SOULEXPANSION] = {
		SkillName = "Impacto Espiritual"
	},
	[SKID.WL_FROSTMISTY] = {
		SkillName = "Zero Absoluto"
	},
	[SKID.WL_JACKFROST] = {
		SkillName = "Esquife de Gelo"
	},
	[SKID.WL_MARSHOFABYSS] = {
		SkillName = "Pântano de Nifflheim"
	},
	[SKID.WL_RECOGNIZEDSPELL] = {
		SkillName = "Maestria Arcana"
	},
	[SKID.WL_SIENNAEXECRATE] = {
		SkillName = "Fúria da Medusa"
	},
	[SKID.WL_RADIUS] = {SkillName = "Radius"},
	[SKID.WL_STASIS] = {SkillName = "Stasis"},
	[SKID.WL_DRAINLIFE] = {
		SkillName = "Drenar Vida"
	},
	[SKID.WL_CRIMSONROCK] = {
		SkillName = "Meteoro Escarlate"
	},
	[SKID.WL_HELLINFERNO] = {
		SkillName = "Chamas de Hela"
	},
	[SKID.WL_COMET] = {SkillName = "Cometa"},
	[SKID.WL_CHAINLIGHTNING] = {
		SkillName = "Corrente Elétrica"
	},
	[SKID.WL_EARTHSTRAIN] = {
		SkillName = "Abalo Sísmico"
	},
	[SKID.WL_TETRAVORTEX] = {
		SkillName = "Tetra Vortex"
	},
	[SKID.WL_SUMMONFB] = {
		SkillName = "Invocar Esfera de Fogo"
	},
	[SKID.WL_SUMMONBL] = {
		SkillName = "Invocar Esfera de Vento"
	},
	[SKID.WL_SUMMONWB] = {
		SkillName = "Invocar Esfera de Água"
	},
	[SKID.WL_SUMMONSTONE] = {
		SkillName = "Invocar Esfera de Terra"
	},
	[SKID.WL_RELEASE] = {
		SkillName = "Lançar Magia"
	},
	[SKID.WL_READING_SB] = {SkillName = "Memorizar"},
	[SKID.WL_FREEZE_SP] = {
		SkillName = "Estudo Arcano Avançado"
	},
	[SKID.RA_ARROWSTORM] = {
		SkillName = "Tempestade de Flechas"
	},
	[SKID.RA_FEARBREEZE] = {
		SkillName = "Disparo Selvagem"
	},
	[SKID.RA_RANGERMAIN] = {
		SkillName = "Táticas de Sobrevivência"
	},
	[SKID.RA_AIMEDBOLT] = {
		SkillName = "Disparo Certeiro"
	},
	[SKID.RA_DETONATOR] = {SkillName = "Detonador"},
	[SKID.RA_ELECTRICSHOCKER] = {
		SkillName = "Armadilha Energizada"
	},
	[SKID.RA_CLUSTERBOMB] = {
		SkillName = "Bomba Relógio"
	},
	[SKID.RA_WUGMASTERY] = {
		SkillName = "Adestrar Worg"
	},
	[SKID.RA_WUGRIDER] = {
		SkillName = "Montaria em Worg"
	},
	[SKID.RA_WUGDASH] = {
		SkillName = "Assalto de Worg"
	},
	[SKID.RA_WUGSTRIKE] = {
		SkillName = "Investida de Worg"
	},
	[SKID.RA_WUGBITE] = {
		SkillName = "Mordida Feroz"
	},
	[SKID.RA_TOOTHOFWUG] = {
		SkillName = "Presas Afiadas"
	},
	[SKID.RA_SENSITIVEKEEN] = {
		SkillName = "Faro Aguçado"
	},
	[SKID.RA_CAMOUFLAGE] = {SkillName = "Camuflagem"},
	[SKID.RA_RESEARCHTRAP] = {
		SkillName = "Perícia com Armadilha"
	},
	[SKID.RA_MAGENTATRAP] = {
		SkillName = "Armadilha Escarlate"
	},
	[SKID.RA_COBALTTRAP] = {
		SkillName = "Armadilha Cobalto"
	},
	[SKID.RA_MAIZETRAP] = {
		SkillName = "Armadilha Ocre"
	},
	[SKID.RA_VERDURETRAP] = {
		SkillName = "Armadilha Esmeralda"
	},
	[SKID.RA_FIRINGTRAP] = {
		SkillName = "Armadilha Incendiária"
	},
	[SKID.RA_ICEBOUNDTRAP] = {
		SkillName = "Armadilha Glacial"
	},
	[SKID.NC_MADOLICENCE] = {
		SkillName = "Licença de Pilotagem"
	},
	[SKID.NC_BOOSTKNUCKLE] = {
		SkillName = "Punho Foguete"
	},
	[SKID.NC_PILEBUNKER] = {
		SkillName = "Bate Estaca"
	},
	[SKID.NC_VULCANARM] = {
		SkillName = "Metralhadora"
	},
	[SKID.NC_FLAMELAUNCHER] = {
		SkillName = "Lança Chamas"
	},
	[SKID.NC_COLDSLOWER] = {
		SkillName = "Gás Criogênico"
	},
	[SKID.NC_ARMSCANNON] = {SkillName = "Canhão"},
	[SKID.NC_ACCELERATION] = {SkillName = "Aceleração"},
	[SKID.NC_HOVERING] = {SkillName = "Planar"},
	[SKID.NC_F_SIDESLIDE] = {
		SkillName = "Propulsão Traseira"
	},
	[SKID.NC_B_SIDESLIDE] = {
		SkillName = "Propulsão Dianteira"
	},
	[SKID.NC_MAINFRAME] = {
		SkillName = "Reforçar Estrutura"
	},
	[SKID.NC_SELFDESTRUCTION] = {
		SkillName = "Autodestruição"
	},
	[SKID.NC_SHAPESHIFT] = {
		SkillName = "Reconfigurar Elemento"
	},
	[SKID.NC_EMERGENCYCOOL] = {
		SkillName = "Resfriamento"
	},
	[SKID.NC_INFRAREDSCAN] = {
		SkillName = "Sensor Infravermelho"
	},
	[SKID.NC_ANALYZE] = {SkillName = "Analisar"},
	[SKID.NC_MAGNETICFIELD] = {
		SkillName = "Campo Magnético"
	},
	[SKID.NC_NEUTRALBARRIER] = {
		SkillName = "Campo Protetor"
	},
	[SKID.NC_STEALTHFIELD] = {
		SkillName = "Campo de Invisibilidade"
	},
	[SKID.NC_REPAIR] = {SkillName = "Reparar"},
	[SKID.NC_TRAININGAXE] = {
		SkillName = "Maestria com Machados"
	},
	[SKID.NC_RESEARCHFE] = {
		SkillName = "Sabedoria de Hefesto"
	},
	[SKID.NC_AXEBOOMERANG] = {
		SkillName = "Arremesso de Machado"
	},
	[SKID.NC_POWERSWING] = {
		SkillName = "Brandir Machado"
	},
	[SKID.NC_AXETORNADO] = {
		SkillName = "Fúria do Furacão "
	},
	[SKID.NC_SILVERSNIPER] = {
		SkillName = "Artilharia Caçadora"
	},
	[SKID.NC_MAGICDECOY] = {
		SkillName = "Artilharia Arcana"
	},
	[SKID.NC_DISJOINT] = {
		SkillName = "Remover Artilharia"
	},
	[SKID.SC_STARTMARK] = {
		SkillName = "Ofensiva Fatal"
	},
	[SKID.SC_REPRODUCE] = {SkillName = "Mimetismo"},
	[SKID.SC_AUTOSHADOWSPELL] = {
		SkillName = "Desejo das Sombras"
	},
	[SKID.SC_SHADOWFORM] = {
		SkillName = "Vínculo Sombrio"
	},
	[SKID.SC_TRIANGLESHOT] = {
		SkillName = "Disparo Triplo"
	},
	[SKID.SC_BODYPAINT] = {
		SkillName = "Borrifar Tinta"
	},
	[SKID.SC_INVISIBILITY] = {
		SkillName = "Forma Etérea"
	},
	[SKID.SC_DEADLYINFECT] = {
		SkillName = "Pestilência"
	},
	[SKID.SC_ENERVATION] = {
		SkillName = "Máscara da Fraqueza"
	},
	[SKID.SC_GROOMY] = {
		SkillName = "Máscara da Melancolia"
	},
	[SKID.SC_IGNORANCE] = {
		SkillName = "Máscara da Tolice"
	},
	[SKID.SC_LAZINESS] = {
		SkillName = "Máscara da Ociosidade"
	},
	[SKID.SC_UNLUCKY] = {
		SkillName = "Máscara do Infortúnio"
	},
	[SKID.SC_WEAKNESS] = {
		SkillName = "Máscara da Vulnerabilidade"
	},
	[SKID.SC_STRIPACCESSARY] = {
		SkillName = "Remover Acessório"
	},
	[SKID.SC_MANHOLE] = {
		SkillName = "Pintar Armadilha"
	},
	[SKID.SC_DIMENSIONDOOR] = {
		SkillName = "Porta Dimensional"
	},
	[SKID.SC_CHAOSPANIC] = {
		SkillName = "Símbolo do Caos"
	},
	[SKID.SC_MAELSTROM] = {
		SkillName = "Redemoinho de Absorção"
	},
	[SKID.SC_BLOODYLUST] = {
		SkillName = "Sede de Sangue"
	},
	[SKID.SC_FEINTBOMB] = {
		SkillName = "Cópia Explosiva"
	},
	[SKID.LG_CANNONSPEAR] = {
		SkillName = "Disparo Perfurante"
	},
	[SKID.LG_BANISHINGPOINT] = {
		SkillName = "Toque do Oblívio"
	},
	[SKID.LG_TRAMPLE] = {
		SkillName = "Pisotear Armadilha"
	},
	[SKID.LG_SHIELDPRESS] = {
		SkillName = "Escudo Compressor"
	},
	[SKID.LG_REFLECTDAMAGE] = {
		SkillName = "Escudo Supressor"
	},
	[SKID.LG_PINPOINTATTACK] = {
		SkillName = "Estocada Precisa"
	},
	[SKID.LG_FORCEOFVANGUARD] = {
		SkillName = "Proteção da Vanguarda"
	},
	[SKID.LG_RAGEBURST] = {
		SkillName = "Retribuição da Vanguarda"
	},
	[SKID.LG_SHIELDSPELL] = {
		SkillName = "Aegis Domini"
	},
	[SKID.LG_EXEEDBREAK] = {
		SkillName = "Exceder Limite"
	},
	[SKID.LG_OVERBRAND] = {
		SkillName = "Lança do Destino"
	},
	[SKID.LG_PRESTIGE] = {
		SkillName = "Prestígio Divino"
	},
	[SKID.LG_BANDING] = {
		SkillName = "Formação Real"
	},
	[SKID.LG_MOONSLASHER] = {
		SkillName = "Espiral Lunar"
	},
	[SKID.LG_RAYOFGENESIS] = {
		SkillName = "Luz da Criação"
	},
	[SKID.LG_PIETY] = {SkillName = "Devoção"},
	[SKID.LG_EARTHDRIVE] = {
		SkillName = "Aegis Inferi"
	},
	[SKID.LG_HESPERUSLIT] = {SkillName = "Trindade"},
	[SKID.LG_INSPIRATION] = {
		SkillName = "Consagração"
	},
	[SKID.SR_DRAGONCOMBO] = {
		SkillName = "Punho do Dragão"
	},
	[SKID.SR_SKYNETBLOW] = {
		SkillName = "Soco Furacão"
	},
	[SKID.SR_EARTHSHAKER] = {
		SkillName = "Impacto Sísmico"
	},
	[SKID.SR_FALLENEMPIRE] = {SkillName = "Ruína"},
	[SKID.SR_TIGERCANNON] = {
		SkillName = "Garra de Tigre"
	},
	[SKID.SR_RAMPAGEBLASTER] = {
		SkillName = "Explosão Espiritual"
	},
	[SKID.SR_CRESCENTELBOW] = {
		SkillName = "Cotovelada Ascendente"
	},
	[SKID.SR_CURSEDCIRCLE] = {
		SkillName = "Campo Amaldiçoado"
	},
	[SKID.SR_LIGHTNINGWALK] = {
		SkillName = "Salto Relâmpago"
	},
	[SKID.SR_KNUCKLEARROW] = {
		SkillName = "Pancada Corporal"
	},
	[SKID.SR_WINDMILL] = {
		SkillName = "Chute Rasteiro"
	},
	[SKID.SR_RAISINGDRAGON] = {
		SkillName = "Dragão Ascendente"
	},
	[SKID.SR_ASSIMILATEPOWER] = {
		SkillName = "Absorção Espiritual"
	},
	[SKID.SR_POWERVELOCITY] = {
		SkillName = "Renúncia Espiritual"
	},
	[SKID.SR_GATEOFHELL] = {
		SkillName = "Portões do Inferno"
	},
	[SKID.SR_GENTLETOUCH_QUIET] = {
		SkillName = "Chakra do Silêncio"
	},
	[SKID.SR_GENTLETOUCH_CURE] = {
		SkillName = "Chakra da Cura"
	},
	[SKID.SR_GENTLETOUCH_ENERGYGAIN] = {
		SkillName = "Chakra da Energia"
	},
	[SKID.SR_GENTLETOUCH_CHANGE] = {
		SkillName = "Chakra da Fúria"
	},
	[SKID.SR_GENTLETOUCH_REVITALIZE] = {
		SkillName = "Chakra do Vigor"
	},
	[SKID.WA_SWING_DANCE] = {
		SkillName = "Ritmo Contagiante"
	},
	[SKID.WA_SYMPHONY_OF_LOVER] = {
		SkillName = "Balada Sinfônica"
	},
	[SKID.WA_MOONLIT_SERENADE] = {
		SkillName = "Serenata ao Luar"
	},
	[SKID.MI_RUSH_WINDMILL] = {
		SkillName = "Sinfonia dos Ventos"
	},
	[SKID.MI_ECHOSONG] = {
		SkillName = "Canção de Balder"
	},
	[SKID.MI_HARMONIZE] = {SkillName = "Harmonizar"},
	[SKID.WM_LESSON] = {
		SkillName = "Domínio Musicial"
	},
	[SKID.WM_METALICSOUND] = {
		SkillName = "Ruído Estridente"
	},
	[SKID.WM_REVERBERATION] = {
		SkillName = "Ressonância"
	},
	[SKID.WM_DOMINION_IMPULSE] = {
		SkillName = "Ativar Ressonância"
	},
	[SKID.WM_SEVERE_RAINSTORM] = {
		SkillName = "Temporal de Flechas"
	},
	[SKID.WM_POEMOFNETHERWORLD] = {
		SkillName = "Poema de Nifflheim"
	},
	[SKID.WM_VOICEOFSIREN] = {
		SkillName = "Canto da Sereia"
	},
	[SKID.WM_DEADHILLHERE] = {
		SkillName = "Réquiem de Orfeu"
	},
	[SKID.WM_LULLABY_DEEPSLEEP] = {
		SkillName = "Melodia de Morfeu"
	},
	[SKID.WM_SIRCLEOFNATURE] = {
		SkillName = "Sibilo de Eir"
	},
	[SKID.WM_RANDOMIZESPELL] = {SkillName = "Improviso"},
	[SKID.WM_GLOOMYDAY] = {SkillName = "Ode a Hela"},
	[SKID.WM_GREAT_ECHO] = {
		SkillName = "Brado de Odin"
	},
	[SKID.WM_SONG_OF_MANA] = {
		SkillName = "Canção de Alfheim"
	},
	[SKID.WM_DANCE_WITH_WUG] = {
		SkillName = "Dança com Lobos"
	},
	[SKID.WM_SOUND_OF_DESTRUCTION] = {
		SkillName = "Prelúdio do Ragnarök"
	},
	[SKID.WM_SATURDAY_NIGHT_FEVER] = {
		SkillName = "Embalos de Sábado a Noite"
	},
	[SKID.WM_LERADS_DEW] = {
		SkillName = "Orvalho de Idun"
	},
	[SKID.WM_MELODYOFSINK] = {
		SkillName = "Cântico da Iluminação"
	},
	[SKID.WM_BEYOND_OF_WARCRY] = {
		SkillName = "Clamor de Batalha"
	},
	[SKID.WM_UNLIMITED_HUMMING_VOICE] = {
		SkillName = "Murmúrio Perene"
	},
	[SKID.SO_FIREWALK] = {
		SkillName = "Passos de Salamandra"
	},
	[SKID.SO_ELECTRICWALK] = {
		SkillName = "Passos de Sílfide"
	},
	[SKID.SO_SPELLFIST] = {
		SkillName = "Punho Arcano"
	},
	[SKID.SO_EARTHGRAVE] = {
		SkillName = "Castigo de Nerthus"
	},
	[SKID.SO_DIAMONDDUST] = {
		SkillName = "Pó de Diamante"
	},
	[SKID.SO_POISON_BUSTER] = {
		SkillName = "Implosão Tóxica"
	},
	[SKID.SO_PSYCHIC_WAVE] = {
		SkillName = "Onda Psíquica"
	},
	[SKID.SO_CLOUD_KILL] = {
		SkillName = "Maldição de Jormungand"
	},
	[SKID.SO_STRIKING] = {
		SkillName = "Encanto de Órion"
	},
	[SKID.SO_WARMER] = {
		SkillName = "Aquecer Terreno"
	},
	[SKID.SO_VACUUM_EXTREME] = {SkillName = "Tornado"},
	[SKID.SO_VARETYR_SPEAR] = {
		SkillName = "Lanças dos Aesir"
	},
	[SKID.SO_ARRULLO] = {SkillName = "Arrullo"},
	[SKID.SO_EL_CONTROL] = {
		SkillName = "Domínio Elemental"
	},
	[SKID.SO_SUMMON_AGNI] = {
		SkillName = "Invocar Agni"
	},
	[SKID.SO_SUMMON_AQUA] = {
		SkillName = "Invocar Varuna"
	},
	[SKID.SO_SUMMON_VENTUS] = {
		SkillName = "Invocar Vayu"
	},
	[SKID.SO_SUMMON_TERA] = {
		SkillName = "Invocar Chandra"
	},
	[SKID.SO_EL_ACTION] = {
		SkillName = "Incitar Elemental"
	},
	[SKID.SO_EL_ANALYSIS] = {
		SkillName = "Análise Elemental"
	},
	[SKID.SO_EL_SYMPATHY] = {
		SkillName = "Empatia Elemental"
	},
	[SKID.SO_EL_CURE] = {
		SkillName = "Troca Espiritual"
	},
	[SKID.SO_FIRE_INSIGNIA] = {
		SkillName = "Insígnia do Fogo"
	},
	[SKID.SO_WATER_INSIGNIA] = {
		SkillName = "Insígnia da Água"
	},
	[SKID.SO_WIND_INSIGNIA] = {
		SkillName = "Insígnia do Vento"
	},
	[SKID.SO_EARTH_INSIGNIA] = {
		SkillName = "Insígnia da Terra"
	},
	[SKID.GN_TRAINING_SWORD] = {
		SkillName = "Perícia em Esgrima"
	},
	[SKID.GN_REMODELING_CART] = {
		SkillName = "Aprimorar Carrinho"
	},
	[SKID.GN_CART_TORNADO] = {
		SkillName = "Tornado de Carrinho"
	},
	[SKID.GN_CARTCANNON] = {
		SkillName = "Canhão de Prótons"
	},
	[SKID.GN_CARTBOOST] = {
		SkillName = "Propulsão do Carrinho"
	},
	[SKID.GN_THORNS_TRAP] = {
		SkillName = "Armadilha de Espinhos"
	},
	[SKID.GN_BLOOD_SUCKER] = {
		SkillName = "Planta Sanguessuga"
	},
	[SKID.GN_SPORE_EXPLOSION] = {
		SkillName = "Esporo Explosivo"
	},
	[SKID.GN_WALLOFTHORN] = {
		SkillName = "Muralha de Espinhos"
	},
	[SKID.GN_CRAZYWEED] = {
		SkillName = "Erva Daninha"
	},
	[SKID.GN_DEMONIC_FIRE] = {
		SkillName = "Bomba Napalm"
	},
	[SKID.GN_FIRE_EXPANSION] = {
		SkillName = "Catalisador Alquímico"
	},
	[SKID.GN_HELLS_PLANT] = {
		SkillName = "Planta Infernal"
	},
	[SKID.GN_MANDRAGORA] = {
		SkillName = "Grito da Mandrágora"
	},
	[SKID.GN_SLINGITEM] = {
		SkillName = "Arremessar Item"
	},
	[SKID.GN_CHANGEMATERIAL] = {
		SkillName = "Reação Alquímica"
	},
	[SKID.GN_MIX_COOKING] = {
		SkillName = "Culinária Avançada"
	},
	[SKID.GN_MAKEBOMB] = {
		SkillName = "Criar Bomba Orgânica"
	},
	[SKID.GN_S_PHARMACY] = {
		SkillName = "Farmacologia Avançada"
	},
	[SKID.AB_SECRAMENT] = {
		SkillName = "Sacramentum"
	},
	[SKID.SR_HOWLINGOFLION] = {
		SkillName = "Rugido do Leão"
	},
	[SKID.SR_RIDEINLIGHTNING] = {
		SkillName = "Tempestade Espiritual"
	},
	[SKID.ALL_ODINS_RECALL] = {
		SkillName = "Chamado de Odin"
	},
	[SKID.RETURN_TO_ELDICASTES] = {
		SkillName = "Para El Dicastes"
	},
	[SKID.ALL_BUYING_STORE] = {
		SkillName = "Loja de Compras"
	},
	[SKID.ALL_GUARDIAN_RECALL] = {
		SkillName = "Chamado do Guardião"
	},
	[SKID.ALL_ODINS_POWER] = {
		SkillName = "Poder de Odin"
	},
	[SKID.NPC_ASSASSINCROSS] = {
		SkillName = "Crepúsculo Sangrento"
	},
	[SKID.NPC_DISSONANCE] = {
		SkillName = "Dissonância"
	},
	[SKID.NPC_UGLYDANCE] = {
		SkillName = "Dança do Ventre"
	},
	[SKID.ALL_RAY_OF_PROTECTION] = {
		SkillName = "Luz Protetora"
	},
	[SKID.MC_CARTDECORATE] = {
		SkillName = "Decorar Carrinho"
	},
	[SKID.GM_ITEM_ATKMAX] = {
		SkillName = "Maximizar ATQ"
	},
	[SKID.GM_ITEM_ATKMIN] = {
		SkillName = "Minimizar ATQ"
	},
	[SKID.GM_ITEM_MATKMAX] = {
		SkillName = "Maximizar ATQM"
	},
	[SKID.GM_ITEM_MATKMIN] = {
		SkillName = "Minimizar ATQM"
	},
	[SKID.RL_RICHS_COIN] = {
		SkillName = "Farta Fortuna"
	},
	[SKID.RL_MASS_SPIRAL] = {
		SkillName = "Espiral Perfurante"
	},
	[SKID.RL_BANISHING_BUSTER] = {
		SkillName = "Tiro Neutralizante"
	},
	[SKID.RL_B_TRAP] = {
		SkillName = "Armadilha Aderente"
	},
	[SKID.RL_FLICKER] = {SkillName = "Ignição"},
	[SKID.RL_S_STORM] = {
		SkillName = "Rajada Estilhaçante"
	},
	[SKID.RL_E_CHAIN] = {
		SkillName = "Reação Ilimitada"
	},
	[SKID.RL_QD_SHOT] = {
		SkillName = "Saque Rápido"
	},
	[SKID.RL_C_MARKER] = {
		SkillName = "Marcar em Brasa"
	},
	[SKID.RL_FIREDANCE] = {
		SkillName = "Descarregar Tambor"
	},
	[SKID.RL_H_MINE] = {
		SkillName = "Arremessar Granada"
	},
	[SKID.RL_P_ALTER] = {
		SkillName = "Proteção de Platina"
	},
	[SKID.RL_FALLEN_ANGEL] = {
		SkillName = "Salto Etéreo"
	},
	[SKID.RL_R_TRIP] = {SkillName = "Expurgar"},
	[SKID.RL_D_TAIL] = {
		SkillName = "Lançar Míssil"
	},
	[SKID.RL_FIRE_RAIN] = {
		SkillName = "Disparo Labareda"
	},
	[SKID.RL_HEAT_BARREL] = {SkillName = "Furor"},
	[SKID.RL_AM_BLAST] = {
		SkillName = "Explosão Antimatéria"
	},
	[SKID.RL_SLUGSHOT] = {
		SkillName = "Calibre Letal"
	},
	[SKID.RL_HAMMER_OF_GOD] = {SkillName = "Execução"},
	[SKID.SJ_LIGHTOFMOON] = {SkillName = "Luz Lunar"},
	[SKID.SJ_LUNARSTANCE] = {
		SkillName = "Postura Lunar"
	},
	[SKID.SJ_FULLMOONKICK] = {
		SkillName = "Chute Lunar"
	},
	[SKID.SJ_LIGHTOFSTAR] = {
		SkillName = "Luz Estelar"
	},
	[SKID.SJ_STARSTANCE] = {
		SkillName = "Postura Estelar"
	},
	[SKID.SJ_NEWMOONKICK] = {
		SkillName = "Eclipse Lunar"
	},
	[SKID.SJ_FLASHKICK] = {
		SkillName = "Chute Estelar"
	},
	[SKID.SJ_STAREMPEROR] = {
		SkillName = "Advento Imperial"
	},
	[SKID.SJ_NOVAEXPLOSING] = {SkillName = "Hipernova"},
	[SKID.SJ_UNIVERSESTANCE] = {
		SkillName = "Postura do Universo"
	},
	[SKID.SJ_FALLINGSTAR] = {
		SkillName = "Chuva Estelar"
	},
	[SKID.SJ_GRAVITYCONTROL] = {
		SkillName = "Controle Gravitacional"
	},
	[SKID.SJ_BOOKOFDIMENSION] = {
		SkillName = "Livro das Dimensões"
	},
	[SKID.SJ_BOOKOFCREATINGSTAR] = {
		SkillName = "Livro da Criação"
	},
	[SKID.SJ_DOCUMENT] = {
		SkillName = "Registro Solar, Lunar e Estelar"
	},
	[SKID.SJ_PURIFY] = {
		SkillName = "Purificação Solar, Lunar e Estelar"
	},
	[SKID.SJ_LIGHTOFSUN] = {SkillName = "Luz Solar"},
	[SKID.SJ_SUNSTANCE] = {
		SkillName = "Postura Solar"
	},
	[SKID.SJ_SOLARBURST] = {
		SkillName = "Explosão Solar"
	},
	[SKID.SJ_PROMINENCEKICK] = {
		SkillName = "Chute Solar"
	},
	[SKID.SJ_FALLINGSTAR_ATK] = {
		SkillName = "Chuva Estelar"
	},
	[SKID.SJ_FALLINGSTAR_ATK2] = {
		SkillName = "Chuva Estelar"
	},
	[SKID.SP_SOULGOLEM] = {
		SkillName = "Espírito do Golem"
	},
	[SKID.SP_SOULSHADOW] = {
		SkillName = "Espírito das Sombras"
	},
	[SKID.SP_SOULFALCON] = {
		SkillName = "Espírito do Falcão"
	},
	[SKID.SP_SOULFAIRY] = {
		SkillName = "Espírito da Fada"
	},
	[SKID.SP_CURSEEXPLOSION] = {
		SkillName = "Necroexplosão"
	},
	[SKID.SP_SOULCURSE] = {
		SkillName = "Necromancia"
	},
	[SKID.SP_SPA] = {SkillName = "Espa"},
	[SKID.SP_SHA] = {SkillName = "Esha"},
	[SKID.SP_SWHOO] = {SkillName = "Eswhoo"},
	[SKID.SP_SOULUNITY] = {
		SkillName = "União Espiritual"
	},
	[SKID.SP_SOULDIVISION] = {
		SkillName = "Divisão de Alma"
	},
	[SKID.SP_SOULREAPER] = {
		SkillName = "Ceifar Alma"
	},
	[SKID.SP_SOULREVOLVE] = {
		SkillName = "Absorver Espírito"
	},
	[SKID.SP_SOULCOLLECT] = {
		SkillName = "Coletar Alma"
	},
	[SKID.SP_SOULEXPLOSION] = {
		SkillName = "Sifão Espiritual"
	},
	[SKID.SP_SOULENERGY] = {
		SkillName = "Perícia com Almas"
	},
	[SKID.SP_KAUTE] = {SkillName = "Kaute"},
	[SKID.KO_YAMIKUMO] = {
		SkillName = "Refúgio das Sombras"
	},
	[SKID.KO_RIGHT] = {
		SkillName = "Perícia com a Mão Direita"
	},
	[SKID.KO_LEFT] = {
		SkillName = "Perícia com a Mão Esquerda"
	},
	[SKID.KO_JYUMONJIKIRI] = {
		SkillName = "Impacto Cruzado"
	},
	[SKID.KO_SETSUDAN] = {
		SkillName = "Corte Espiritual"
	},
	[SKID.KO_BAKURETSU] = {
		SkillName = "Kunai Explosiva"
	},
	[SKID.KO_HAPPOKUNAI] = {
		SkillName = "Turbilhão de Kunais"
	},
	[SKID.KO_MUCHANAGE] = {
		SkillName = "Explosão de Moedas"
	},
	[SKID.KO_HUUMARANKA] = {
		SkillName = "Turbilhão de Pétalas"
	},
	[SKID.KO_MAKIBISHI] = {SkillName = "Estrepes"},
	[SKID.KO_MEIKYOUSISUI] = {
		SkillName = "Purificação da Alma"
	},
	[SKID.KO_ZANZOU] = {
		SkillName = "Genjutsu: Clone das Sombras"
	},
	[SKID.KO_KYOUGAKU] = {
		SkillName = "Genjutsu: Assalto das Sombras"
	},
	[SKID.KO_JYUSATSU] = {
		SkillName = "Genjutsu: Chamado da Morte"
	},
	[SKID.KO_KAHU_ENTEN] = {
		SkillName = "Amuleto de Fogo"
	},
	[SKID.KO_HYOUHU_HUBUKI] = {
		SkillName = "Amuleto de Água"
	},
	[SKID.KO_KAZEHU_SEIRAN] = {
		SkillName = "Amuleto de Vento"
	},
	[SKID.KO_DOHU_KOUKAI] = {
		SkillName = "Amuleto de Terra"
	},
	[SKID.KO_KAIHOU] = {
		SkillName = "Arremessar Amuleto"
	},
	[SKID.KO_ZENKAI] = {
		SkillName = "Voragem Espiritual"
	},
	[SKID.KO_GENWAKU] = {
		SkillName = "Genjutsu: Substituição"
	},
	[SKID.KO_IZAYOI] = {SkillName = "Inspiração"},
	[SKID.KG_KAGEHUMI] = {
		SkillName = "Esmagamento Sombrio"
	},
	[SKID.KG_KYOMU] = {
		SkillName = "Vazio das Sombras"
	},
	[SKID.KG_KAGEMUSYA] = {
		SkillName = "Contrato das Sombras"
	},
	[SKID.OB_ZANGETSU] = {
		SkillName = "Distorção Crescente"
	},
	[SKID.OB_OBOROGENSOU] = {
		SkillName = "Ilusão do Luar"
	},
	[SKID.OB_AKAITSUKI] = {
		SkillName = "Luar Sinistro"
	},
	[SKID.ECL_SNOWFLIP] = {SkillName = "Gardênia"},
	[SKID.ECL_PEONYMAMY] = {SkillName = "Peônia"},
	[SKID.ECL_SADAGUI] = {SkillName = "Erva-Mate"},
	[SKID.ECL_SEQUOIADUST] = {
		SkillName = "Pó de Yggdrasil"
	},
	[SKID.ECLAGE_RECALL] = {
		SkillName = "Para Eclage"
	},
	[SKID.BA_POEMBRAGI2] = {
		SkillName = "Poema de Bragi"
	},
	[SKID.DC_FORTUNEKISS2] = {
		SkillName = "Beijo da Sorte"
	},
	[SKID.ALL_NIFLHEIM_RECALL] = {
		SkillName = "Para Nifflheim"
	},
	[SKID.ALL_PRONTERA_RECALL] = {
		SkillName = "Para Prontera"
	},
	[SKID.ALL_THANATOS_RECALL] = {
		SkillName = "Para Thanatos"
	},
	[SKID.ALL_LIGHTHALZEN_RECALL] = {
		SkillName = "Para Lighthalzen"
	},
	[SKID.GC_DARKCROW] = {
		SkillName = "Garra Sombria"
	},
	[SKID.RA_UNLIMIT] = {SkillName = "Ilimitar"},
	[SKID.GN_ILLUSIONDOPING] = {
		SkillName = "Alucinógeno"
	},
	[SKID.RK_DRAGONBREATH_WATER] = {
		SkillName = "Bafo do Dragão"
	},
	[SKID.RK_LUXANIMA] = {
		SkillName = "Luz da Alma"
	},
	[SKID.NC_MAGMA_ERUPTION] = {
		SkillName = "Erupção de Magma"
	},
	[SKID.WM_FRIGG_SONG] = {
		SkillName = "Canção de Frigga"
	},
	[SKID.SO_ELEMENTAL_SHIELD] = {
		SkillName = "Escudo Elemental"
	},
	[SKID.SR_FLASHCOMBO] = {
		SkillName = "Combo Rápido"
	},
	[SKID.SC_ESCAPE] = {SkillName = "Escapar"},
	[SKID.AB_OFFERTORIUM] = {
		SkillName = "Offertorium"
	},
	[SKID.WL_TELEKINESIS_INTENSE] = {
		SkillName = "Telecinesia"
	},
	[SKID.LG_KINGS_GRACE] = {SkillName = "Graça Real"},
	[SKID.ALL_FULL_THROTTLE] = {
		SkillName = "Maximizar Poder"
	},
	[SKID.SU_BASIC_SKILL] = {
		SkillName = "Instintos Básicos"
	},
	[SKID.SU_BITE] = {SkillName = "Mordida"},
	[SKID.SU_HIDE] = {SkillName = "Gato-Mia"},
	[SKID.SU_SCRATCH] = {SkillName = "Arranhar"},
	[SKID.SU_STOOP] = {
		SkillName = "Quatro Patas"
	},
	[SKID.SU_LOPE] = {
		SkillName = "Pulo do Gato"
	},
	[SKID.SU_SPRITEMABLE] = {SkillName = "Invocar"},
	[SKID.SU_POWEROFLAND] = {
		SkillName = "Poder da Selva"
	},
	[SKID.SU_SV_STEMSPEAR] = {
		SkillName = "Lança Gateira"
	},
	[SKID.SU_CN_POWDERING] = {SkillName = "Pulverizar"},
	[SKID.SU_CN_METEOR] = {
		SkillName = "Meteoros de Nepeta"
	},
	[SKID.SU_SV_ROOTTWIST] = {
		SkillName = "Hera Venenosa"
	},
	[SKID.SU_POWEROFLIFE] = {
		SkillName = "Poder da Fauna"
	},
	[SKID.SU_SCAROFTAROU] = {
		SkillName = "Ferida de Tarou"
	},
	[SKID.SU_PICKYPECK] = {
		SkillName = "Chilique de Picky"
	},
	[SKID.SU_ARCLOUSEDASH] = {
		SkillName = "Impulso de Arclouse"
	},
	[SKID.SU_LUNATICCARROTBEAT] = {
		SkillName = "Cometas Lunáticos"
	},
	[SKID.SU_POWEROFSEA] = {
		SkillName = "Poder das Marés"
	},
	[SKID.SU_TUNABELLY] = {
		SkillName = "Dádiva do Atum"
	},
	[SKID.SU_TUNAPARTY] = {
		SkillName = "Proteção da Orla"
	},
	[SKID.SU_BUNCHOFSHRIMP] = {
		SkillName = "Chuva de Mariscos"
	},
	[SKID.SU_FRESHSHRIMP] = {
		SkillName = "Camarão Fresquinho"
	},
	[SKID.SU_SOULATTACK] = {
		SkillName = "Ampliar Alcance"
	},
	[SKID.SU_POWEROFFLOCK] = {SkillName = "Intimidar"},
	[SKID.SU_SVG_SPIRIT] = {
		SkillName = "Ataque Selvagem"
	},
	[SKID.SU_HISS] = {
		SkillName = "Riscar Fósforo"
	},
	[SKID.SU_NYANGGRASS] = {
		SkillName = "Mato de Gato"
	},
	[SKID.SU_GROOMING] = {SkillName = "Lambida"},
	[SKID.SU_PURRING] = {
		SkillName = "Banho de Lambidas"
	},
	[SKID.SU_SHRIMPARTY] = {
		SkillName = "Festa do Camarão"
	},
	[SKID.SU_SPIRITOFLIFE] = {
		SkillName = "Invocação da Fauna"
	},
	[SKID.SU_MEOWMEOW] = {
		SkillName = "Balaio de Gato"
	},
	[SKID.SU_SPIRITOFLAND] = {
		SkillName = "Invocação da Selva"
	},
	[SKID.SU_CHATTERING] = {SkillName = "Despertar"},
	[SKID.SU_SPIRITOFSEA] = {
		SkillName = "Invocação das Marés"
	},
	[SKID.WE_CALLALLFAMILY] = {SkillName = "Em Família"},
	[SKID.WE_ONEFOREVER] = {
		SkillName = "Felizes Para Sempre"
	},
	[SKID.WE_CHEERUP] = {SkillName = "Animar"},
	[SKID.CG_SPECIALSINGER] = {
		SkillName = "Vocação Musical"
	},
	[SKID.AB_VITUPERATUM] = {
		SkillName = "Vituperatum"
	},
	[SKID.AB_CONVENIO] = {SkillName = "Convenio"},
	[SKID.NV_BREAKTHROUGH] = {
		SkillName = "Crescimento"
	},
	[SKID.NV_HELPANGEL] = {
		SkillName = "Ajuda Angelical"
	},
	[SKID.NV_TRANSCENDENCE] = {
		SkillName = "Transcendência"
	},
	[SKID.WL_READING_SB_READING] = {SkillName = "Memorizar"},
	[SKID.DK_SERVANTWEAPON] = {
		SkillName = "Espada Alada"
	},
	[SKID.DK_SERVANT_W_SIGN] = {
		SkillName = "Marca Alada"
	},
	[SKID.DK_SERVANT_W_PHANTOM] = {
		SkillName = "Impulsão Alada"
	},
	[SKID.DK_SERVANT_W_DEMOL] = {
		SkillName = "Demolição Alada"
	},
	[SKID.DK_CHARGINGPIERCE] = {
		SkillName = "Acumular Poder"
	},
	[SKID.DK_TWOHANDDEF] = {
		SkillName = "Defesa de Duas Mãos"
	},
	[SKID.DK_HACKANDSLASHER] = {
		SkillName = "Corte e Massacre"
	},
	[SKID.DK_DRAGONIC_AURA] = {
		SkillName = "Aura Draconiana"
	},
	[SKID.DK_MADNESS_CRUSHER] = {
		SkillName = "Cólera Esmagadora"
	},
	[SKID.DK_VIGOR] = {
		SkillName = "Determinação"
	},
	[SKID.DK_STORMSLASH] = {SkillName = "Dilacerar"},
	[SKID.AG_DEADLY_PROJECTION] = {
		SkillName = "Projeção Maldita"
	},
	[SKID.AG_DESTRUCTIVE_HURRICANE] = {
		SkillName = "Tufão Destrutivo"
	},
	[SKID.AG_RAIN_OF_CRYSTAL] = {
		SkillName = "Cascata de Cristal"
	},
	[SKID.AG_MYSTERY_ILLUSION] = {
		SkillName = "Portal do Submundo"
	},
	[SKID.AG_VIOLENT_QUAKE] = {
		SkillName = "Pilares de Pedra"
	},
	[SKID.AG_SOUL_VC_STRIKE] = {
		SkillName = "Espíritos Ancestrais"
	},
	[SKID.AG_STRANTUM_TREMOR] = {
		SkillName = "Stratum Tremor"
	},
	[SKID.AG_ALL_BLOOM] = {SkillName = "Florescer"},
	[SKID.AG_CRYSTAL_IMPACT] = {
		SkillName = "Cacos de Gelo"
	},
	[SKID.AG_TORNADO_STORM] = {SkillName = "Ciclone"},
	[SKID.AG_TWOHANDSTAFF] = {
		SkillName = "Perícia com Cajado de Duas Mãos"
	},
	[SKID.AG_FLORAL_FLARE_ROAD] = {
		SkillName = "Vereda Floral Incandescente"
	},
	[SKID.AG_ASTRAL_STRIKE] = {
		SkillName = "Colisão Astral"
	},
	[SKID.AG_CLIMAX] = {
		SkillName = "Intensificação"
	},
	[SKID.AG_ROCK_DOWN] = {
		SkillName = "Deslize de Pedras"
	},
	[SKID.AG_STORM_CANNON] = {
		SkillName = "Lufada de Vento"
	},
	[SKID.AG_CRIMSON_ARROW] = {
		SkillName = "Flecha Escarlate"
	},
	[SKID.AG_FROZEN_SLASH] = {
		SkillName = "Gelo Cortante"
	},
	[SKID.IQ_POWERFUL_FAITH] = {
		SkillName = "Mantra da Força"
	},
	[SKID.IQ_FIRM_FAITH] = {
		SkillName = "Mantra da Saúde"
	},
	[SKID.IQ_WILL_OF_FAITH] = {
		SkillName = "Caminhos da Fé "
	},
	[SKID.IQ_OLEUM_SANCTUM] = {
		SkillName = "Oleum Sanctum"
	},
	[SKID.IQ_SINCERE_FAITH] = {
		SkillName = "Mantra da Energia"
	},
	[SKID.IQ_MASSIVE_F_BLASTER] = {
		SkillName = "Punho Labareda"
	},
	[SKID.IQ_EXPOSION_BLASTER] = {
		SkillName = "Técnica da Mão Explosiva"
	},
	[SKID.IQ_FIRST_BRAND] = {SkillName = "Estigma"},
	[SKID.IQ_FIRST_FAITH_POWER] = {
		SkillName = "Auréola do Poder"
	},
	[SKID.IQ_JUDGE] = {
		SkillName = "Auréola do Juiz"
	},
	[SKID.IQ_SECOND_FLAME] = {
		SkillName = "Palma Fervorosa"
	},
	[SKID.IQ_SECOND_FAITH] = {
		SkillName = "Golpe Pantocrator"
	},
	[SKID.IQ_SECOND_JUDGEMENT] = {
		SkillName = "Toque Intercessor"
	},
	[SKID.IQ_THIRD_PUNISH] = {
		SkillName = "Soco Guilhotina"
	},
	[SKID.IQ_THIRD_FLAME_BOMB] = {
		SkillName = "Fogueira Espiritual"
	},
	[SKID.IQ_THIRD_CONSECRATION] = {SkillName = "Veredicto"},
	[SKID.IQ_THIRD_EXOR_FLAME] = {
		SkillName = "Auréola das Chamas"
	},
	[SKID.IG_GUARD_STANCE] = {
		SkillName = "Posição de Defesa"
	},
	[SKID.IG_GUARDIAN_SHIELD] = {
		SkillName = "Escudo Guardião"
	},
	[SKID.IG_REBOUND_SHIELD] = {SkillName = "Remissão"},
	[SKID.IG_SHIELD_MASTERY] = {
		SkillName = "Perícia com Escudo"
	},
	[SKID.IG_SPEAR_SWORD_M] = {
		SkillName = "Maestria da Guarda"
	},
	[SKID.IG_ATTACK_STANCE] = {
		SkillName = "Posição de Ataque"
	},
	[SKID.IG_ULTIMATE_SACRIFICE] = {SkillName = "Ultimato"},
	[SKID.IG_HOLY_SHIELD] = {
		SkillName = "Escudo Divino"
	},
	[SKID.IG_GRAND_JUDGEMENT] = {
		SkillName = "Lança da Justiça"
	},
	[SKID.IG_JUDGEMENT_CROSS] = {
		SkillName = "Crux Judicium"
	},
	[SKID.IG_SHIELD_SHOOTING] = {
		SkillName = "Arremessar Escudo"
	},
	[SKID.IG_OVERSLASH] = {
		SkillName = "Golpe do Destino"
	},
	[SKID.IG_CROSS_RAIN] = {
		SkillName = "Crux Tempestas"
	},
	[SKID.CD_REPARATIO] = {SkillName = "Reparatio"},
	[SKID.CD_MEDIALE_VOTUM] = {
		SkillName = "Mediale Votum"
	},
	[SKID.CD_MACE_BOOK_M] = {
		SkillName = "Maestria Litúrgica"
	},
	[SKID.CD_ARGUTUS_VITA] = {
		SkillName = "Argutus Vita"
	},
	[SKID.CD_ARGUTUS_TELUM] = {
		SkillName = "Argutus Telum"
	},
	[SKID.CD_ARBITRIUM] = {SkillName = "Arbitrium"},
	[SKID.CD_PRESENS_ACIES] = {
		SkillName = "Presens Acies"
	},
	[SKID.CD_FIDUS_ANIMUS] = {
		SkillName = "Fidus Animus"
	},
	[SKID.CD_EFFLIGO] = {SkillName = "Effligo"},
	[SKID.CD_COMPETENTIA] = {
		SkillName = "Competentia"
	},
	[SKID.CD_PNEUMATICUS_PROCELLA] = {
		SkillName = "Chuva Milagrosa"
	},
	[SKID.CD_DILECTIO_HEAL] = {
		SkillName = "Prece Divina"
	},
	[SKID.CD_RELIGIO] = {SkillName = "Religio"},
	[SKID.CD_BENEDICTUM] = {SkillName = "Benedictum"},
	[SKID.CD_PETITIO] = {SkillName = "Petitio"},
	[SKID.CD_FRAMEN] = {SkillName = "Flamen"},
	[SKID.SHC_SHADOW_EXCEED] = {
		SkillName = "Carrasco Sombrio"
	},
	[SKID.SHC_DANCING_KNIFE] = {
		SkillName = "Adagas Voadoras"
	},
	[SKID.SHC_SAVAGE_IMPACT] = {
		SkillName = "Impacto Brutal"
	},
	[SKID.SHC_SHADOW_SENSE] = {
		SkillName = "Senso das Sombras"
	},
	[SKID.SHC_ETERNAL_SLASH] = {
		SkillName = "Talho Eterno"
	},
	[SKID.SHC_POTENT_VENOM] = {
		SkillName = "Adulterar Veneno"
	},
	[SKID.SHC_SHADOW_STAB] = {SkillName = "Facada"},
	[SKID.SHC_IMPACT_CRATER] = {
		SkillName = "Impacto Cratera"
	},
	[SKID.SHC_ENCHANTING_SHADOW] = {
		SkillName = "Profanar Arma"
	},
	[SKID.SHC_FATAL_SHADOW_CROW] = {
		SkillName = "Revoada Sombria"
	},
	[SKID.MT_AXE_STOMP] = {
		SkillName = "Machado Esmagador"
	},
	[SKID.MT_RUSH_QUAKE] = {
		SkillName = "Avanço Sísmico"
	},
	[SKID.MT_M_MACHINE] = {
		SkillName = "Criar Máquina"
	},
	[SKID.MT_A_MACHINE] = {
		SkillName = "Ativar Robô Atacante"
	},
	[SKID.MT_D_MACHINE] = {
		SkillName = "Ativar Robô Defensor"
	},
	[SKID.MT_TWOAXEDEF] = {
		SkillName = "Defesa com Machado"
	},
	[SKID.MT_ABR_M] = {
		SkillName = "Mecatrônica"
	},
	[SKID.MT_SUMMON_ABR_BATTLE_WARIOR] = {
		SkillName = "Fabricar Soldado"
	},
	[SKID.MT_SUMMON_ABR_DUAL_CANNON] = {
		SkillName = "Fabricar Canhoneiro"
	},
	[SKID.MT_SUMMON_ABR_MOTHER_NET] = {
		SkillName = "Fabricar Curandeira"
	},
	[SKID.MT_SUMMON_ABR_INFINITY] = {
		SkillName = "Fabricar Escoteiro"
	},
	[SKID.ABC_DAGGER_AND_BOW_M] = {
		SkillName = "Versatilidade"
	},
	[SKID.ABC_MAGIC_SWORD_M] = {
		SkillName = "Lâmina Mágica"
	},
	[SKID.ABC_STRIP_SHADOW] = {
		SkillName = "Remoção Sombria Total"
	},
	[SKID.ABC_ABYSS_DAGGER] = {
		SkillName = "Ofensiva Abissal"
	},
	[SKID.ABC_UNLUCKY_RUSH] = {
		SkillName = "Salto Revés"
	},
	[SKID.ABC_CHAIN_REACTION_SHOT] = {
		SkillName = "Disparo em Cadeia"
	},
	[SKID.ABC_FROM_THE_ABYSS] = {
		SkillName = "Invocação do Abismo"
	},
	[SKID.ABC_ABYSS_SLAYER] = {
		SkillName = "Sangue-Frio"
	},
	[SKID.ABC_ABYSS_STRIKE] = {
		SkillName = "Meteoro Ômega"
	},
	[SKID.ABC_DEFT_STAB] = {SkillName = "Esfaquear"},
	[SKID.ABC_ABYSS_SQUARE] = {
		SkillName = "Fenda do Abismo"
	},
	[SKID.ABC_FRENZY_SHOT] = {
		SkillName = "Rajada Frenética"
	},
	[SKID.WH_ADVANCED_TRAP] = {
		SkillName = "Armadilha Avançada"
	},
	[SKID.WH_WIND_SIGN] = {
		SkillName = "Sinal do Vento"
	},
	[SKID.WH_NATUREFRIENDLY] = {
		SkillName = "Amigo da Natureza"
	},
	[SKID.WH_HAWKRUSH] = {
		SkillName = "Mergulho Aéreo"
	},
	[SKID.WH_HAWK_M] = {SkillName = "Chamar Ave"},
	[SKID.WH_CALAMITYGALE] = {
		SkillName = "Ventos Sinistros"
	},
	[SKID.WH_HAWKBOOMERANG] = {
		SkillName = "Aterrissagem"
	},
	[SKID.WH_GALESTORM] = {
		SkillName = "Vendaval de Flechas"
	},
	[SKID.WH_DEEPBLINDTRAP] = {
		SkillName = "Armadilha Sombria"
	},
	[SKID.WH_SOLIDTRAP] = {
		SkillName = "Armadilha Terrestre"
	},
	[SKID.WH_SWIFTTRAP] = {
		SkillName = "Armadilha Zéfira"
	},
	[SKID.WH_CRESCIVE_BOLT] = {
		SkillName = "Tiro Crescente"
	},
	[SKID.WH_FLAMETRAP] = {
		SkillName = "Armadilha Ardente"
	},
	[SKID.BO_BIONIC_PHARMACY] = {
		SkillName = "Manipular Poção"
	},
	[SKID.BO_BIONICS_M] = {SkillName = "Botânica"},
	[SKID.BO_THE_WHOLE_PROTECTION] = {
		SkillName = "Proteção Química Coletiva"
	},
	[SKID.BO_ADVANCE_PROTECTION] = {
		SkillName = "Proteção Sombria Total"
	},
	[SKID.BO_ACIDIFIED_ZONE_WATER] = {
		SkillName = "Atirar Ácido Álgido"
	},
	[SKID.BO_ACIDIFIED_ZONE_GROUND] = {
		SkillName = "Atirar Ácido Sólido"
	},
	[SKID.BO_ACIDIFIED_ZONE_WIND] = {
		SkillName = "Atirar Ácido Gasoso"
	},
	[SKID.BO_ACIDIFIED_ZONE_FIRE] = {
		SkillName = "Atirar Ácido Inflamável"
	},
	[SKID.BO_WOODENWARRIOR] = {
		SkillName = "Cultivar Bárbaro"
	},
	[SKID.BO_WOODEN_FAIRY] = {
		SkillName = "Cultivar Fada"
	},
	[SKID.BO_CREEPER] = {
		SkillName = "Cultivar Vinha"
	},
	[SKID.BO_RESEARCHREPORT] = {SkillName = "Eureka"},
	[SKID.BO_HELLTREE] = {
		SkillName = "Cultivar Árvore Infernal"
	},
	[SKID.TR_STAGE_MANNER] = {
		SkillName = "Presença de Palco"
	},
	[SKID.TR_RETROSPECTION] = {
		SkillName = "Retrospectiva"
	},
	[SKID.TR_MYSTIC_SYMPHONY] = {
		SkillName = "Sinfonia Mística"
	},
	[SKID.TR_KVASIR_SONATA] = {
		SkillName = "Sonata de Kvasir"
	},
	[SKID.TR_ROSEBLOSSOM] = {
		SkillName = "Atirar Rosas"
	},
	[SKID.TR_ROSEBLOSSOM_ATK] = {
		SkillName = "Atirar Rosas"
	},
	[SKID.TR_RHYTHMSHOOTING] = {
		SkillName = "Disparo Rítmico"
	},
	[SKID.TR_METALIC_FURY] = {
		SkillName = "Fúria Estridente"
	},
	[SKID.TR_SOUNDBLEND] = {
		SkillName = "Arranjo Musical"
	},
	[SKID.TR_GEF_NOCTURN] = {
		SkillName = "Recital de Geffenia"
	},
	[SKID.TR_ROKI_CAPRICCIO] = {
		SkillName = "Caprichos de Loki"
	},
	[SKID.TR_AIN_RHAPSODY] = {
		SkillName = "Rapsódia do Minerador"
	},
	[SKID.TR_MUSICAL_INTERLUDE] = {SkillName = "Interlúdio"},
	[SKID.TR_JAWAII_SERENADE] = {
		SkillName = "Serenata de Jawaii"
	},
	[SKID.TR_NIPELHEIM_REQUIEM] = {
		SkillName = "Réquiem de Nifflheim"
	},
	[SKID.TR_PRON_MARCH] = {
		SkillName = "Marcha de Prontera"
	},
	[SKID.EM_MAGIC_BOOK_M] = {
		SkillName = "Estudo Elemental"
	},
	[SKID.EM_SPELL_ENCHANTING] = {SkillName = "Enfeitiçar"},
	[SKID.EM_ACTIVITY_BURN] = {SkillName = "Exaurir"},
	[SKID.EM_INCREASING_ACTIVITY] = {
		SkillName = "Canalizar Energia"
	},
	[SKID.EM_DIAMOND_STORM] = {
		SkillName = "Execução Aurora"
	},
	[SKID.EM_LIGHTNING_LAND] = {SkillName = "Tormenta"},
	[SKID.EM_VENOM_SWAMP] = {
		SkillName = "Poço Venenoso"
	},
	[SKID.EM_CONFLAGRATION] = {
		SkillName = "Conflagração"
	},
	[SKID.EM_TERRA_DRIVE] = {
		SkillName = "Tremor de Terra"
	},
	[SKID.EM_ELEMENTAL_SPIRIT_M] = {
		SkillName = "Maestria Elemental"
	},
	[SKID.EM_SUMMON_ELEMENTAL_ARDOR] = {
		SkillName = "Invocar Ardor"
	},
	[SKID.EM_SUMMON_ELEMENTAL_DILUVIO] = {
		SkillName = "Invocar Diluvium"
	},
	[SKID.EM_SUMMON_ELEMENTAL_PROCELLA] = {
		SkillName = "Invocar Procella"
	},
	[SKID.EM_SUMMON_ELEMENTAL_TERREMOTUS] = {
		SkillName = "Invocar Terremotus"
	},
	[SKID.EM_SUMMON_ELEMENTAL_SERPENS] = {
		SkillName = "Invocar Serpens"
	},
	[SKID.EM_ELEMENTAL_BUSTER] = {
		SkillName = "Círculo Elemental"
	},
	[SKID.EM_ELEMENTAL_VEIL] = {
		SkillName = "Véu Elemental"
	},
	[SKID.NW_P_F_I] = {SkillName = "A.D.P"},
	[SKID.NW_GRENADE_MASTERY] = {
		SkillName = "Perícia em Granada"
	},
	[SKID.NW_INTENSIVE_AIM] = {
		SkillName = "Mira Focalizada"
	},
	[SKID.NW_GRENADE_FRAGMENT] = {
		SkillName = "Fragmentação"
	},
	[SKID.NW_THE_VIGILANTE_AT_NIGHT] = {
		SkillName = "Vigília Noturna"
	},
	[SKID.NW_ONLY_ONE_BULLET] = {
		SkillName = "Disparo Único"
	},
	[SKID.NW_SPIRAL_SHOOTING] = {
		SkillName = "Atirar em Espiral"
	},
	[SKID.NW_MAGAZINE_FOR_ONE] = {
		SkillName = "Artilharia Pesada"
	},
	[SKID.NW_WILD_FIRE] = {SkillName = "Tiroteio"},
	[SKID.NW_BASIC_GRENADE] = {
		SkillName = "Granada Explosiva"
	},
	[SKID.NW_HASTY_FIRE_IN_THE_HOLE] = {
		SkillName = "Gatilho Incandescente"
	},
	[SKID.NW_GRENADES_DROPPING] = {
		SkillName = "Chuva de Granadas"
	},
	[SKID.NW_AUTO_FIRING_LAUNCHER] = {
		SkillName = "Disparo Automático"
	},
	[SKID.NW_HIDDEN_CARD] = {
		SkillName = "Carta na Manga"
	},
	[SKID.NW_MISSION_BOMBARD] = {SkillName = "Bombardeio"},
	[SKID.SOA_TALISMAN_MASTERY] = {
		SkillName = "Perícia com Talismãs"
	},
	[SKID.SOA_SOUL_MASTERY] = {
		SkillName = "Maestria com Almas"
	},
	[SKID.SOA_TALISMAN_OF_PROTECTION] = {
		SkillName = "Talismã do Protetor"
	},
	[SKID.SOA_TALISMAN_OF_WARRIOR] = {
		SkillName = "Talismã do Guerreiro"
	},
	[SKID.SOA_TALISMAN_OF_MAGICIAN] = {
		SkillName = "Talismã do Mágico"
	},
	[SKID.SOA_SOUL_GATHERING] = {
		SkillName = "Convocar Almas"
	},
	[SKID.SOA_TOTEM_OF_TUTELARY] = {
		SkillName = "Totem de Tutela"
	},
	[SKID.SOA_TALISMAN_OF_FIVE_ELEMENTS] = {
		SkillName = "Talismã dos Elementos"
	},
	[SKID.SOA_TALISMAN_OF_SOUL_STEALING] = {
		SkillName = "Talismã do Ceifeiro"
	},
	[SKID.SOA_EXORCISM_OF_MALICIOUS_SOUL] = {
		SkillName = "Exorcizar Assombração"
	},
	[SKID.SOA_TALISMAN_OF_BLUE_DRAGON] = {
		SkillName = "Talismã do Dragão"
	},
	[SKID.SOA_TALISMAN_OF_WHITE_TIGER] = {
		SkillName = "Talismã do Tigre"
	},
	[SKID.SOA_TALISMAN_OF_RED_PHOENIX] = {
		SkillName = "Talimã da Fênix"
	},
	[SKID.SOA_TALISMAN_OF_BLACK_TORTOISE] = {
		SkillName = "Talismã do Jabuti"
	},
	[SKID.SOA_TALISMAN_OF_FOUR_BEARING_GOD] = {
		SkillName = "Talismã das Divindades"
	},
	[SKID.SOA_CIRCLE_OF_DIRECTIONS_AND_ELEMENTALS] = {
		SkillName = "Mandala das Feras"
	},
	[SKID.SOA_SOUL_OF_HEAVEN_AND_EARTH] = {
		SkillName = "Reencarnação das Almas"
	},
	[SKID.SH_MYSTICAL_CREATURE_MASTERY] = {
		SkillName = "Maestria Mística"
	},
	[SKID.SH_COMMUNE_WITH_CHUL_HO] = {
		SkillName = "União com Onça"
	},
	[SKID.SH_CHUL_HO_SONIC_CLAW] = {
		SkillName = "Garras de Onça"
	},
	[SKID.SH_HOWLING_OF_CHUL_HO] = {
		SkillName = "Rugido da Onça"
	},
	[SKID.SH_HOGOGONG_STRIKE] = {
		SkillName = "Patada Felina"
	},
	[SKID.SH_COMMUNE_WITH_KI_SUL] = {
		SkillName = "União com Tartaruga"
	},
	[SKID.SH_KI_SUL_WATER_SPRAYING] = {
		SkillName = "Esguichar Água"
	},
	[SKID.SH_MARINE_FESTIVAL_OF_KI_SUL] = {
		SkillName = "Festival Marinho"
	},
	[SKID.SH_SANDY_FESTIVAL_OF_KI_SUL] = {
		SkillName = "Luau nas Areias"
	},
	[SKID.SH_KI_SUL_RAMPAGE] = {
		SkillName = "Ira da Tartaruga"
	},
	[SKID.SH_COMMUNE_WITH_HYUN_ROK] = {
		SkillName = "União com Cervo"
	},
	[SKID.SH_COLORS_OF_HYUN_ROK] = {
		SkillName = "Cores do Arco-Íris"
	},
	[SKID.SH_HYUN_ROKS_BREEZE] = {
		SkillName = "Brisa Silvestre"
	},
	[SKID.SH_HYUN_ROK_CANNON] = {
		SkillName = "Lança Galhada"
	},
	[SKID.SH_TEMPORARY_COMMUNION] = {
		SkillName = "Aliança Animal"
	},
	[SKID.SH_BLESSING_OF_MYSTICAL_CREATURES] = {
		SkillName = "Bênção da Natureza"
	},
	[SKID.HN_SELFSTUDY_TATICS] = {
		SkillName = "Físico Autodidata"
	},
	[SKID.HN_SELFSTUDY_SOCERY] = {
		SkillName = "Mágico Autodidata"
	},
	[SKID.HN_DOUBLEBOWLINGBASH] = {
		SkillName = "Golpe de Tyr"
	},
	[SKID.HN_MEGA_SONIC_BLOW] = {
		SkillName = "Lâminas Devastadoras"
	},
	[SKID.HN_SHIELD_CHAIN_RUSH] = {
		SkillName = "Choque Violento"
	},
	[SKID.HN_SPIRAL_PIERCE_MAX] = {
		SkillName = "Cortar em Espiral"
	},
	[SKID.HN_METEOR_STORM_BUSTER] = {
		SkillName = "Chuva de Meteoritos"
	},
	[SKID.HN_JUPITEL_THUNDER_STORM] = {
		SkillName = "Tempestade de Júpiter"
	},
	[SKID.HN_JACK_FROST_NOVA] = {
		SkillName = "Esquife Congelante"
	},
	[SKID.HN_HELLS_DRIVE] = {
		SkillName = "Ira da Terra"
	},
	[SKID.HN_GROUND_GRAVITATION] = {
		SkillName = "Zona Gravitacional"
	},
	[SKID.HN_NAPALM_VULCAN_STRIKE] = {
		SkillName = "Espectro Napalm"
	},
	[SKID.HN_BREAKINGLIMIT] = {
		SkillName = "Anjo do Poder"
	},
	[SKID.HN_RULEBREAK] = {
		SkillName = "Anjo da Magia"
	},
	[SKID.SKE_SKY_MASTERY] = {
		SkillName = "Maestria Celestial"
	},
	[SKID.SKE_WAR_BOOK_MASTERY] = {
		SkillName = "Perícia com Livro"
	},
	[SKID.SKE_RISING_SUN] = {SkillName = "Amanhecer"},
	[SKID.SKE_NOON_BLAST] = {SkillName = "Entardecer"},
	[SKID.SKE_SUNSET_BLAST] = {
		SkillName = "Crepúsculo Explosivo"
	},
	[SKID.SKE_RISING_MOON] = {SkillName = "Anoitecer"},
	[SKID.SKE_MIDNIGHT_KICK] = {
		SkillName = "Chute Meia-Lua"
	},
	[SKID.SKE_DAWN_BREAK] = {SkillName = "Alvorada"},
	[SKID.SKE_TWINKLING_GALAXY] = {
		SkillName = "Constelação"
	},
	[SKID.SKE_STAR_BURST] = {
		SkillName = "Colapso Estelar"
	},
	[SKID.SKE_STAR_CANNON] = {
		SkillName = "Explosão Galática"
	},
	[SKID.SKE_ALL_IN_THE_SKY] = {SkillName = "Firmamento"},
	[SKID.SKE_ENCHANTING_SKY] = {
		SkillName = "Elo Celestial"
	},
	[SKID.SS_TOKEDASU] = {SkillName = "Derreter"},
	[SKID.SS_SHIMIRU] = {SkillName = "Infiltrar"},
	[SKID.SS_AKUMUKESU] = {
		SkillName = "Devorar Pesadelo"
	},
	[SKID.SS_SHINKIROU] = {SkillName = "Miragem"},
	[SKID.SS_KAGEGARI] = {
		SkillName = "Lâmina das Trevas"
	},
	[SKID.SS_KAGENOMAI] = {
		SkillName = "Dança das Trevas"
	},
	[SKID.SS_KAGEGISSEN] = {
		SkillName = "Centelha das Trevas"
	},
	[SKID.SS_FUUMASHOUAKU] = {
		SkillName = "Huuma Aderente"
	},
	[SKID.SS_FUUMAKOUCHIKU] = {
		SkillName = "Huuma Andarilho"
	},
	[SKID.SS_KUNAIWAIKYOKU] = {
		SkillName = "Kunai Distorcida"
	},
	[SKID.SS_KUNAIKAITEN] = {
		SkillName = "Kunai Rotatória"
	},
	[SKID.SS_KUNAIKUSSETSU] = {
		SkillName = "Kunai Refratada"
	},
	[SKID.SS_SEKIENHOU] = {
		SkillName = "Mahou da Chama Vermelha"
	},
	[SKID.SS_REIKETSUHOU] = {
		SkillName = "Mahou da Névoa Fria"
	},
	[SKID.SS_RAIDENPOU] = {
		SkillName = "Mahou da Trovoada"
	},
	[SKID.SS_KINRYUUHOU] = {
		SkillName = "Mahou do Dragão Áureo"
	},
	[SKID.SS_ANTENPOU] = {
		SkillName = "Mahou da Escuridão"
	},
	[SKID.SS_KAGEAKUMU] = {
		SkillName = "Pesadelo das Trevas"
	},
	[SKID.SS_HITOUAKUMU] = {
		SkillName = "Kunai do Pesadelo"
	},
	[SKID.SS_ANKOKURYUUAKUMU] = {
		SkillName = "Mahou do Pesadelo"
	},
	[SKID.NW_THE_VIGILANTE_AT_NIGHT_GUN_GATLING] = {
		SkillName = "Vigília Noturna"
	},
	[SKID.NW_THE_VIGILANTE_AT_NIGHT_GUN_SHOTGUN] = {
		SkillName = "Vigília Noturna"
	},
	[SKID.HLIF_HEAL] = {
		SkillName = "Cura pelas Mãos"
	},
	[SKID.HLIF_AVOID] = {
		SkillName = "Bater em Retirada"
	},
	[SKID.HLIF_BRAIN] = {
		SkillName = "Cirurgia Cerebral"
	},
	[SKID.HLIF_CHANGE] = {
		SkillName = "Esforço Mental"
	},
	[SKID.HAMI_CASTLE] = {
		SkillName = "Troca de Lugar"
	},
	[SKID.HAMI_DEFENCE] = {SkillName = "Fortaleza"},
	[SKID.HAMI_SKIN] = {
		SkillName = "Pele de Adamantium"
	},
	[SKID.HAMI_BLOODLUST] = {
		SkillName = "Desejo por Sangue"
	},
	[SKID.HFLI_MOON] = {SkillName = "Pica-Pau"},
	[SKID.HFLI_FLEET] = {
		SkillName = "Voo Frenético"
	},
	[SKID.HFLI_SPEED] = {
		SkillName = "Voo Acelerado"
	},
	[SKID.HFLI_SBR44] = {SkillName = "S.B.R.44"},
	[SKID.HVAN_CAPRICE] = {SkillName = "Capricho"},
	[SKID.HVAN_CHAOTIC] = {
		SkillName = "Bênção Caótica"
	},
	[SKID.HVAN_INSTRUCT] = {
		SkillName = "Mudança de Planos"
	},
	[SKID.HVAN_EXPLOSION] = {
		SkillName = "Bioexplosão"
	},
	[SKID.MH_SUMMON_LEGION] = {SkillName = "Convocação"},
	[SKID.MH_NEEDLE_OF_PARALYZE] = {
		SkillName = "Ponto de Paralisia"
	},
	[SKID.MH_POISON_MIST] = {
		SkillName = "Névoa Venenosa"
	},
	[SKID.MH_PAIN_KILLER] = {SkillName = "Analgésico"},
	[SKID.MH_LIGHT_OF_REGENE] = {
		SkillName = "Luz da Vida"
	},
	[SKID.MH_OVERED_BOOST] = {
		SkillName = "Superestimular"
	},
	[SKID.MH_ERASER_CUTTER] = {
		SkillName = "Choque Supersônico"
	},
	[SKID.MH_XENO_SLASHER] = {
		SkillName = "Onda Supersônica"
	},
	[SKID.MH_SILENT_BREEZE] = {
		SkillName = "Brisa da Calmaria"
	},
	[SKID.MH_STYLE_CHANGE] = {
		SkillName = "Estilo de Luta"
	},
	[SKID.MH_SONIC_CRAW] = {
		SkillName = "Garra Supersônica"
	},
	[SKID.MH_SILVERVEIN_RUSH] = {
		SkillName = "Rastro Brilhante"
	},
	[SKID.MH_MIDNIGHT_FRENZY] = {
		SkillName = "Delírio Noturno"
	},
	[SKID.MH_STAHL_HORN] = {
		SkillName = "Impacto Preciso"
	},
	[SKID.MH_GOLDENE_FERSE] = {
		SkillName = "Fúria Iluminada"
	},
	[SKID.MH_STEINWAND] = {
		SkillName = "Luz Salvadora"
	},
	[SKID.MH_HEILIGE_STANGE] = {
		SkillName = "Illuminatus"
	},
	[SKID.MH_ANGRIFFS_MODUS] = {
		SkillName = "Raiva Iluminada"
	},
	[SKID.MH_TINDER_BREAKER] = {
		SkillName = "Quebra-Costelas"
	},
	[SKID.MH_CBC] = {
		SkillName = "Dreno Traumático"
	},
	[SKID.MH_EQC] = {
		SkillName = "Estado Traumático"
	},
	[SKID.MH_MAGMA_FLOW] = {
		SkillName = "Inundação de Magma"
	},
	[SKID.MH_GRANITIC_ARMOR] = {
		SkillName = "Armadura de Granito"
	},
	[SKID.MH_LAVA_SLIDE] = {
		SkillName = "Derretimento com Lava"
	},
	[SKID.MH_PYROCLASTIC] = {
		SkillName = "Piroclástico"
	},
	[SKID.MH_VOLCANIC_ASH] = {
		SkillName = "Cinzas Vulcânicas"
	},
	[SKID.MH_BLAST_FORGE] = {
		SkillName = "Grande Fornalha"
	},
	[SKID.MH_TEMPERING] = {SkillName = "Têmpera"},
	[SKID.MH_CLASSY_FLUTTER] = {
		SkillName = "Voo Elegante"
	},
	[SKID.MH_TWISTER_CUTTER] = {
		SkillName = "Corte Supersônico"
	},
	[SKID.MH_ABSOLUTE_ZEPHYR] = {
		SkillName = "Zéfiro Absoluto "
	},
	[SKID.MH_BRUSHUP_CLAW] = {
		SkillName = "Garras Afiadas"
	},
	[SKID.MH_BLAZING_AND_FURIOUS] = {
		SkillName = "Veloz e Furiosa"
	},
	[SKID.MH_THE_ONE_FIGHTER_RISES] = {
		SkillName = "Lutadora Ascendente"
	},
	[SKID.MH_POLISHING_NEEDLE] = {
		SkillName = "Ferrão Polido"
	},
	[SKID.MH_TOXIN_OF_MANDARA] = {
		SkillName = "Toxina de Mandara"
	},
	[SKID.MH_NEEDLE_STINGER] = {SkillName = "Ferroada"},
	[SKID.MH_LICHT_GEHORN] = {
		SkillName = "Chifre Iluminado"
	},
	[SKID.MH_GLANZEN_SPIES] = {
		SkillName = "Ponta Reluzente"
	},
	[SKID.MH_HEILIGE_PFERD] = {SkillName = "Equitatio"},
	[SKID.MH_GOLDENE_TONE] = {
		SkillName = "Couro Dourado"
	},
	[SKID.MH_BLAZING_LAVA] = {
		SkillName = "Lava Quente "
	},
	[SKID.MS_BASH] = {
		SkillName = "Golpe Fulminante"
	},
	[SKID.MS_MAGNUM] = {
		SkillName = "Impacto Explosivo"
	},
	[SKID.MS_BOWLINGBASH] = {
		SkillName = "Impacto de Tyr"
	},
	[SKID.MS_PARRYING] = {
		SkillName = "Aparar Golpe"
	},
	[SKID.MS_REFLECTSHIELD] = {
		SkillName = "Escudo Refletor"
	},
	[SKID.MS_BERSERK] = {SkillName = "Frenesi"},
	[SKID.MA_DOUBLE] = {
		SkillName = "Rajada de Flechas"
	},
	[SKID.MA_SHOWER] = {
		SkillName = "Chuva de Flechas"
	},
	[SKID.MA_SKIDTRAP] = {
		SkillName = "Armadilha Escorregadia"
	},
	[SKID.MA_LANDMINE] = {
		SkillName = "Armadilha Atordoante"
	},
	[SKID.MA_SANDMAN] = {
		SkillName = "Armadilha Sonífera"
	},
	[SKID.MA_FREEZINGTRAP] = {
		SkillName = "Armadilha Congelante"
	},
	[SKID.MA_REMOVETRAP] = {
		SkillName = "Remover Armadilha"
	},
	[SKID.MA_CHARGEARROW] = {
		SkillName = "Disparo Violento"
	},
	[SKID.MA_SHARPSHOOTING] = {
		SkillName = "Tiro Preciso"
	},
	[SKID.ML_PIERCE] = {SkillName = "Perfurar"},
	[SKID.ML_BRANDISH] = {
		SkillName = "Brandir Lança"
	},
	[SKID.ML_SPIRALPIERCE] = {
		SkillName = "Perfurar em Espiral"
	},
	[SKID.ML_DEFENDER] = {
		SkillName = "Aura Sagrada"
	},
	[SKID.ML_AUTOGUARD] = {SkillName = "Bloqueio"},
	[SKID.ML_DEVOTION] = {SkillName = "Redenção"},
	[SKID.MER_MAGNIFICAT] = {SkillName = "Magnificat"},
	[SKID.MER_QUICKEN] = {
		SkillName = "Rapidez com Arma"
	},
	[SKID.MER_SIGHT] = {
		SkillName = "Chama Reveladora"
	},
	[SKID.MER_CRASH] = {SkillName = "Impacto"},
	[SKID.MER_REGAIN] = {SkillName = "Recompor"},
	[SKID.MER_TENDER] = {SkillName = "Amolecer"},
	[SKID.MER_BENEDICTION] = {SkillName = "Bendição"},
	[SKID.MER_RECUPERATE] = {
		SkillName = "Restabelecer"
	},
	[SKID.MER_MENTALCURE] = {
		SkillName = "Cura Mental"
	},
	[SKID.MER_COMPRESS] = {SkillName = "Compressa"},
	[SKID.MER_PROVOKE] = {SkillName = "Provocar"},
	[SKID.MER_AUTOBERSERK] = {
		SkillName = "Instinto de Sobrevivência"
	},
	[SKID.MER_DECAGI] = {
		SkillName = "Diminuir Agilidade"
	},
	[SKID.MER_SCAPEGOAT] = {
		SkillName = "Bode Expiatório"
	},
	[SKID.MER_LEXDIVINA] = {SkillName = "Lex Divina"},
	[SKID.MER_ESTIMATION] = {
		SkillName = "Sentido Sobrenatural"
	},
	[SKID.MER_KYRIE] = {
		SkillName = "Kyrie Eleison"
	},
	[SKID.MER_BLESSING] = {SkillName = "Bênção"},
	[SKID.MER_INCAGI] = {
		SkillName = "Aumentar Agilidade"
	},
	[SKID.MER_INVINCIBLEOFF2] = {
		SkillName = "Explosão da Mente"
	},
	[SKID.EL_CIRCLE_OF_FIRE] = {
		SkillName = "Círculo de Fogo"
	},
	[SKID.EL_FIRE_CLOAK] = {
		SkillName = "Capa de Fogo"
	},
	[SKID.EL_FIRE_MANTLE] = {
		SkillName = "Manto de Fogo"
	},
	[SKID.EL_WATER_SCREEN] = {
		SkillName = "Treliça de Água"
	},
	[SKID.EL_WATER_DROP] = {
		SkillName = "Gotas de Água"
	},
	[SKID.EL_WATER_BARRIER] = {
		SkillName = "Barreira de Água"
	},
	[SKID.EL_WIND_STEP] = {
		SkillName = "Passos do Vento"
	},
	[SKID.EL_WIND_CURTAIN] = {
		SkillName = "Cortina de Vento"
	},
	[SKID.EL_ZEPHYR] = {SkillName = "Zéfiro"},
	[SKID.EL_SOLID_SKIN] = {
		SkillName = "Pele de Pedra"
	},
	[SKID.EL_STONE_SHIELD] = {
		SkillName = "Escudo de Pedra"
	},
	[SKID.EL_POWER_OF_GAIA] = {
		SkillName = "Poder de Gaia"
	},
	[SKID.EL_PYROTECHNIC] = {SkillName = "Pirotecnia"},
	[SKID.EL_HEATER] = {
		SkillName = "Aquecimento"
	},
	[SKID.EL_TROPIC] = {
		SkillName = "Calor do Trópico"
	},
	[SKID.EL_AQUAPLAY] = {
		SkillName = "Truque Aquático"
	},
	[SKID.EL_COOLER] = {SkillName = "Geleira"},
	[SKID.EL_CHILLY_AIR] = {
		SkillName = "Ar Congelante"
	},
	[SKID.EL_GUST] = {SkillName = "Estática"},
	[SKID.EL_BLAST] = {
		SkillName = "Rajada de Vento"
	},
	[SKID.EL_WILD_STORM] = {
		SkillName = "Tempestade de Vento"
	},
	[SKID.EL_PETROLOGY] = {SkillName = "Geologia"},
	[SKID.EL_CURSED_SOIL] = {
		SkillName = "Solo Amaldiçoado"
	},
	[SKID.EL_UPHEAVAL] = {SkillName = "Tremores"},
	[SKID.EL_FIRE_ARROW] = {
		SkillName = "Flecha de Fogo"
	},
	[SKID.EL_FIRE_BOMB] = {
		SkillName = "Bomba de Fogo"
	},
	[SKID.EL_FIRE_BOMB_ATK] = {
		SkillName = "Bomba de Fogo"
	},
	[SKID.EL_FIRE_WAVE] = {
		SkillName = "Onda de Fogo"
	},
	[SKID.EL_FIRE_WAVE_ATK] = {
		SkillName = "Onda de Fogo"
	},
	[SKID.EL_ICE_NEEDLE] = {
		SkillName = "Agulha de Gelo"
	},
	[SKID.EL_WATER_SCREW] = {
		SkillName = "Parafuso de Água"
	},
	[SKID.EL_WATER_SCREW_ATK] = {
		SkillName = "Parafuso de Água"
	},
	[SKID.EL_TIDAL_WEAPON] = {
		SkillName = "Arma das Marés"
	},
	[SKID.EL_WIND_SLASH] = {
		SkillName = "Vento Cortante"
	},
	[SKID.EL_HURRICANE] = {
		SkillName = "Ira dos Furacões"
	},
	[SKID.EL_HURRICANE_ATK] = {
		SkillName = "Ira dos Furacões"
	},
	[SKID.EL_TYPOON_MIS] = {
		SkillName = "Lança-Ciclone"
	},
	[SKID.EL_TYPOON_MIS_ATK] = {
		SkillName = "Lança-Ciclone"
	},
	[SKID.EL_STONE_HAMMER] = {
		SkillName = "Martelo de Pedra"
	},
	[SKID.EL_ROCK_CRUSHER] = {
		SkillName = "Pedra Esmagadora"
	},
	[SKID.EL_ROCK_CRUSHER_ATK] = {SkillName = "Triturador"},
	[SKID.EL_STONE_RAIN] = {
		SkillName = "Chuva de Pedras"
	},
	[SKID.EM_EL_FLAMETECHNIC] = {
		SkillName = "Técnica das Chamas"
	},
	[SKID.EM_EL_FLAMEARMOR] = {
		SkillName = "Armadura de Lava"
	},
	[SKID.EM_EL_FLAMEROCK] = {
		SkillName = "Rocha Ígnea"
	},
	[SKID.EM_EL_COLD_FORCE] = {
		SkillName = "Força Gélida"
	},
	[SKID.EM_EL_CRYSTAL_ARMOR] = {
		SkillName = "Armadura de Cristal"
	},
	[SKID.EM_EL_AGE_OF_ICE] = {
		SkillName = "Era do Gelo"
	},
	[SKID.EM_EL_GRACE_BREEZE] = {
		SkillName = "Brisa Refrescante"
	},
	[SKID.EM_EL_EYES_OF_STORM] = {
		SkillName = "Armadura de Ar"
	},
	[SKID.EM_EL_STORM_WIND] = {
		SkillName = "Olho do Furacão"
	},
	[SKID.EM_EL_EARTH_CARE] = {
		SkillName = "Cuidado com a Terra"
	},
	[SKID.EM_EL_STRONG_PROTECTION] = {
		SkillName = "Armadura de Rocha"
	},
	[SKID.EM_EL_AVALANCHE] = {
		SkillName = "Desmoronamento"
	},
	[SKID.EM_EL_DEEP_POISONING] = {
		SkillName = "Serpente Venenosa"
	},
	[SKID.EM_EL_POISON_SHIELD] = {
		SkillName = "Armadura Tóxica"
	},
	[SKID.EM_EL_DEADLY_POISON] = {
		SkillName = "Peçonha Mortal"
	},
	[SKID.ABR_BATTLE_BUSTER] = {
		SkillName = "Eliminar Alvo"
	},
	[SKID.ABR_DUAL_CANNON_FIRE] = {
		SkillName = "Canhão Pulverizador"
	},
	[SKID.ABR_NET_REPAIR] = {SkillName = "Manutenção"},
	[SKID.ABR_NET_SUPPORT] = {
		SkillName = "Assistência Técnica"
	},
	[SKID.ABR_INFINITY_BUSTER] = {
		SkillName = "Aniquilação"
	},
	[SKID.GD_APPROVAL] = {
		SkillName = "Autorização Oficial"
	},
	[SKID.GD_KAFRACONTRACT] = {
		SkillName = "Contrato com Kafra"
	},
	[SKID.GD_GUARDRESEARCH] = {
		SkillName = "Comandar Guardiões"
	},
	[SKID.GD_GUARDUP] = {
		SkillName = "Fortalecer Guardiões"
	},
	[SKID.GD_EXTENSION] = {
		SkillName = "Expandir Clã"
	},
	[SKID.GD_GLORYGUILD] = {
		SkillName = "Gloria do Clã"
	},
	[SKID.GD_LEADERSHIP] = {
		SkillName = "Grande Liderança"
	},
	[SKID.GD_GLORYWOUNDS] = {
		SkillName = "Ferimentos de Glória"
	},
	[SKID.GD_SOULCOLD] = {
		SkillName = "Coração Frio"
	},
	[SKID.GD_HAWKEYES] = {
		SkillName = "Olhar Apurado"
	},
	[SKID.GD_BATTLEORDER] = {
		SkillName = "Comando de Batalha"
	},
	[SKID.GD_REGENERATION] = {
		SkillName = "Regeneração"
	},
	[SKID.GD_RESTORE] = {
		SkillName = "Restauração"
	},
	[SKID.GD_EMERGENCYCALL] = {
		SkillName = "Chamado Urgente"
	},
	[SKID.GD_DEVELOPMENT] = {
		SkillName = "Desenvolvimento Permanente"
	},
	[SKID.GD_ITEMEMERGENCYCALL] = {
		SkillName = "Chamado Urgente"
	},
	[SKID.GD_GUILD_STORAGE] = {
		SkillName = "Armazém do Clã"
	},
	[SKID.GD_CHARGESHOUT_FLAG] = {
		SkillName = "Marcar Território"
	},
	[SKID.GD_CHARGESHOUT_BEATING] = {
		SkillName = "Grito de Batalha"
	},
	[SKID.GD_EMERGENCY_MOVE] = {SkillName = "Disparada"}
}
